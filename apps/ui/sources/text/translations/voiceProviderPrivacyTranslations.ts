export const voiceProviderPrivacyTranslations = {
  ...
  de: {
    ...
    google: {
      privacyDisclosure: 'Audio zur Transkription verarbeitet Google Gemini, Text für die Sprachausgabe verarbeitet Google Cloud Text-to-Speech. Happier schickt diese Anfragen über den gewählten Ausführungsrechner mit dessen Google-API-Zugangsdaten. Google kann empfangene Daten gemäß den Einstellungen des gewählten Google-Kontos und den Bedingungen von Google aufbewahren.',
    },
    speechProcessing: {
      openaiCompatStt: 'Audio zur Transkription geht an den eingerichteten externen OpenAI-kompatiblen Endpunkt. Dieser Dienst verarbeitet das Audio und kann empfangene Daten gemäß seiner Konfiguration und seinen Bedingungen aufbewahren.',
      openaiCompatTts: 'Antworttext für die Sprachausgabe geht an den eingerichteten externen OpenAI-kompatiblen Endpunkt. Dieser Dienst verarbeitet den Text und kann empfangene Daten gemäß seiner Konfiguration und seinen Bedingungen aufbewahren.',
  ...
  ru: {
    ...
    google: {
      privacyDisclosure: 'Аудио для распознавания обрабатывается Google Gemini, а текст для озвучивания — Google Cloud Text-to-Speech. Happier отправляет эти запросы через выбранную машину, используя её учётные данные Google API. Google может хранить полученные данные согласно настройкам выбранной учётной записи Google и условиям Google.',
    },
    speechProcessing: {
  ...
  es: {
    ...
    google: {
      privacyDisclosure: 'El audio enviado para transcripción lo procesa Google Gemini, y el texto enviado para voz lo procesa Google Cloud Text-to-Speech. Happier envía estas solicitudes mediante la máquina seleccionada usando su credencial de Google API. Google puede conservar los datos recibidos según la configuración de la cuenta de Google seleccionada y sus términos.',
    },
    speechProcessing: {
  ...
  it: {
    ...
    google: {
      privacyDisclosure: 'L’audio inviato per la trascrizione viene elaborato da Google Gemini e il testo inviato per la sintesi vocale da Google Cloud Text-to-Speech. Happier invia queste richieste tramite la macchina selezionata usando le sue credenziali Google API. Google può conservare i dati ricevuti secondo le impostazioni dell’account Google selezionato e i termini di Google.',
    },
    speechProcessing: {
  ...
  pt: {
    ...
    google: {
      privacyDisclosure: 'O áudio enviado para transcrição é processado pelo Google Gemini e o texto enviado para voz pelo Google Cloud Text-to-Speech. O Happier envia estes pedidos através da máquina selecionada usando a credencial Google API dessa máquina. A Google pode reter os dados recebidos segundo as definições da conta Google selecionada e os termos da Google.',
    },
    speechProcessing: {
  ...
  'zh-Hans': {
    ...
    google: {
      privacyDisclosure: '发送用于转写的音频由 Google Gemini 处理，发送用于语音合成的文本由 Google Cloud Text-to-Speech 处理。Happier 通过所选执行机器并使用该机器的 Google API 凭据发送这些请求。Google 可能根据所选 Google 账户设置和 Google 条款保留收到的数据。',
    },
    speechProcessing: {
  ...
  'zh-Hant': {
    ...
    google: {
      privacyDisclosure: '送出以供轉錄的音訊由 Google Gemini 處理，送出以供語音合成的文字由 Google Cloud Text-to-Speech 處理。Happier 會透過所選執行機器並使用該機器的 Google API 憑證送出這些要求。Google 可能依所選 Google 帳戶設定與 Google 條款保留收到的資料。',
    },
    speechProcessing: {
  ...
  ja: {
    ...
    google: {
      privacyDisclosure: '文字起こし用に送信された音声は Google Gemini が処理し、音声合成用に送信されたテキストは Google Cloud Text-to-Speech が処理します。Happier は選択した実行マシンとそのマシンの Google API 認証情報を使ってこれらのリクエストを送信します。Google は選択した Google アカウント設定と Google の規約に従って受信データを保持する場合があります。',
    },
    speechProcessing: {

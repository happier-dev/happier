  ...
  directSessions: {
    ...
    localServices: {
        ...
        publicPreview: {
            ...
            secretLinkMode: '秘密連結',
            authenticatedMode: '登入後存取',
            publicMode: '擁有連結的任何人',
            expiredSubtitle: '連結已過期',
            expiresInMinutes: ({ count }: { count: number }) => `${count} 分鐘後過期`,
            expiresInHours: ({ count }: { count: number }) => `${count} 小時後過期`,
            expiresInDays: ({ count }: { count: number }) => `${count} 天後過期`,
            modePromptTitle: '連結類型',
            ttlPromptTitle: '連結有效期',
            ttlPromptMessage: ({ service }: { service: string }) => `到「${service}」的公開連結應保持有效多久?`,
            ttlOptionMinutes: ({ count }: { count: number }) => `${count} 分鐘`,
            ttlOptionHours: ({ count }: { count: number }) => `${count} 小時`,

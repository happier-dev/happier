...
export const pl = {
  ...
  automations: {
    ...
    form: {
      trigger: {
        ...
        executionReadOnly: 'Tylko odczyt',
      },
    ...
    detail: {
      ...
      event: {
        ...
        observationPlacementTitle: "Miejsce obserwacji zdarzeń",
  ...
  localServices: {
    ...
    publicPreview: {
      ...
      authenticatedMode: 'Dostęp po zalogowaniu',
      publicMode: 'Każdy, kto ma link',
      expiredSubtitle: 'Link wygasł',
      expiresInMinutes: ({ count }: { count: number }) => `Wygasa za ${count} min`,
      expiresInHours: ({ count }: { count: number }) => `Wygasa za ${count} godz.`,
      expiresInDays: ({ count }: { count: number }) => `Wygasa za ${count} dni`,
      modePromptTitle: 'Typ linku',
      ttlPromptTitle: 'Czas życia linku',
      ttlPromptMessage: ({ service }: { service: string }) => `Jak długo publiczny link do „${service}" ma pozostać ważny?`,
      ttlOptionMinutes: ({ count }: { count: number }) => `${count} minut`,
      ttlOptionHours: ({ count }: { count: number }) => `${count} godzin`,
      ttlOptionDays: ({ count }: { count: number }) => `${count} dni`,
  ...
  browserContext: {
    ...
    editor: {
      ...
      attach: 'Dołącz adnotację',
      cancel: 'Anuluj',
      captureUnavailable: 'Przechwytywanie adnotacji jest niedostępne dla tego widoku.',
      selectUnavailable: 'Wybór elementów jest niedostępny dla tego widoku.',
      marked: ({ count }: { count: string }) => `Oznaczono: ${count}`,
      removeMark: ({ label }: { label: string }) => `Usuń ${label}`,

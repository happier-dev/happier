...
settingsSession: {
  ...
  setupOnboarding: {
    ...
    relayOnThisMacTitle: "На этом Mac",
    relayOnThisMacSubtitle: "Запустите Relay локально на этом Mac и добавьте Tailscale для доступа с телефона",
    relayCustomUrlTitle: "Мой собственный URL Relay",
    ...
    webDesktopOnlyBody: "Откройте приложение для компьютера, чтобы настроить этот компьютер. Веб‑приложение может показывать статус, но не может установить или настроить фоновую службу.",
	    ...
	    preAuthBody: "Телефон отправляет сообщения через Relay на ваши компьютеры. Выберите, где будет работать Relay — это можно изменить позже.",

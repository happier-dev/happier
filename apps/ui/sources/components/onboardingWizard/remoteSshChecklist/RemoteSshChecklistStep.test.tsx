...
describe('RemoteSshChecklistStep', () => {
    ...
    it('defaults save-host off on non-desktop platforms (so password saving is never preselected)', async () => {
        ...
        const passwordAuthChoice = screen.findByProps({ testID: 'remote-ssh-step-ssh-sshAuthPassword' }) as unknown as { props: { onPress?: () => void } };
        await act(async () => {
    ...
    it('defaults save-host on for desktop (so users can opt-in to saving a private key after selecting keyfile auth)', async () => {
        ...
        const keyfileAuthChoice = screen.findByProps({ testID: 'remote-ssh-step-ssh-sshAuthKeyfile' }) as unknown as { props: { onPress?: () => void } };
        await act(async () => {
    ...
    it('forwards an encrypted private key from the wizard form into the bootstrap task spec when keyfile auth is selected', async () => {
        ...
        await act(async () => {
            await screen.pressByTestIdAsync('remote-ssh-step-save-private-key');
        });

        await act(async () => {
            screen.changeTextByTestId('remote-ssh-step-private-key', 'MY_PRIVATE_KEY');
        });

...
export type ScmReviewImagePreviewState =
...
export function useScmReviewImagePreview(input: Readonly<{
    ...
    const sizeBytes =
        typeof input.sizeBytes === 'number' && Number.isFinite(input.sizeBytes)
            ...
            : null;
    const sessionWorkspaceScope = useSessionWorkspaceTarget(input.source.kind === 'session' ? input.source.sessionId : null);
    const resolvedScope = React.useMemo<WorkspaceScopeBase | null>(() => {
        if (input.source.kind === 'workspace') return input.source.scope;
        return sessionWorkspaceScope;
    }, [input.source, sessionWorkspaceScope]);
    const cacheScopeId = React.useMemo(() => {
        if (input.source.kind === 'workspace') return input.source.scopeId;
        return sessionWorkspaceScope?.workspaceCacheKey ?? input.source.sessionId;
    }, [input.source, sessionWorkspaceScope]);
    ...
    const [state, setState] = React.useState<ScmReviewImagePreviewState>(() => {
    ...
    React.useEffect(() => {
        ...
        if (tooLarge) {
            ...
            if (canCache) {
                imagePreviewCache.set({ sessionId: cacheScopeId, signature: cacheKey!, filePath }, { status: 'error', error: errorMessage });
            }
        ...
        void (async () => {
            try {
                const res = await workspaceReadFile(resolvedScope, filePath);
                if (cancelled) return;
                ...
                if (canCache) {
                    imagePreviewCache.set({ sessionId: cacheScopeId, signature: cacheKey!, filePath }, { status: 'loaded', uri, svgXml });
                }

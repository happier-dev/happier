...
import { HISTORY_SLOTS, useVoiceLabEnergy } from './useVoiceLabEnergy';
import {
    PLANET_DARK,
    PLANET_LIGHT,
    VOICE_BREATH,
 ...
 * the GPU, and never needs a gradient mesh or a shader.
 */
...
export const Bloom = React.memo(function Bloom(props: BloomProps) {
    ...
    // Draw small, scale up. `blur` stays expressed in final-size points so
    // callers keep thinking in real units.
    const raster = Math.min(size, BLOOM_RASTER);
    const rasterScale = size / raster;
    const rasterBlur = blur / rasterScale;
    ...
    const animated = useAnimatedStyle(() => {
        ...
        // A coprime sub-pixel drift so nothing perceptibly loops over an hour.
        const drift = Math.sin(t * 0.43 + phase) * 0.0028 + Math.sin(t * 0.36 + phase * 1.7) * 0.0035;
        const breathScale = 1 + lung * 0.07 * breath + drift;
        ...
        >
            <View
                style={{
                    ...
                    height: raster,
                    borderRadius: raster,
                    backgroundColor: light(stop, alpha, tokens),
                    // `filter` is a first-class RN style prop and maps to the CSS
                    // filter on web. It is static, and the enlarging scale lives on
                    // an ancestor, so the blurred bitmap is rasterised once at
                    // 96×96 and only ever composited afterwards.
                    filter: `blur(${rasterBlur}px)`,
            ...
            style={[
                {
                    ...
                    // instead of sitting as a straight bar across the bounding
                    // box. Blur stays low or the 1.5pt edge dissolves.
                    borderWidth: 1.5,
...
export const DigitField = React.memo(function DigitField(props: Readonly<{
    ...
    return (
        <View
            pointerEvents="none"
            style={{ position: 'absolute', inset: 0, overflow: 'hidden', opacity: props.opacity ?? 0.5 }}
        >
            <Text
                selectable={false}
...
const OrbRing = React.memo(function OrbRing(props: Readonly<{
    ...
    size: number;
    stop: VoiceLightStop;
    tokens: ReturnType<typeof useVoiceLabTokens>;
...
export const PlanetOrb = React.memo(function PlanetOrb(props: Readonly<{
    ...
    const body = useAnimatedStyle(() => {
        ...
        const scale = 1 + lung * depth + speech;
                ...
                style={[
                    {
                        ...
                        left: '-30%',
                        width: '30%',
                        backgroundColor: light(props.stop, 1, tokens),
                        filter: 'blur(3px)',
                    },
                    traveller,
                ]}

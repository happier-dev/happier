...
const createTestProps = (): AgentAuthenticationTerminalPaneProps => ({
    agentId: 'claude',
    machineId: 'machine-1',
    machineHomeDir: '/Users/tester',
    loginLaunch: {
...
const agentAuthenticationTerminalPaneModulePromise = import('./AgentAuthenticationTerminalPane');

describe('AgentAuthenticationTerminalPane', () => {
    beforeEach(() => {
    ...
    it('re-sends provider initial input after a reconnect cycle', () => {
        return agentAuthenticationTerminalPaneModulePromise.then(async (module) => {
            const { AgentAuthenticationTerminalPane } = module;
            const screen = await renderScreen(<AgentAuthenticationTerminalPane {...createTestProps()} />);

            expect(onInputMock).not.toHaveBeenCalled();

            await act(async () => {
                mockTerminalStatus = 'connected';
                screen.tree.update(<AgentAuthenticationTerminalPane {...createTestProps()} />);
            });

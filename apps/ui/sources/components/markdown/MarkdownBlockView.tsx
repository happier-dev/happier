...
type MarkdownHeaderLevel = Extract<MarkdownBlock, { type: 'header' }>['level'];
...
function RenderHeaderBlock(props: { level: 1 | 2 | 3 | 4 | 5 | 6, spans: MarkdownSpan[], first: boolean, last: boolean, selectable: boolean, onLinkPress?: (url: string) => boolean | void, textStyle?: StyleProp<TextStyle>, variant: 'default' | 'thinking', streamingReveal: boolean, streamingRevealPreset?: StreamingTextRevealPreset }) {
    const s = resolveMarkdownHeaderStyle(props.level);
...
const markdownHeaderStylesByLevel = {
    1: style.header1,
    2: style.header2,
    3: style.header3,
    4: style.header4,
    5: style.header5,
    6: style.header6,
} satisfies Record<MarkdownHeaderLevel, StyleProp<TextStyle>>;

function resolveMarkdownSpanStyle(
    styleId: MarkdownSpanStyleId,
    variant: MarkdownBlockViewProps['variant']
): StyleProp<TextStyle> {
    if (variant === 'thinking' && styleId === 'code') {
        return style.thinkingInlineCode;
    }

    return markdownSpanStylesById[styleId];
}

function resolveMarkdownHeaderStyle(level: MarkdownHeaderLevel): StyleProp<TextStyle> {
    return markdownHeaderStylesByLevel[level];
}

 ...
 *
 * MEASURED on device (iOS simulator, rAF sampler on the mounted LegendList fiber, trace batch
 * 2026-08-09, `.project/reviews/2026-08-09-residual-wobble/ATTRIBUTE.md` §M2/M3): a one-line
 * committed USER row paints **62.0px** (12/12 W1 trials, 10/10 W2) and a one-line AGENT reply
 * paints **46px** (10/10, `tail=msg:*\/46`). The estimator answered **54** for BOTH — 8px under for
 * the user row and 8px over for the agent row — producing `requestAdjust(-14.2)` at the handover
 * and a compensating `requestAdjust(+8)` after the bubble already looked settled.
 *
 * Each measured height decomposes exactly against the styles that produce it, which is what pins
 * the two bases below to source rather than to a calibration:
 *
...
/** MEASURED — one-line committed user row, iOS simulator 2026-08-09 (12/12 and 10/10). */
const MEASURED_ONE_LINE_USER_ROW_PX = 62;
/** MEASURED — one-line committed agent row, same batch (10/10). */

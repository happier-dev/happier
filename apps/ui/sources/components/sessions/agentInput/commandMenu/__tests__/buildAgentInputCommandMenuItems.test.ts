...
describe('buildAgentInputCommandMenuItems', () => {
    ...
    it('qualifies row ids by kind when providers reuse a local candidate id', () => {
        const items = buildAgentInputCommandMenuItems([
            { kind: 'file', key: 'shared', text: '@README.md' },
            { kind: 'vendorPlugin', key: 'shared', text: '@plugin', label: 'Plugin' },
        ]);
    ...
    it('uses declared composer-reference presentation in the existing CommandMenu row', () => {
        const declaredIcon = React.createElement('span', { 'data-icon': 'declared' });
        const items = buildAgentInputCommandMenuItems([{

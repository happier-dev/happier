 ...
 *
 * It rides the same `composerAttachmentBadge` channel as the review-comments
 * chip, but it is NOT a Session mention: a mention says "this Session is
 * available to inspect", while this says "create this Session as a continuation,
 * with bounded Replay through the cutoff". Removing it turns the draft back into
 * an ordinary new Session and touches nothing else the user configured.
 */
export function createSessionSourceContextActionChip(params: Readonly<{
    ...
    return {
        ...
        composerAttachmentBadge: {
            ...
            // The predecessor badge has no invalid/error presentation, so the
            // mismatch is carried by the announced label and the detail sheet,
            // and submission is blocked by the screen model — the recipe is
            // never silently dropped at submit.

 ...
 *
 * Overlay routes (`/new`, `/direct/browse`, the zen modals, and their sub-routes) are presented
 * *over* the current screen rather than replacing it: on web expo-router's modal stack filters them
 * out of the state handed to the stack view and renders them as a scaled drawer overlay, so the
 * previous screen stays mounted **and visible**; on native they are `pageSheet`/`modal`/
 * `containedModal` presentations that likewise leave the previous screen on screen.
 *
 ...
 * route, otherwise the last non-overlay pathname observed. Surfaces key their visibility/data
 * decisions off the anchor, and their *interactivity* off the raw pathname (an overlay must still
 * block clicks on what is behind it).
 */

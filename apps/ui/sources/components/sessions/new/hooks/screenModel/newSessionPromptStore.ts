 ...
 * render graph.
 *
 * The new-session screen model is a large hook tree (~370 hook slots across ~25
 * sub-hooks). While the prompt was `React.useState` inside that tree, every keystroke
 * re-executed the whole model, rebuilt both screen-variant prop bundles, and
 * invalidated `React.memo` on the panel — for a value that only the composer input and
 * a handful of imperative callbacks (submit, send, draft persistence) actually read.
 *

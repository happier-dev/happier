...
/**
 * Target/bullseye goal glyph shared by the AgentInput goal chip and the goal control surface, so
 * both the chip affordance and the set/read forms render one canonical icon (single owner — no
 * duplicated SVG path data across the goal domain).
 */
...
export function GoalIcon(props: Readonly<{ color: string; size?: number }>): React.ReactElement {
    const size = props.size ?? 18;
    return (
        <Svg width={size} height={size} viewBox="0 0 24 24" fill="none">
            {GOAL_ICON_PATHS.map((d) => (

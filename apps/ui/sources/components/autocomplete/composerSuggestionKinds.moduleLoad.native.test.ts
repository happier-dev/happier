 ...
 *
 * Measured on an iOS dev client (2026-08-10) by wrapping `globalThis.__loadBundleAsync`
 * and driving the real composer:
 *
...
// The registry's row-rendering leaves are irrelevant to WHEN a dependency loads, and
// pulling their real graphs costs more than the contract under test. They are stubbed
// so this file stays a module-loading test; the two mocks below are the subject.
...
describe('composer suggestion registry — native module loading', () => {
    it('loads the file-search and prompt-invocation modules with the registry itself', async () => {
        ...
        // The registry statically imports `@happier-dev/protocol`; transforming that
        // package on a cold Vite cache dominates this file's runtime, so the default
        // 60 s budget is not enough to import the module under test even once.
    }, 600_000);
});

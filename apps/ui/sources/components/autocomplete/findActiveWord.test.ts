...
describe('findActiveWord token boundaries (EU-1)', () => {
    ...
    it.each<ActiveWordCase>([
        ...
        {
            name: 'keeps a public reference label quoted under the skill trigger',
            content: '$"my skill"',
            selection: { start: 11, end: 11 },
            expected: expectedActiveWord({ word: '$"my skill"', offset: 0, endOffset: 11 }),
        },
        {
...
describe('findActiveWord host eligibility (R-9, INV-1)', () => {
    const SESSION_KINDS: readonly ComposerSuggestionKindId[] = ['file', 'vendorPlugin', 'skill', 'slashCommand'];
    const PARTICIPANT_KINDS: readonly ComposerSuggestionKindId[] = ['file', 'vendorPlugin', 'slashCommand'];
    const NEW_SESSION_KINDS: readonly ComposerSuggestionKindId[] = ['slashCommand'];

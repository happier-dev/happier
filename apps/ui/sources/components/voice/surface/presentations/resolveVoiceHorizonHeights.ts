...
export function resolveVoiceHorizonHeights(input: VoiceHorizonHeightsInput): VoiceHorizonHeights {
    ...
    /*
     * The scaffold has already subtracted the header, composer, keyboard and
     * safe area. Unlike the heuristic host-chrome reserve above, that published
     * region is a physical allocation shared with interactive siblings, so the
     * vessel may not borrow through it. When measured status/actions need more,
     * the information region shrinks (and the transcript remains its one scroll
     * owner) while the pinned action layer stays reachable inside the vessel.
     */

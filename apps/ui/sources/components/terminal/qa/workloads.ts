export const TERMINAL_QA_WORKLOAD_IDS = [
    'ansi-burst',
    'heavy-tui-redraw',
    'alternate-screen',
    'cursor-style-churn',
...
export type TerminalQaWorkloadId = typeof TERMINAL_QA_WORKLOAD_IDS[number];

export type TerminalQaWorkload = Readonly<{
    id: TerminalQaWorkloadId;
    description: string;
    bytes: Uint8Array;
    byteLength: number;
}>;
...
function workload(
    ...
    return Object.freeze({ id, description, bytes, byteLength: bytes.byteLength });
}
...
const WORKLOADS: readonly TerminalQaWorkload[] = Object.freeze([
    workload(
        'ansi-burst',
...
export function listTerminalQaWorkloads(): readonly TerminalQaWorkload[] {
    return WORKLOADS;
}

export function getTerminalQaWorkload(id: TerminalQaWorkloadId): TerminalQaWorkload {
    const match = WORKLOADS.find((candidate) => candidate.id === id);
    if (!match) throw new Error(`Unknown terminal QA workload: ${id}`);
    return match;
}

export function encodeTerminalQaText(value: string): Uint8Array {

...
export function useFullscreenDetailsRouteParamSync(
    input: UseFullscreenDetailsRouteParamSyncInput,
): void {
    ...
    React.useEffect(() => {
        ...
        if (pendingWriteSignature) {
            if (input.routeSelectionSignature !== pendingWriteSignature) {
                return;
            }
            pendingRouteWriteSignatureRef.current = null;
        }
        ...
        if (!routeDidChange && !isReconcilingCurrentRoute) {
            return;
        }

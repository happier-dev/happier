...
const BUILT_IN_AGENT_IDS = new Set<string>(agents.AGENT_IDS);

export function listProviderUniverseIds(): readonly AgentId[] {
    // Some Vitest tests mock `@happier-dev/agents` with a minimal export set and omit
    // `getAllProviderDefinitions`. Accessing a missing export on a Vitest mock throws, so
    // we must check export presence before reading/calling it.
    if ('getAllProviderDefinitions' in agents) {
        const maybeGetAllProviderDefinitions = (agents as unknown as {
            getAllProviderDefinitions?: () => readonly { id: string }[];
...
export function buildProviderUniverseBackendTargetKey(id: string): string {
    const normalizedId = id.trim();
    return resolveBackendTargetKeyV2({ kind: 'backend', backendId: normalizedId });
}

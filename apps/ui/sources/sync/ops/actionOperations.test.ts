...
describe('action operation transport', () => {
    it('maps and validates list/get/cancel through the versioned observation RPC methods', async () => {
    ...
    it.each([
        ...
        ['cancel', cancelActionOperation, { machineId: 'machine-1', operationId: 'operation-1' }],
    ] as const)('rejects malformed %s responses', async (_name, invoke, params) => {
        const rpc = createRpc(async () => ({ malformed: true }));

...
describe('apiPush retry modes', () => {
    ...
    it('does not retry registerPushToken when retry mode is none', async () => {
        vi.useFakeTimers();
        ...
        runtimeFetchSpy.mockImplementation(async (url: unknown) => {
            ...
            if (href.includes('/v1/push-tokens')) return new Response('nope', { status: 408 });
        ...
        const promise = (
            registerPushToken as unknown as (
                credentials: AuthCredentials,
                token: string,
                opts?: { retry?: string }
            ) => Promise<unknown>
        )(credentials, 'ExponentPushToken[redacted]', { retry: 'none' });

        const assertion = expect(promise).rejects.toThrow();
        await vi.runAllTimersAsync();
        await assertion;

        const requestedUrls = runtimeFetchSpy.mock.calls.map(([url]) => String(url ?? ''));
        const pushTokenRequests = requestedUrls.filter((href) => href.includes('/v1/push-tokens'));
        expect(pushTokenRequests).toHaveLength(1);
    });
});

...
function lengthPreservingItems(items: readonly (unknown | null)[], count: number): Array<unknown | null> {
    return Array.from({ length: count }, (_value, index) => index < items.length ? items[index] : null);
}
...
export async function decryptSecretboxJsonBatchWithNativeWorker(
    ...
    if (!shouldUseNativeWorkerAfterCheapChecks(routing, data.length)) {
        return lengthPreservingItems(await referenceRun(), data.length);
    }
    ...
    return result.status === 'ok' ? lengthPreservingItems(result.items, data.length) : nullItems(data.length);
}
...
export async function decryptSecretboxJsonBase64BatchWithNativeWorker(
    ...
    if (!shouldUseNativeWorkerAfterCheapChecks(routing, ciphertextBase64.length)) {
        return lengthPreservingItems(await referenceRun(), ciphertextBase64.length);
    }
    ...
    const result = await runNativeCryptoWorkerBatch<unknown | null>({
        ...
        referenceRun,
        isScopeCurrent: binding.isScopeCurrent
            ? () => capturedScope !== null && binding.isScopeCurrent?.(capturedScope) === true
            : undefined,
    });

    return result.status === 'ok' ? lengthPreservingItems(result.items, ciphertextBase64.length) : nullItems(ciphertextBase64.length);
}
...
export async function decryptAesGcmJsonBase64BatchWithNativeWorker(
    ...
    if (!shouldUseNativeWorkerAfterCheapChecks(routing, encryptedPayloadBase64.length)) {
        return lengthPreservingItems(await referenceRun(), encryptedPayloadBase64.length);
    }
    ...
    return result.status === 'ok' ? lengthPreservingItems(result.items, encryptedPayloadBase64.length) : nullItems(encryptedPayloadBase64.length);
}

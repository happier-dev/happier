...
} from './nativeCryptoWorker/nativeCryptoWorkerFallbackReport';
...
describe('Encryption native crypto worker routing default', () => {
    ...
    it('routes a batch through the worker on an instance nobody configured routing on', async () => {
        ...
        // constructed by createEncryptionFromAuthCredentials and never handed to
        // sync.ts#configureEncryptionRuntime. Only the worker is injected here — the
        // routing must come from construction.
    ...
    it('keeps a single data-key envelope on the JS thread: one open is cheaper than one bridge crossing', async () => {
        ...
        // ...and it says so. A batch that stays on JS by configuration is otherwise
        // indistinguishable in a profile from one that stayed there because the
        // worker is broken, which is how this exclusion went unnoticed.
        expect(readNativeCryptoWorkerFallbackDiagnostics()).toMatchObject({
            ...
            lastRoutingDeclineOperation: 'decryptDataKeyEnvelopeV1',
        });
    });

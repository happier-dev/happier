...
describe('connectedServiceRegistry', () => {
  ...
  it('projects every declared authentication mode and preserves the default mode', () => {
    ...
    expect(getConnectedServiceRegistryEntry('multi-account')).toMatchObject({
      executable: true,
      supportsToken: true,
      supportsOauth: true,
  ...
  it('consumes projected plugin descriptors through the canonical registry', () => {
    install([{ id: 'bitbucket-account', serviceId: 'bitbucket', pluginId: 'scm-bitbucket', provenance: 'external', sourceKind: 'bundled', title: 'Bitbucket account', authentication: manualAuthentication([{ id: 'token', title: 'Token', secret: true }]), capabilities: [], availability: { state: 'available', reason: 'resolved' }, diagnostics: [] }]);
  ...
  it('preserves rich blocked descriptor facts while keeping the service visible and non-executable', () => {
    const projected = {
      id: 'bitbucket-account', serviceId: 'bitbucket' as const, pluginId: 'scm-bitbucket', provenance: 'external' as const,
  ...
  it('retains last-known-good facts as stale and reports lifecycle without making them executable', () => {
    const projected = { id: 'bitbucket-account', serviceId: 'bitbucket' as const, pluginId: 'scm-bitbucket', provenance: 'external' as const, sourceKind: 'bundled', title: 'Bitbucket', authentication: manualAuthentication([{ id: 'token', title: 'Token', secret: true }]), capabilities: [], availability: { state: 'available' as const, reason: 'resolved' }, diagnostics: [] };
  ...
  it('keeps unrelated qualified services executable when one service ownership conflicts', () => {
    ...
    installConnectedAccountDescriptorProjection({
      ...
      conflicts: [{
        ...
        descriptorIdentities: [
          'acme.vertical-a\u0000github',
          'happier.scm.hosting.github\u0000github-account',
        ],
      }],
      errorReason: null,
    });
    ...
    expect(getConnectedServiceRegistryEntry('forge')).toMatchObject({
      service: { pluginId: 'acme.vertical-a', localId: 'novel-cloud' },
      executable: true,
      projectionStatus: 'ready',
    });
    expect(getConnectedServiceRegistryEntry('anthropic')).toMatchObject({
  ...
  it('projects Bitbucket API-token setup metadata from descriptors', () => {
    install([{
      id: 'bitbucket-account', serviceId: 'bitbucket', pluginId: 'scm-bitbucket', provenance: 'external', sourceKind: 'bundled',

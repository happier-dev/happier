...
import {
    ...
    deriveAgentTeamHintFromSubagentMessages,
} from './hints';

type HistoricalAgentTeamMember = {
    teamId: string;
    memberId: string;
    memberLabel?: string;
    memberColor?: string;
...
type ActiveAgentTeamMember = Readonly<{
    memberId: string;
    memberLabel?: string;
    memberColor?: string;
}>;
...
function deriveHistoricalAgentTeamMembers(
    descriptor: AgentTeamSessionProviderDescriptor,
    messages: readonly AgentTeamMessage[],
): Map<string, HistoricalAgentTeamMember> {
    const historicalMembers = new Map<string, HistoricalAgentTeamMember>();
    ...
    for (const message of messages) {
        ...
        if (!isAgentTeamSubagentSpawnToolName(descriptor, toolName)) continue;

        const spawned =
            coerceSpawnedTeammate(asRecord(toolMessage.tool.result)?.tool_use_result)
        ...
        const hasExplicitSpawnSignal =
            !isAgentTeamActiveTeamFallbackSubagentSpawnToolName(descriptor, toolName)
            && (
                deriveAgentTeamSpawnedTeammateFromTaskToolResult(toolMessage.tool.result) !== null
                || deriveAgentTeamSpawnedTeammateFromTaskToolInput(toolMessage.tool.input) !== null
            );
        const routePriority = hasExplicitSpawnSignal
            ? 3
            : isAgentTeamActiveTeamFallbackSubagentSpawnToolName(descriptor, toolName)
                ? 2
                : 1;
...
export function deriveAgentTeamSubagents(params: Readonly<{
    descriptor: AgentTeamSessionProviderDescriptor;
    flavor: string | null;
    messages: readonly AgentTeamMessage[];
}>): readonly AgentTeamSessionSubagent[] {
    const hasToolSignal = messagesContainAgentTeamToolSignal(params.descriptor, params.messages);
    if (!hasAgentTeamFlavor(params.descriptor, params.flavor) && !hasToolSignal) return [];

    const snapshot = deriveAgentTeamParticipants({ descriptor: params.descriptor, messages: params.messages });
    const participantHint = !hasToolSignal ? deriveAgentTeamHintFromParticipantMessages(params.messages) : null;
    const subagentHint = deriveAgentTeamHintFromSubagentMessages(params.messages);
    const activeTeamId = snapshot.teamId ?? subagentHint?.teamId ?? participantHint?.teamId ?? null;
    const activeMembersByKey = new Map<string, ActiveAgentTeamMember>();
    ...
    const activeMembersByIdentity = new Map<string, ActiveAgentTeamMember>(
        activeMembers.map((member) => [`${activeTeamId ?? ''}:${member.memberId}`, member]),
    );
    const historicalMembers = deriveHistoricalAgentTeamMembers(
        params.descriptor,
        params.messages.map((message) => {
            if (!message || message.kind !== 'tool-call') return message;
            const toolMessage = message as AgentTeamToolCallMessage;
            if (!isAgentTeamActiveTeamFallbackSubagentSpawnToolName(params.descriptor, toolMessage.tool?.name) || activeTeamId == null) {
                return message;
            }
            const inputRecord =
    ...
    return Array.from(historicalMembers.values()).map((member) => {
        ...
        return {
            ...
            display: {
                title: displayLabel,
                providerLabel: params.descriptor.providerLabel,
        ...
        } satisfies AgentTeamSessionSubagent;
    });
}

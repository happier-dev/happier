...
const currentActivationBySessionId = new Map<string, number>();
...
function refreshCurrentActivation(sessionId: string): void {
    const activeForSession = activeActivationsBySessionId.get(sessionId);
    if (!activeForSession || activeForSession.size === 0) {
        currentActivationBySessionId.delete(sessionId);
        return;
    }
    let current: number | null = null;
    for (const activationId of activeForSession) {
        current = current === null ? activationId : Math.max(current, activationId);
    }
    if (current !== null) {
        currentActivationBySessionId.set(sessionId, current);
    }
}
...
export function beginSessionViewingActivation(sessionId: string): number {
    ...
    currentActivationBySessionId.set(sessionId, activationId);
...
export function endSessionViewingActivation(sessionId: string, activationId: number): void {
    ...
    refreshCurrentActivation(sessionId);
}

export function getCurrentSessionViewingActivationId(sessionId: string): number | null {
    return currentActivationBySessionId.get(sessionId) ?? null;
}

export function getCurrentViewingSessionId(): string | null {
    let currentSessionId: string | null = null;
    let currentActivationId: number | null = null;
    for (const [sessionId, activationId] of currentActivationBySessionId) {
        if (currentActivationId === null || activationId > currentActivationId) {
...
export function clearSessionViewingActivationsForServerScopeReset(): void {
    activeActivationsBySessionId.clear();
    currentActivationBySessionId.clear();
    holdsByKey.clear();
}

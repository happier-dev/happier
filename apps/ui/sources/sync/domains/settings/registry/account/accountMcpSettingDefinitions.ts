...
function buildMcpSummaryProperties(value: unknown): Record<string, boolean | number> {
    ...
    for (const server of servers) {
        ...
        if (serverRecord.transport === 'stdio')
            stdioCount += 1;
        if (serverRecord.transport === 'http')
            httpCount += 1;
        if (serverRecord.transport === 'sse')
            sseCount += 1;
        savedSecretRefCount += countSavedSecretRefs(serverRecord.env);
        if (serverRecord.remote
    ...
    for (const binding of bindings) {
        ...
        if (bindingRecord.target && typeof bindingRecord.target === 'object' && !Array.isArray(bindingRecord.target)) {
            ...
            if (targetRecord.t === 'allMachines')
                allMachinesCount += 1;
            if (targetRecord.t === 'machine')
                machineTargetCount += 1;
            if (targetRecord.t === 'workspace')
                workspaceTargetCount += 1;
        }
        if (bindingRecord.overrides
...
export const ACCOUNT_MCP_SETTING_ANALYTICS = defineAccountSettingAnalytics({

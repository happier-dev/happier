...
async function resetDestinationAfterCandidateFailure(destination: BulkTransferFileDestination): Promise<void> {
    if (!destination.cleanup) {
        return;
    }
        ...
        try {
            ...
            if (!download.ok) {
                if (hasMoreCandidates) {
                    await resetDestinationAfterCandidateFailure(params.destination);
                    continue;
                }
                break;
            }

...
function makeFile(path: string): GitFileStatus {
    return {
...
function makeLog(
    entry: Partial<GitProjectOperationLogEntry> & Pick<GitProjectOperationLogEntry, 'operation' | 'status' | 'timestamp'>
...
function confidenceForPath(
    attributed: Array<{ file: GitFileStatus; confidence: SessionAttributionConfidence }>,
    path: string

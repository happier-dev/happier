...
import { HappierFocusWidgetComponent } from './HappierFocusWidget';
        ...
        sessions: [
            createSessionFixture({
                ...
                metadata: {
                    ...
                    summary: { text: 'Need your approval', updatedAt: 2 },
                },
            }),
            createSessionFixture({
                id: 'secondary',
...
describe('HappierFocusWidget', () => {
    it('uses a single hero surface on systemSmall instead of separate primary and inbox CTA buttons', () => {
        const rendered = HappierFocusWidgetComponent(createSnapshot(), { widgetFamily: 'systemSmall' } as never);
        const buttons = collectButtons(rendered);

        expect(buttons).toHaveLength(1);
    });

    it('uses session surfaces on systemMedium without an extra inbox CTA row', () => {
        const rendered = HappierFocusWidgetComponent(createSnapshot(), { widgetFamily: 'systemMedium' } as never);
        const buttons = collectButtons(rendered);

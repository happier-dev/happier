...
type FetchWithTimeoutArgs = [input: RequestInfo | URL, init?: RequestInit];

const fetchWithTimeout = vi.fn<(...args: FetchWithTimeoutArgs) => Promise<{ ok: true; json: () => Promise<{ text: string }> }>>(
  async () => ({
    ok: true,

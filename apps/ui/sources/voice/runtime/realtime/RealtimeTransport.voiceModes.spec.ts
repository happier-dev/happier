...
import { installRealtimeCommonModuleMocks } from '@/realtime/realtimeTestHelpers';
...
vi.mock('@/realtime/elevenLabsByo', () => ({
...
async function loadRealtimeTransport() {
...
describe('Realtime voice modes', () => {
  ...
  beforeEach(() => {
    ...
    vi.doMock('@/voice/binding/voiceConversationBindingRuntime', () => ({
    ...
    vi.doMock('@/voice/transcript/voiceConversationTranscript', () => ({
  ...
  describe('provider selection', () => {
    it('does nothing when voice provider is off', async () => {
      ...
      expect(syncVoiceBindingTarget).not.toHaveBeenCalled();
    });
  ...
  describe('happier voice lifecycle', () => {
    ...
    it('starts realtime voice in text-only mode when explicitly requested', async () => {
      ...
      await realtimeTransport.startRealtimeSession('s1', 'hi', false, { textOnly: true });
    ...
    it('treats the global voice sentinel as a control id and not a target session id', async () => {
      ...
      await realtimeTransport.startRealtimeSession('__voice_agent__', 'hi', false, { textOnly: true });
    ...
    it('does not mark the voice session started when provider returns no conversation id', async () => {
      ...
      expect(realtimeTransport.isVoiceSessionStarted()).toBe(false);
    ...
    it('retries after paywall purchase without deadlocking', async () => {
      ...
      const startPromise = realtimeTransport.startRealtimeSession('s1', 'hi');
  ...
  describe('concurrency and stop behavior', () => {
    it('dedupes concurrent start calls (account-scoped)', async () => {
      ...
      const p1 = realtimeTransport.startRealtimeSession('s1', 'hi');
      const p2 = realtimeTransport.startRealtimeSession('s2', 'hi');
    ...
    it('does not alert when a start is already in-flight (even with a different session id)', async () => {
      ...
      void realtimeTransport.startRealtimeSession('s1', 'hi');
    ...
    it('stop does not hang when a start attempt is stuck in token minting', async () => {
      ...
      const stopPromise = realtimeTransport.stopRealtimeSession();
      const race = Promise.race([

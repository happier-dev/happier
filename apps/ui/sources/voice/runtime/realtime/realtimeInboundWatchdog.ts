 ...
 *
 * Mirrors the FluidVoice route/device watchdog (plan snippet #15c): while a turn
 * is active, a timer expects a steady stream of inbound audio/events; each
 * inbound event resets the timer. If no inbound event arrives within
 ...
 * event or a new active turn, so a single stall can't spam reconnects.
 *
 * Provider-agnostic: it knows nothing about ElevenLabs; the transport feeds it
 * inbound events and turn-active transitions.
 */

...
export const VOICE_RUNTIME_CONFIG_DEFAULTS = {
    ...
    /**
     * Realtime ElevenLabs hardening knobs — the single source of truth for the
     * transient-drop reconnect backoff and the inbound-audio/event stall
     * watchdog. Modules under `runtime/realtime/**` derive their typed timers
     * from these so call sites carry no magic numbers and the values cannot
     * drift. ElevenLabs-specific behavior stays inside the realtime provider

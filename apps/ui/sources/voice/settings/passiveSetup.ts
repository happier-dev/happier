...
export function projectVoiceProviderPassiveSetupFacts(
  ...
  if (data?.available === false) {
    ...
    runtime = passiveSetup.supportedRuntimeVersions.includes(data.version)
      // Exact version compatibility is only a passive bound. The app-server
      // Start path remains the authoritative feature-availability check.
      ? 'unknown'
      : 'incompatible';
  }
...
export function projectVoiceProviderConnectedServicesCredentialFact(
  ...
  for (const serviceId of bindingDeclaration.serviceIds) {
    const resolution = resolveConnectedServiceSessionSelection({
      serviceId,
      binding: parsedBindings.data.bindingsByServiceId[serviceId],
      availability: {
        kind: 'known',

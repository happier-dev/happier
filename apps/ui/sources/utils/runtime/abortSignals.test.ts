...
import { mergeAbortSignals, throwIfAborted } from './abortSignals';
...
function expectReason(signal: AbortSignal, reason: unknown): void {
    if ('reason' in signal) {
...
describe('mergeAbortSignals', () => {
    ...
    it('uses the first abort reason and detaches every source listener without AbortSignal.any', () => {
        ...
        try {
            ...
            expectReason(merged.signal!, firstReason);
...
describe('throwIfAborted', () => {
    it('does not rely on AbortSignal.prototype.throwIfAborted and preserves its reason', () => {
        const controller = new AbortController();
        const reason = new Error('operation-aborted');
        const throwIfAbortedDescriptor = Object.getOwnPropertyDescriptor(
        ...
        try {
            ...
            if (throwIfAbortedDescriptor) {
                Object.defineProperty(AbortSignal.prototype, 'throwIfAborted', throwIfAbortedDescriptor);
            } else {
    ...
    it('uses an AbortError-compatible fallback when the aborted signal has no reason', () => {
        const signal = { aborted: true } as unknown as AbortSignal;

        expect(() => throwIfAborted(signal)).toThrow(expect.objectContaining({ name: 'AbortError' }));
    });
});

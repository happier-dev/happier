...
use objc2_web_kit::{
  WKAudiovisualMediaTypes, WKInactiveSchedulingPolicy, WKURLSchemeHandler, WKPreferences, WKUserContentController,
...
impl InnerWebView {
  ...
  fn new_ns_view(
    ...
    unsafe {
      ...
      // HAPPIER FORK: lift the WKWebView page-rendering cap.
      //
      // WKWebView clamps its whole page rendering update to ~60fps via the
      // `PreferPageRenderingUpdatesNear60FPSEnabled` WebKit feature, which defaults to true on
      // every macOS version. On a ProMotion display that halves the achievable frame rate:
      // measured 60.5fps capped vs 120.5fps uncapped on the same workload, with Chromium at
      // 120.6fps for reference. The flag is only honoured when set on the configuration BEFORE
      // the WKWebView is constructed — mutating a live webview's preferences does nothing
      // (measured: unchanged at 60.5fps) — so it has to happen here rather than in the host.
      //
      // This reaches WebKit's private `_features` / `_setEnabled:forFeature:` surface. Happier
      // already sets `macOSPrivateApi` and ships outside the App Store, so it qualifies; a build
      // that must not use private API should drop this hunk.
      #[cfg(target_os = "macos")]
      {
        use objc2::{msg_send, runtime::AnyObject};

        let features: *mut AnyObject = msg_send![WKPreferences::class(), _features];
        if !features.is_null() {
          ...
          for index in 0..count {
            ...
            if (*key).to_string() == "PreferPageRenderingUpdatesNear60FPSEnabled" {
              let _: () = msg_send![&*_preference, _setEnabled: false, forFeature: feature];
              break;
            }
          }
        }
      ...
      // HAPPIER FORK: captured before `ipc_handler_delegate` is moved into the webview, so the
      // `window.ipc` bridge below can be gated on whether an IPC handler actually exists.
      let should_initialize_ipc = ipc_handler_delegate.is_some();
      // Download handler
      ...
      // Initialize scripts
      // HAPPIER FORK: only install the `window.ipc` bridge when an IPC handler exists, so a
      // handler-less webview (used to host untrusted external pages) gets no privileged channel
      // at all. Asserted by src/browser/tests.rs.
      if should_initialize_ipc {
        w.init(
r#"Object.defineProperty(window, 'ipc', {
  ...
  #[cfg(target_os = "macos")]
  pub fn go_back(&self) -> crate::Result<bool> {
...
pub fn url_from_webview(webview: &WKWebView) -> Result<String> {
  let url_obj = unsafe { webview.URL().ok_or(Error::WebViewUrlUnavailable)? };

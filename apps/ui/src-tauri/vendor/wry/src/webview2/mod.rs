  ...
  fn create_controller(
    ...
    unsafe {
      if let Ok(env10) = env.cast::<ICoreWebView2Environment10>() {
        ...
        // HAPPIER FORK: private mode is a storage boundary, not a hint. Older WebView2 runtimes
        // lack Environment10; rather than silently constructing a persistent controller, fail
        // construction when private mode was explicitly requested.
        if incognito {
          return Err(windows::core::Error::from(E_NOINTERFACE).into());
        }

...
For the concrete QR/button flows, see [Features → Device linking & restore](/features/device-linking-and-restore).
...
For the user-facing sharing model, see [Session sharing](/features/session-sharing).
...
If you operate the server yourself, see [Server → Encryption & plaintext storage](../server/encryption).

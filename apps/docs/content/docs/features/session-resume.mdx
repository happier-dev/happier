...
- **Resume session**: starting a new Happier session by resuming a session the agent already has (see [Resume an agent's own session](/features/resume-provider-sessions))

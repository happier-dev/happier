...
- `HAPPIER_BUG_REPORTS_ENABLED` (default `1`)
- `HAPPIER_BUG_REPORTS_PROVIDER_URL` (default `https://reports.happier.dev`)
  - If set to an invalid URL, provider submission is disabled instead of falling back to a central default.
- `HAPPIER_BUG_REPORTS_DEFAULT_INCLUDE_DIAGNOSTICS` (default `1`)
- `HAPPIER_BUG_REPORTS_MAX_ARTIFACT_BYTES` (default `10485760`)
- `HAPPIER_BUG_REPORTS_UPLOAD_TIMEOUT_MS` (default `120000`)
- `HAPPIER_BUG_REPORTS_CONTEXT_WINDOW_MS` (default `1800000`; min `1000`, max `86400000`)
- `HAPPIER_BUG_REPORTS_ACCEPTED_ARTIFACT_KINDS` (comma-separated allowlist)
...
To hard-disable server log exfiltration, keep `HAPPIER_BUG_REPORTS_SERVER_DIAGNOSTICS_ENABLED=0` and/or remove `server` from `HAPPIER_BUG_REPORTS_ACCEPTED_ARTIFACT_KINDS`.

...
If you want to change the Agent without branching at all, see [Continue with another Agent](/features/continue-with-another-agent) — the same session keeps its id and transcript while its Agent is replaced. In the current development source, the fork flow also offers **Configure a new session**, which opens the normal new-session screen carrying this conversation as removable source context so you can change Agent, model, machine, and folder before the child starts.

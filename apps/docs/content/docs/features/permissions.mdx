...
For the full user-facing guide, see [Session modes](/features/session-modes).
...
For the user-facing workflow, see [Resume inactive sessions](/features/session-resume).
...
For the full guide to engine picking, dynamic/static model lists, custom model ids, model-specific options, and favorite models, see [Model and engine selection](/features/model-and-engine-selection).
...
For the broader model, see [Happier tools & actions](/features/happier-tools) and [Inbox and approvals](/features/inbox-and-approvals).

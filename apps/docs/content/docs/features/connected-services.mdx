...
See [Agent authentication](/docs/features/provider-authentication) and [Provider integrations](/docs/advanced/provider-integrations).
...
Token-based services, including GitHub token profiles when enabled, are stored as connected-service credentials and decrypted only where the client/daemon needs to use them.

...
See [ACP backends](/features/acp-backends).
...
If the provider/backend supports Happier [Connected services](/features/connected-services), that is often the easiest way to make the target machine ready for handoff without depending only on a native CLI login on that machine.

...
Happier supports using multiple servers at once (for example official + personal self-host + company self-host).
...
- `<WEBAPP_URL>/server?url=<ENCODED_SERVER_URL>&auto=1`

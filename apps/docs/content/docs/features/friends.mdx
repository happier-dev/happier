...
- `FRIENDS_ENABLED` (`true`/`false`, default `true`): master switch for Friends endpoints + UI.
- `FRIENDS_ALLOW_USERNAME` (`true`/`false`, default `false`):
...
### Username policy (when `FRIENDS_ALLOW_USERNAME=true`)
...
- `POST /v1/account/username`: claim/update a username (requires auth; available when Friends is enabled and either `FRIENDS_ALLOW_USERNAME=true` or GitHub is connected).
  - Body: `{ "username": "alice" }`
  - Success: `200 { "username": "alice" }`
  - Errors:
    - `400 { "error": "friends-disabled" }` when Friends is disabled
    - `400 { "error": "username-disabled" }` when `FRIENDS_ALLOW_USERNAME=false` and the user has no GitHub identity

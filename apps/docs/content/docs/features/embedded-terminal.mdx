...
description: Use the connected machine terminal directly inside Happier, including the bottom panel, sidebar, details panel, and mobile behavior.
---
...
3. Happier opens the terminal in your selected dock location.
...
If the terminal tab is not available, Happier hides it instead of showing a broken empty tab. In that case, use the session header terminal action or change the embedded-terminal location setting.
...
Provider login flows are documented in [Provider authentication](/features/provider-authentication).
...
For daemon setup and lifecycle commands, see [Daemon](/clients/daemon).

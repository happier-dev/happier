...
Stopped sessions are not shown in this picker. Use [`happier resume`](/features/session-resume) for inactive sessions.
...
If you send a message from the app while a Claude or Codex session is in local mode, Happier queues it first instead of typing it into your terminal. If the local turn is still running, Happier waits for that turn to finish, abort, or otherwise reach a safe boundary before switching the session back to remote processing and delivering the queued message.
...
Remote-to-local control starts from the terminal with `happier attach`. Local-to-remote control can be requested from the app when the session is currently locally controlled. Happier may wait until the current local turn is done before it takes remote control, because forcing a prompt into a busy terminal TUI could interrupt or abort the provider’s current work.
...
For Claude and Codex local control, the pending queue also protects the current terminal turn. Happier does not send a queued app message into the provider TUI while that TUI is busy. It waits for the local turn to complete or stop, then resumes remote control and processes the queued message from the same session.

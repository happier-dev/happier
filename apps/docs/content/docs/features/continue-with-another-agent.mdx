...
- [Session handoff](/features/session-handoff), which moves a session to a **different machine** and keeps the same Agent
- [Session forking & replay](/features/session-forking), which creates a **new** session branched from this one
- [Model and engine selection](/features/model-and-engine-selection), which chooses the engine and model for a **new** session
...
- **The session's transcript is not hosted by Happier.** A [direct session](/features/direct-sessions), where the Agent owns its own store, has no Happier conversation for another Agent to continue.
...
The switch never moves a session to another machine. If you want that, use [session handoff](/features/session-handoff).
...
- [Resume provider sessions](/features/resume-provider-sessions)

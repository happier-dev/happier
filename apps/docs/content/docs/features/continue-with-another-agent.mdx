...
description: Switch the coding Agent running a live session in place, keep the same session, and see what context was handed over.
---
...
- [Session handoff](/docs/features/session-handoff), which moves a session to a **different machine** and keeps the same Agent
- [Session forking & replay](/docs/features/session-forking), which creates a **new** session branched from this one
- [Model and engine selection](/docs/features/model-and-engine-selection), which chooses the engine and model for a **new** session
...
The picker lists the Agents available on the machine hosting this session. The Agent currently running the session is marked as such and stays selectable, because that row is also where you edit its model and configuration.
...
The switch works only for a **hosted** session, on the machine that already hosts it, whose current Agent Happier can name, and only while that Agent is **strictly idle**.

It is not available when:

- **The session's transcript is not hosted by Happier.** A [direct session](/docs/features/direct-sessions), where the Agent owns its own store, has no Happier conversation for another Agent to continue.
...
- **The machine hosting the session is offline, or its CLI is older than the feature.** Happier distinguishes the two where it can, and says "update or reconnect" when it genuinely cannot tell.
...
The switch never moves a session to another machine. If you want that, use [session handoff](/docs/features/session-handoff).
...
- It only happens when that Agent's own provider-session resume is supported and allowed for you. An Agent you have disabled in settings, a provider that does not support resume at all, and — for Codex — a backend mode that does not offer it, all fall back to a fresh start with the **full** carried-over context instead. See [Resume a provider session](/docs/features/resume-provider-sessions).
- The Agent's own vendor session can be gone by the time you return: providers prune, rotate and expire their session stores. Happier does not pre-check it. A dead session **fails at the first turn**, visibly, exactly like any other resume whose provider state has disappeared. This is expected. Switch to another Agent and back, or continue with a different Agent, to get a fresh start with the full context.
...
- the session title
- an explicit note that the previous Agent's own conversation state does **not** carry over, so what follows is a replay
...
Read this card knowing one thing about it: **nothing was stored to show you.** The brief is discarded as soon as the arriving Agent accepts it. The card rebuilds it on open, on the machine hosting the session, by running the same context pass over the same point in the transcript that the switch recorded.
...
- **Nothing was sent, nothing changed.** Wrong permissions, an unsupported session, a target that is not available on that machine, a session that changed while you were choosing, an Agent that was still working, or an Agent that could not be stopped. Fix the cause and send again — your draft is kept.
- **The previous Agent stopped, but the switch did not complete.** Your message was not sent. The session is recoverable; check it before sending again.
- **The session switched, but your message was not sent.** Send it again.
- **Happier could not confirm what happened.** Check the session before sending again. While this is unresolved, the composer refuses new sends rather than risk sending your message twice.
...
- [Resume provider sessions](./resume-provider-sessions)

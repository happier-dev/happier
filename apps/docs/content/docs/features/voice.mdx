...
- **OpenAI Realtime**: WebRTC voice using either a saved Voice API key or an explicitly selected OpenAI Connected Service.
- **Codex Live (experimental)**: WebRTC voice attached to an exact Codex session, with coding work, tools, and permissions remaining under that session's canonical owners.
...
- choose **OpenAI Realtime** for OpenAI WebRTC voice with an explicit saved-secret or Connected Service authentication source
- choose **Codex Live** when you want an experimental realtime conversation attached to an existing or global hidden Codex session
...
Use **OpenAI Realtime**, then explicitly choose either the standalone saved-secret source or a Connected Service source.

### I want realtime voice attached to Codex

Use **Codex Live (experimental)** and choose the exact session or global Voice account and machine you want to use.

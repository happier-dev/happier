...
This page explains how to choose between them, how provider-session browsing works, and when to use **Take over** vs **Take over + import**.
...
## Browse sessions to resume

Happier also uses that same browsing to help you fill **Resume session** when you start a new session.

This is a separate workflow from “resume inactive sessions”:

- **Resume inactive sessions** resumes an existing Happier session. See [Resume inactive sessions](/features/session-resume).
- **Resume session** starts a new Happier session by resuming a session the agent already has. See [Resume an agent's own session](/features/resume-provider-sessions).

When the selected backend supports browsing sessions, the input offers **Browse sessions**. The picker is locked to the machine, agent and auth context you selected, so you cannot accidentally pick a session from a different machine or a different agent home.
...
Choose **Take over + import** when you want to convert the workflow into a Happier-managed session.
...
If you want Happier to own the session moving forward, use **Take over + import**.
...
Start it as **Synced**, or open it direct first and then use **Take over + import**.
...
For the full cross-machine workflow, see [Session handoff](/features/session-handoff).
...
They simply are not synced into Happier until you explicitly choose **Take over + import**.

## Related guides

- [Session handoff](/features/session-handoff)
- [Resume inactive sessions](/features/session-resume)
- [Resume an agent's own session](/features/resume-provider-sessions)

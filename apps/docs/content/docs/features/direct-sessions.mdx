...
- **Resume ID** starts a new Happier session by resuming an existing provider session id. See [Resume a provider session (Resume ID)](/features/resume-provider-sessions).

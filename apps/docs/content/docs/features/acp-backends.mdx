...
Configured ACP backend models can also be favorited from the new-session engine picker. Favorites are saved per configured backend, so two ACP backends using the same CLI can keep separate favorite model lists.

For the full user-facing guide, see [Model and engine selection](/features/model-and-engine-selection).

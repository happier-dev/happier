...
This page describes Happier-managed agent work. Some providers also expose their own native subagent or delegation tools to the session agent. When you explicitly want provider-native subagents, use those provider-native tools when they are available. Happier review, plan, and delegate actions create product-managed execution runs that Happier can list, route, approve, and reopen consistently.
...
Backend selection chooses the provider/backend that should run the Happier-managed execution run. Selecting multiple different backend targets intentionally starts one run for each selected provider/backend. It is not a capacity pool or a requirement for parallelism; for homogeneous parallel work, start multiple runs with the same backend or use provider-native subagent tooling when that is what you want.
...
- [Attachments & image previews](/features/attachments)

...
Use **Take over + import** when you want:
...
When a user chooses **Take over + import**:
...
In the **Resume session** flow, the picker is intentionally locked to the machine, agent and auth context you already selected, so the session you pick is one that will actually resume.

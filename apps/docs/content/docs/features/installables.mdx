---
title: Installables
description: Let Happier manage machine-local tools like Codex ACP and GitHub CLI, with install and update policies per machine.
---
...
3. If neither exists, Happier can show installation/remediation guidance.
...
That keeps GitHub CLI installation explicit. Happier does not silently install a new GitHub CLI binary just because you opened a source-control screen.

GitLab note: GitLab merge-request workflows can use an authenticated `glab` CLI when it is already available, but `glab` is not currently a Happier-managed installable.

---
title: Installables
description: Let Happier manage machine-local tools like Codex ACP and optional source-control CLIs, with install and update policies per machine.
---
...
- **GitHub CLI (`gh`)**, in builds that expose it — optional helper for GitHub source-control workflows when you want to use local GitHub CLI authentication instead of a connected GitHub token
...
For optional helper tools such as `gh`, Happier can use a more conservative policy such as "do not auto-install" and "notify about updates."
...
GitHub CLI is a generic installable dependency when it is exposed by the build, not an agent backend.

Happier can use `gh` for GitHub source-control workflows when:

- you have not connected a GitHub token in Happier, or
...
1. Happier prefers a working system `gh` if it is already installed.
2. If a Happier-managed `gh` exists for the machine, Happier can use it.
3. If neither exists, Happier can show installation/remediation guidance when that integration is available.
...
For cross-machine workflows, a connected GitHub token is usually more portable because it travels through Happier's connected-service model. The `gh` path is best when the machine is already set up and authenticated.

GitLab note: GitLab merge-request workflows can use an authenticated `glab` CLI when it is already available, but `glab` is not currently documented as a Happier-managed installable.

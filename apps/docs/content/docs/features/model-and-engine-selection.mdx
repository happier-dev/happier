...
Inside a running session, that same picker can also change the Agent itself — see [Continue with another Agent](/features/continue-with-another-agent).
...
Use [Profiles](/features/profiles) for reusable environment, secret, endpoint, and backend-default setup.

...
If you use hstack, you can install Codex with its legacy-named `providers` installer:
...
- [Browse and import Agent sessions](/features/browse-and-import-sessions)
- [Resume an Agent session (Resume ID)](/features/resume-provider-sessions)
...
For the end-to-end workflow, see [Features → Attach to a running session](/features/attach-to-session).
...
Happier uses an **Agent-agnostic permission mode** (`default`, `read-only`, `safe-yolo`, `yolo`) and applies it to Codex in the best available way.
...
- **Agent-native sandbox/policy** (what the Codex runtime enforces internally).

...
Happier supports **Kiro** as a local CLI-backed Agent.

Kiro is a built-in Agent in its own right. It is **not** the same thing as **Custom ACP**.

- **Kiro**: Happier's dedicated built-in Kiro integration
- **Custom ACP**: the generic Agent family for arbitrary ACP-compatible CLIs and user-defined backends

That means custom ACP backends do **not** run through the Kiro Agent path.
...
Kiro also appears in Happier's **ACP Backends** settings screen because it is a built-in ACP-capable Agent.
...
- **Kiro Agent page** when you want Kiro itself
...
2. Restart Happier.
3. Re-open the Agent settings page and use **Check now**.

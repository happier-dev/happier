...
**Custom ACP** is Happier's generic Agent family for any CLI that speaks the Agent Client Protocol (ACP).

Use it when Happier does not ship a dedicated built-in Agent for the tool you want to run, or when you want to add your own backend choices on top of an ACP-compatible CLI.
...
Custom ACP models can also be saved as favorites from the new-session engine picker. Favorites are stored per configured backend, not just per Agent family.

## Authentication

Custom ACP does not imply one single Agent-level auth system.
...
See [Agent authentication](/docs/features/provider-authentication) for the general model.

## Custom ACP vs built-in Agents

Use a built-in Agent when Happier already ships a dedicated integration for that tool.
...
- you want a generic ACP integration rather than a bespoke Agent page

---
title: Claude
description: Configure Claude in Happier, including the unified terminal runtime, Agent SDK remote mode, partial streaming, and /rewind.
---
...
Remote mode currently has **three implementations**:

| Runtime | Default | What it is |
| --- | --- | --- |
| **Unified terminal** | Off | Happier hosts **Claude Code’s own TUI** in a terminal session and types prompts into its composer, so the app and the terminal share one Claude Code run. See [Claude unified terminal](/features/claude-unified-terminal). |
| **Agent SDK remote** | On | Uses the official `@anthropic-ai/claude-agent-sdk` to run Claude, publish capabilities, and (optionally) enable file checkpointing. |
...
Selection order is: unified terminal when it is enabled, otherwise Agent SDK, otherwise legacy. Happier makes that choice when the provider session starts and keeps it for the life of that session, so changing the setting affects the next Claude session you start rather than the one already running.

Note that the unified terminal runtime is a **remote** mode, not local mode: Happier drives it from the app. What is different is that there is a real Claude Code TUI to attach to, and you can type in it. Happier reads the composer before every write so an app message never lands on top of what you are typing.
...
- an **Agent-agnostic permission mode** (`default`, `read-only`, `safe-yolo`, `yolo`)
...
If you use hstack, you can run its legacy-named `providers` installer (uses the upstream native installer by default):
...
Claude also supports **machine-local Agent authentication** directly in Agent settings.
...
For the shared user guide, see [Agent authentication](/features/provider-authentication).
...
- direct-session handoff support is Agent-dependent, and Claude is one of the supported Agents
...
| --- | --- | --- |
| **Use unified terminal runtime** | Off | Runs Claude Code’s own TUI in a terminal session and drives it from the app. Takes precedence over the Agent SDK runner. See [Claude unified terminal](/features/claude-unified-terminal). |
| **Terminal host** | Auto | Unified terminal only. **Auto** uses `tmux` when available, otherwise Happier’s bundled zellij. You can also force **tmux** or **zellij**. |
| **Large-session resume** | Ask every time | Unified terminal only. How Happier answers Claude’s question about resuming a large session: ask, resume from summary, or resume the full session. |
| **Workspace trust** | Ask every time | Unified terminal only. How Happier answers Claude’s workspace trust question: ask, always trust Happier workspaces, or always reject them. |
| **Use Agent SDK fallback** | On | Enables the official Agent SDK remote runner (`@anthropic-ai/claude-agent-sdk`) when the unified terminal runtime is off. When this is off too, Happier uses the legacy remote runner. |
...
Routed messages are stored as an Agent-agnostic structured message kind (`participant_message.v1`) so Happier can render them as a distinct card in the transcript (instead of looking like a normal bubble).
...
You can do that either in your terminal or directly from the Claude Agent settings page in Happier.

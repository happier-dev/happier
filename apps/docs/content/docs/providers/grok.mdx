...
- Grok's structured questions support multiple questions and multiple selected options without comma-delimited encoding.
...
- **Models:** Happier reports the active `grok-build` model. Model switching remains disabled until the CLI advertises and proves a distinct ACP selection contract.
- **MCP tools:** native MCP delivery is experimental.
- **Images and session media:** not currently advertised as supported through this integration.

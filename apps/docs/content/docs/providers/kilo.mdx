...
Happier supports **Kilo** as a local CLI-backed Agent.
...
If you use hstack, you can install Kilo with its legacy-named `providers` installer:
...
Kilo is behind Happier’s experimental Agent rollout.
...
- When in doubt, treat Kilo’s own CLI configuration as the source of truth for Agent-native behavior.
...
Use the Kilo Agent settings page in Happier or run Kilo’s normal local login flow in your terminal, then use **Check now**.

...
### 1c) Plugin Platform contract tests
...
This runs every `packages/tests/src/plugin-platform/*.test.ts` contract test. The file set is derived
from the directory, so a new contract test beside the existing ones is executed without editing a
script list. CI runs this lane alongside the shared package unit tests.

...
Use the canonical [controlled-stack workflow](/hstack/stacks#controlled-stacks-for-agent-qa).
The setup flow intentionally starts without copied auth, but it still reuses the repository build
authority and Expo owner:

1. Create one fresh isolated consumer from the checkout under test:
   - `node ./apps/stack/scripts/repo_local.mjs stack new codex-bootstrap-qa-<port> --repo=/Users/leeroy/Documents/Development/happier/dev --port=<uniquePort> --server=happier-server-light --db-provider=sqlite --no-copy-auth --non-interactive`
2. Require snapshots and borrow the repository stack's Expo endpoint:
   - `node ./apps/stack/scripts/repo_local.mjs stack env codex-bootstrap-qa-<port> set HAPPIER_STACK_RUNTIME_MODE=require`
   - `node ./apps/stack/scripts/repo_local.mjs stack env codex-bootstrap-qa-<port> set HAPPIER_STACK_EXPO_SOURCE_STACK=<repository-stack>`
3. Select the producer's existing snapshot without building when it already contains the bytes under test:
   - `node ./apps/stack/scripts/repo_local.mjs stack runtime codex-bootstrap-qa-<port> select --json`
4. Start the controlled stack:
   - `node ./apps/stack/scripts/repo_local.mjs tui stack start codex-bootstrap-qa-<port> --runtime --mobile`
5. Exercise the intended bootstrap/auth flow against that isolated stack.

If current server or daemon bytes are required, request them through this same consumer. The
repository authority reuses an in-flight/completed publication and stores only one artifact set:

- `node ./apps/stack/scripts/repo_local.mjs stack build codex-bootstrap-qa-<port> --server --daemon --json`
- `node ./apps/stack/scripts/repo_local.mjs stack runtime codex-bootstrap-qa-<port> activate --all --json`

Do not launch a second publisher or a consumer-owned Expo process. For UI-only controlled-live QA,
reload the borrowed Expo page rather than building a web artifact.
...
- The dev config enables the MCP capability:
...
- `[partial]` Desktop `/setup` + post-auth continuation + relay drift repair + provider setup reuse
  ...
  - Agent setup (canonical flow): `apps/ui/sources/components/settings/agents/setup/AgentSetupFlow.tsx`
...
- CLI relay set: `.project/logs/bootstrap-qa/codex-bootstrap-webcli-24502/13-cli-relay-set.json`
...
- [partial] the old `cargo metadata ... (os error 2)` blocker is retired; current native desktop proof comes from the Compose-backed `yarn --cwd apps/ui tauri:qa` run on `com.happier.stack.compose-desktop-qa`.
  - Proof artifacts: `.project/logs/bootstrap-qa/tauri-onboarding-wizard-20260419-2026-04-19_00-44-18/00-driver-session-status.json`, `.project/logs/bootstrap-qa/tauri-onboarding-wizard-20260419-2026-04-19_00-44-18/00-backend-state.json`, `.project/logs/bootstrap-qa/tauri-onboarding-wizard-20260419-2026-04-19_00-44-18/08-setup.png`, `.project/logs/bootstrap-qa/tauri-onboarding-wizard-20260419-2026-04-19_00-44-18/08-setup.a11y.yml`, `.project/logs/bootstrap-qa/tauri-onboarding-wizard-20260419-2026-04-19_00-44-18/08-setup.structure.yml`
...
- 2026-08-14: A custom native dev client could fail in Metro when a bundled Plugin UI rebuild changed a content-hashed native chunk but the app's generated bundled-plugin registry still referenced the prior filename. The canonical UI workspace preflight now publishes the bundled-plugin projection after the complete UI workspace closure, and Metro watches every bundled-plugin package through a stable package-root topology even when an artifact tree is absent at startup. Stack adopts a structurally complete last-green closure immediately and refreshes it in the background; without one, a failed preparation leaves Expo running in an explicitly degraded state so the rest of the Stack and its repair loop remain available. Validated via `apps/ui/scripts/ensureWorkspacePackagesBuilt.test.mjs`, `apps/ui/sources/__tests__/config/metro.reactNativeWebShim.test.ts`, `apps/stack/scripts/utils/dev/expo_dev.ensure_workspace_built.test.mjs`, and `apps/stack/scripts/utils/dev_targets/supervisor.test.mjs`.
- 2026-04-19: Compose-backed native desktop QA completed successfully on `com.happier.stack.compose-desktop-qa`; the earlier Lane B1 blocker notes are retired in favor of the proof artifacts under `.project/logs/bootstrap-qa/tauri-onboarding-wizard-20260419-2026-04-19_00-44-18/`.

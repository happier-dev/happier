...
Deep reference (host setup and security): [MCP host setup](/extending/mcp-hosts).
...
Optional overrides (when supported by the Agent):
...
Call `execution_run_start` to start a structured Happier-managed run in a session. It requires an `intent` plus an Agent target in the retained `backendTarget` field.
...
The target selects which Agent runs the execution run. It is not a capacity
slot. If you choose multiple targets through an Action that supports that
shape, Happier starts one managed run per selected Agent.
...
- you want to create, inspect, or list approval artifacts programmatically
...
This is also the path to approval-queue actions such as:
...
### Approval decisions require a present user

External MCP hosts can create and inspect approval requests, but they cannot approve or reject them. `approval.request.decide` remains discoverable in the shared action catalog and truthfully returns `present_user_required` to automation callers.

Review and decide the request in the app's **Inbox**. This boundary prevents the same automation credential from requesting an approval and granting it to itself.
...
Starting and managing local Agent processes (creating sessions, sending messages, attaching, etc.) generally requires a running Happier daemon on that machine.

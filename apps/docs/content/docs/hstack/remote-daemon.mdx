---
title: Remote background service setup (SSH)
description: Install Happier on a remote host, pair credentials, and optionally install the background service.
---
...
- optionally installs + starts the background service in **user** mode
...
- installs/starts the background service in **user** mode
...
## Skip background-service install

If you only want the CLI installed + authenticated (no automatic startup/background service):

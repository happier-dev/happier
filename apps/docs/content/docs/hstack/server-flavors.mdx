...
- Docker-managed infra per stack (Postgres + Minio/S3; optional Redis for multi-replica realtime)
...
- Public files backend (choose one):
  - S3-compatible object storage (`S3_*`) with `HAPPIER_FILES_BACKEND=s3` (default)
  - Local disk with `HAPPIER_FILES_BACKEND=local`
- Optional: Redis fanout for multi-replica realtime (`REDIS_URL` + `HAPPIER_SOCKET_ADAPTER=redis-streams`)

hstack can **manage this for you automatically per stack** using Docker Compose (Postgres + Minio — Redis when needed),
...
If disabled, you must provide `DATABASE_URL` and configure a public files backend (`S3_*` with `HAPPIER_FILES_BACKEND=s3`, or `HAPPIER_FILES_BACKEND=local`). `REDIS_URL` is only required for multi-replica realtime (`HAPPIER_SOCKET_ADAPTER=redis-streams`).

...
- [CLI](/apps/cli) for install/auth/server selection
- [CLI sessions](/apps/cli-sessions) for create/list/send/resume/attach/manage
- [Happier MCP (external)](/extending/happier-as-mcp-server) for editor/agent automation
- [Happier MCP (external)](/extending/happier-as-mcp-server) for privileged host configuration

If you’re looking specifically for background operation and service installation, see [Daemon](/apps/daemon).

For server selection, profile switching, and multi-server behavior, see [Features → Multi-server](/accounts/multi-server).

...
This is separate from **MCP servers you manage inside Happier** (Playwright, GitHub, etc). Those are documented under [MCP servers](/features/mcp-servers).
...
Deep reference (host setup and security): [MCP host setup](/clients/mcp-hosts).
...
If you’re targeting a self-hosted server, ensure the MCP host launches Happier with the right server profile (or server URL flags). See [CLI → Server profiles](/clients/cli#server-profiles).

...
`happier setup` is the guided setup executor from the terminal. It configures this computer using the same setup orchestration the app uses for Relay selection, sign-in if needed, background-service install/start, and optional provider setup.
...
Background service reference: [Background service](./daemon).
...
For automatic startup, remote control, and headless servers, see [Background service](./daemon).
...
For the full workflow, see [Features → Attach to a running session](/features/attach-to-session).
...
For the full user guide and the meaning of “resumable”, see [Features → Resume inactive sessions](/features/session-resume).
...
For the full guide, see [Features → Profiles](/features/profiles).
...
For the broader product model, see [Happier tools & actions](/features/happier-tools).

## Multi-server reference

For full multi-Relay and background-service guidance, see [Features → Multi-server](/features/multi-server).

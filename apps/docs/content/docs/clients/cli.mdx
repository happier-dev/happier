...
For always-on background sessions, remote control, and headless servers, see [Daemon](/clients/daemon).
...
Deep reference: [CLI sessions](/clients/cli-sessions).
...
Guide: [Happier MCP (external)](/clients/mcp)
...
For full multi-server and multi-daemon guidance, see [Features → Multi-server](/features/multi-server).

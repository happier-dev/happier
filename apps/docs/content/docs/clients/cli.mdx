...
`happier setup` is the guided setup executor from the terminal. It runs the same setup orchestration the app uses for Relay selection, sign-in if needed, daemon service install, and optional provider setup.
...
- `--skip-daemon` (don’t install/enable the background service)
...
If you want to set up a remote machine to run the daemon/providers (VPS, home server, dev box):
...
- `--service-mode user|none` (daemon service mode)
...
Daemon background service reference: [Daemon](./daemon).

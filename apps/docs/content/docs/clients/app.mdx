...
For detailed Relay profile, switching, and multi-Relay machine guidance, see [Features → Multi-server](/docs/features/multi-server).
...
Full guide: [Session read and unread state](/docs/features/session-read-state).
...
Full guide: [Model and engine selection](/docs/features/model-and-engine-selection).
...
Review comments guide: [Review comments](/docs/features/review-comments).
...
Full guide: [Projects & worktrees](/docs/features/projects-and-worktrees).
...
If you enable **Enter to Send** in [Session settings](/docs/features/session-settings), Enter sends from the composer when the current platform input path has content it can send. On web, Shift+Enter inserts a new line. On iOS and iPadOS with a physical keyboard, Shift+Enter also continues to insert a new line while Enter sends.
...
If the composer is empty or the current send action is disabled, pressing Enter does not send anything. Use the visible send button for attachment-only drafts.
...
For the new-session-specific flow, see [Start a new session](/docs/features/start-a-session).
...
- if **Enter to Send** is enabled, typed prompts can be sent from the keyboard; use the visible send button for attachment-only drafts

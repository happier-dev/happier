...
For detailed Relay profile, switching, and multi-daemon guidance, see [Features → Multi-server](/features/multi-server).
...
See [Notifications](/advanced/notifications) for setup/self-hosting notes and [Notification routing](/features/notifications-routing) for the exact in-app behavior.

## Session read and unread state

The app shows unread state for sessions with committed transcript activity after your last viewed position.

You can manually change that state from:
...
Use **Mark as unread** when a session still holds activity you have not taken in. Use **Mark as read** to clear unread attention for a session.
...
If you mark the session unread while you are already inside it, Happier keeps it unread for that current view instead of immediately marking it read again. Closing and reopening the session is treated as a new viewing intent, so the session can become read again when it is visible and focused.
...
On phone-sized layouts, the same workflows are shown through the **session cockpit** by default. The bottom session tab bar lets you switch between the agent chat, Files, Git, transcript Navigation, open details tabs, and Terminal when a terminal tab is available.

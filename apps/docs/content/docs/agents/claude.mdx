...
| --- | --- | --- |
| **Unified terminal** | Off | Happier hosts **Claude Code’s own TUI** in a terminal session and types prompts into its composer, so the app and the terminal share one Claude Code run. See [Claude unified terminal](/sessions/claude-unified-terminal). |
...
For the shared user guide, see [Agent authentication](/agents/provider-authentication).
...
| --- | --- | --- |
| **Use unified terminal runtime** | Off | Runs Claude Code’s own TUI in a terminal session and drives it from the app. Takes precedence over the Agent SDK runner. See [Claude unified terminal](/sessions/claude-unified-terminal). |

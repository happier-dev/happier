---
title: Profiles
description: Use Agent profiles to reuse environment variables, secret bindings, and session-launch defaults across the app and CLI.
---

Agent profiles let you save a **named session-launch configuration** and reuse it whenever you start a session.

Use profiles when you want to:

- configure an Agent to use a different model-provider endpoint or auth scheme
...
- target either built-in Agents or configured **Custom ACP** Agents with the same profile model
...
- **compatibility** with one or more Agent choices, including configured ACP Agents
- **default per-Agent startup preferences**, such as permission and transcript-storage defaults where supported
...
4. Choose which Agents it should be compatible with
...
2. Choose the Agent you want to run
3. Pick a profile
...
Configured ACP Agents participate in this flow the same way built-in Agents do.
If a profile is intended for one specific Agent, Happier preserves that
...
### Example: profile for a Custom ACP Agent

If you have a custom ACP Agent such as **Review Bot**, you can create a profile
that is compatible only with that Agent.

This is useful when you want:

- an Agent-specific endpoint or env setup
- an Agent-specific permission default
- an Agent-specific transcript-storage default
- a profile that should not appear for unrelated Agents
...
- [Custom ACP Agents](/agents/acp-backends)

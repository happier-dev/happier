...
```
happier.example.com {
  reverse_proxy http://<container-ip>:53288
}
```
...
server {
  ...
  location / {
    proxy_pass http://<container-ip>:53288;
...
If you use a UI-driven reverse proxy (e.g. Nginx Proxy Manager / Traefik), make sure “WebSocket support” is enabled and the upstream is `http://<container-ip>:53288`.
...
- **Reverse proxy**: terminate TLS on your network edge and proxy to `http://<container-ip>:53288` (ensure WebSockets are supported).

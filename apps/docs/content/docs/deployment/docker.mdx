...
- Server env var reference: [Environment variables](/deployment/env)
...
The `relay-server` image is built from the same server code and can run full flavor too — you just override the defaults. Postgres is the external database supported by the prebuilt image.
...
For S3/Minio instead of local files, set `HAPPIER_FILES_BACKEND=s3` and the `S3_*` env vars (see [Environment variables](/deployment/env)).
...
- If your environment can’t use `/dev/net/tun` (some restricted hosts), fall back to a normal reverse proxy (Caddy/Traefik/nginx) or run Tailscale on the host OS and proxy to the container.
  ...
  - See: [Deployment → Environment variables → Networking / reverse proxy](/deployment/env#websockets-required-for-realtime--machine-online)
...
- Light flavor: `sqlite` (default), `pglite`
...
SQLite is the light flavor's default and the recommended light-mode database for Docker because it is single-process and easy to back up. The server image entrypoint automatically runs `prisma migrate deploy` for SQLite when `RUN_MIGRATIONS=1` (default).
...
- The DB provider is selected via `HAPPIER_DB_PROVIDER`:
  ...
  - Light: `sqlite` (default) or `pglite`

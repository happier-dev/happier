...
- **Docker** (recommended starting point): use the published image, or build from this repo’s `Dockerfile`, and run with Docker / Docker Compose.
  - See: [Docker](./docker)
- **Observability** (Prometheus + Grafana + Alertmanager): the repo ships a Docker-friendly observability pack under `apps/server/observability`.
...
- **Full**: external Postgres (default) or MySQL 8.0.16+ + S3/Minio-compatible storage (default) or local files (`HAPPIER_FILES_BACKEND=local`); optional Redis for multi-replica realtime.
...
If you want dashboards and alerts, use the repo-owned observability pack described in [Observability](./observability).
...
When Socket.IO polling is enabled, sticky sessions are required in front of multiple API replicas. Also make sure the reverse proxy idle timeout is greater than `pingInterval + pingTimeout` so long-lived realtime connections are not dropped prematurely.

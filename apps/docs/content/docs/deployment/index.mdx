...
- **Docker** (recommended starting point): build from this repo’s `Dockerfile` and run with Docker / Docker Compose.
  - See: [Docker](./docker)
- **Configuration reference**: see [Environment variables](./env) for common settings, including optional voice configuration.
...
- **Light**: embedded Postgres (PGlite, default) or SQLite stored on disk; local file storage served by the server at `GET /files/*`.

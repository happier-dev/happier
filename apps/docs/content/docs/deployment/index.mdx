...
- **Managed self-host runtime** (service + install-or-update).
  - See: [Self-host runtime (installer vs runner)](/deployment/self-host-runtime)
- **Run from a monorepo clone** (repo-local mode): use `yarn` from the repo root to run a stackless local stack, install a service, and configure Tailscale.
  - See: [Run from a monorepo clone (repo-local mode)](/deployment/repo-local)
...
- **Configuration reference**: see [Environment variables](/deployment/env) for the complete server runtime env surface.
...
See [Docker](/deployment/docker) for concrete examples.
...
See: [Environment variables → Networking / reverse proxy](/deployment/env#websockets-required-for-realtime--machine-online)

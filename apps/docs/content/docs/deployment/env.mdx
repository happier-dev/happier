...
and both environment variables — are on [API rate limits](/deployment/rate-limits),
...
This is an opt-out: in-place Agent switching is advertised unless it is explicitly disabled. For what the feature does, see [Continue with another Agent](/features/continue-with-another-agent).
...
[Server retention policies](/server/retention) for the full domain list and the
...
variable, is on [Feature flags](/features/feature-flags).

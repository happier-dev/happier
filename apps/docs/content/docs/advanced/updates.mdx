...
Inspect or change the default release channel followed by the `happier` shim:
...
If this computer has a default-following automatic startup entry, the release-channel switch also runs or prompts for the background-service follow-up so automatic startup uses the selected lane.
...
The installer installs or refreshes the CLI payload and shims for the selected lane. It does not always install or enable this computer's background service; use `happier setup` for first-time computer setup and `happier doctor repair` to fix drift or broken state later.
...
There are multiple supported self-host shapes. See also: [Self-host runtime (installer vs runner)](/deployment/self-host-runtime).

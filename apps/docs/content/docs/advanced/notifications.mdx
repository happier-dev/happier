...
Happier also has separate **in-app attention surfaces** such as the Inbox dot, the Sessions-tab dot, and unread/shared-session organization in the session list. Those are part of the normal in-app UI, not the push-notification system. See [Inbox and approvals](/docs/features/inbox-and-approvals) and [Session sharing](/docs/features/session-sharing).

For the full cross-surface model, including the desktop overlay, Live Activities, widgets, badges, quiet hours, privacy modes, and sounds, see [Attention surfaces](/docs/features/attention-surfaces).

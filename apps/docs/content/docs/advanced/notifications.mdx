...
This page is about **push notifications**.

Happier also has separate **in-app attention surfaces** such as the Inbox dot, the Sessions-tab dot, and unread/shared-session organization in the session list. Those are part of the normal in-app UI, not the push-notification system. See [Inbox and approvals](/features/inbox-and-approvals) and [Session sharing](/features/session-sharing).

For the full cross-surface model, including the desktop overlay, Live Activities, widgets, badges, quiet hours, privacy modes, and sounds, see [Attention surfaces](/features/attention-surfaces).
...
- pressing a notification action button can execute the direct action when the runtime can verify the target server/session and the surface policy allows direct actions
- when direct execution is not safe, Happier opens the targeted session instead
...
For the exact routing behavior, see [Notification routing](/features/notifications-routing).
...
- bundled **Soft** and **Urgent** notification sounds, native system sounds, or silent mode
- Live Activity remote update preferences when the selected server advertises support
...
See [Multi-server](/features/multi-server) and [Notification routing](/features/notifications-routing) for the full user-facing flow.

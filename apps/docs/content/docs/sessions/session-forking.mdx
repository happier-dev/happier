...
If you want to change the Agent without branching at all, see [Continue with another Agent](/sessions/continue-with-another-agent) — the same session keeps its id and transcript while its Agent is replaced. In the current development source, the fork flow also offers **Configure a new session**, which opens the normal new-session screen carrying this conversation as removable source context so you can change Agent, model, machine, and folder before the child starts.
...
3. **Happier replay fork** (always available when replay is enabled) builds a replay seed from the transcript and starts a fresh provider session. Replay is enabled by default, so this path is available unless you turn it off in Settings.
...
The machine running the session supplies the default when no setting is chosen. Override it on that machine with `HAPPIER_REPLAY_MAX_SEED_CHARS`, which defaults to `120000` and is clamped to the range `1024`–`200000`. The seed's own framing counts against that budget, so the number is a true total rather than a transcript-only allowance. The minimum is not arbitrary: below roughly 800 characters the prompt builder correctly produces **no** seed at all rather than a frame with no conversation under it, so `1024` is the smallest value that can still deliver a usable seed.

...
Git is not the only backend. Happier ships a Sapling plugin, and it is worth
knowing what it does and does not do before you point Happier at a Sapling
checkout — it is neither a full peer of Git nor read-only.

| Area | Sapling |
| --- | --- |
| Detection | Repository, identity, ignored paths, repo mode, executable — all supported |
...
| Remotes | Read only — no fetch, pull, push or publish |
| Hosting providers | None, so no pull-request surface |
| Line-level staging | Not supported — path-level selection only |

So you can read a Sapling repository, review what changed, commit selected
paths and back a commit out. Anything that talks to a remote, or anything
branch- or stash-shaped, is Git-only.

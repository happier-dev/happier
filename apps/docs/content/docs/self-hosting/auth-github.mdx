...
4. If your GitHub login is already taken (or doesn’t match the username rules), Happier will ask you to choose a username during signup. This is expected.
   - Important: this is **signup** (creating a new Happier account). If you’re trying to use GitHub to “log in” to an existing Happier account from another device, you must use **device-key restore** (for example **Add your phone** / **Restore account**) and then connect GitHub from **Settings → Account**.
...
## Keyless GitHub login contract (activation pending)

The target plaintext-account flow can use GitHub OAuth as a **keyless login method**.
Keyless means a bearer-token credential with zero Account E2EE material, not a
device-generated replacement key.

The complete token-only UI/daemon flow is currently activation-closed while
account-data migrations and composed checks finish. Keep keyless auto-provisioning
disabled in production until the [Server Auth](/self-hosting/auth) rollout warning is
removed. The variables below document the activation contract; their presence does
not prove the flow is active.
...
Notes:
- After activation, keyless GitHub login is **separate** from E2EE GitHub signup.
...
- After activation, an `optional` server may offer a first-login **choice** (E2EE vs
  plaintext) when GitHub supports both E2EE provisioning and token-only provisioning.
  See [Server Auth](/self-hosting/auth).

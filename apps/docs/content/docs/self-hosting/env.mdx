...
- `PORT` (default `3005`; some managed/self-host flows use `53288`)
...
- `METRICS_ENABLED` (default `true`)
  Direct `happier-server` launches default to `true`. Some managed self-host/runtime wrappers override this to `false` unless you opt in explicitly, to avoid metrics-port collisions in multi-process setups.
- `METRICS_PORT` (default `9090`)
- `HAPPIER_PRESENCE_STREAM_MAXLEN` (alias: `HAPPY_PRESENCE_STREAM_MAXLEN`, default `100000`)
  Presence stream trim target; set `0` to disable trimming
...
- `HAPPIER_PRESENCE_WORKER_RECLAIM_IDLE_MS` (default `60000`)
  Presence worker batching and reclaim tuning
...
If you run **multiple API replicas**, configure **sticky sessions** at the reverse proxy or load balancer. Polling fallback is part of the supported realtime path, so requests from one logical Socket.IO client must keep reaching the same API process while that transport is active.
...
Your reverse proxy idle timeout must be greater than the server’s Socket.IO `pingInterval + pingTimeout`.

The current Happier server defaults require **more than 60 seconds**. A higher timeout such as several minutes is safer for long-lived realtime connections.
...
The server exposes Prometheus-format metrics on `GET /metrics`.

For self-hosting, the recommended stack is:

- Prometheus for scraping and storage
- Grafana for dashboards
- Alertmanager for alert routing

The repo-owned Docker/Dokploy-friendly pack lives in:

- `apps/server/observability/`

See [Observability](/self-hosting/observability) for the full setup and dashboard pack.

## Live Activity remote updates (optional)

Live Activity remote updates are disabled by default. Enable them only if you want your server to update or end iOS Live Activities after the app has registered a remote target.

The full configuration reference is in [Live Activity remote updates](/self-hosting/live-activity-remote-updates). The main selected-server variables are:

- `HAPPIER_LIVE_ACTIVITY_REMOTE_UPDATES_ENABLED`
- `HAPPIER_LIVE_ACTIVITY_REMOTE_UPDATE_MODE`
- `HAPPIER_LIVE_ACTIVITY_REMOTE_UPDATE_ALLOW_FALLBACK`
- `HAPPIER_LIVE_ACTIVITY_REMOTE_UPDATE_DEDUPE_WINDOW_MS`
- `HAPPIER_LIVE_ACTIVITY_BACKGROUND_WAKE_ENABLED`
- `HAPPIER_LIVE_ACTIVITY_BACKGROUND_WAKE_MIN_INTERVAL_MS`
- `HAPPIER_LIVE_ACTIVITY_HOSTED_RELAY_ALLOWED`
- `HAPPIER_LIVE_ACTIVITY_HOSTED_RELAY_BASE_URL`
- `HAPPIER_LIVE_ACTIVITY_HOSTED_RELAY_ACCESS_KEY`
- `HAPPIER_LIVE_ACTIVITY_APNS_TEAM_ID`
- `HAPPIER_LIVE_ACTIVITY_APNS_KEY_ID`
- `HAPPIER_LIVE_ACTIVITY_APNS_PRIVATE_KEY`
- `HAPPIER_LIVE_ACTIVITY_APNS_PRIVATE_KEY_FILE`
- `HAPPIER_LIVE_ACTIVITY_APNS_BUNDLE_IDS`
- `HAPPIER_LIVE_ACTIVITY_APNS_ACTIVITY_NAMES`
- `HAPPIER_LIVE_ACTIVITY_APNS_ENVIRONMENT`

APNs private keys are configured through environment variables or mounted secret files. There is no Live Activity-only settings file or database settings model for APNs credentials.
...
- `HAPPIER_PUBLIC_SERVER_URL`
  ...
  - If you intentionally run LAN-only, using `http://<lan-ip>:<your-port>` is fine (for example `http://<lan-ip>:3005`), but QR/deep links will only work on the same LAN/VPN.
...
<Callout type="info">
  This page documents the variables self-hosters tune most, with their defaults.
  The server reads 158 `HAPPIER_FEATURE_*` variables in total —
  [Feature environment variables](/self-hosting/feature-env) lists every one,
  generated from the server's schema.
</Callout>
...
Happier's target plaintext-account contract can use OAuth providers (GitHub/OIDC) as
**keyless login methods**. Keyless means a token-only credential with no recovery
secret, Account machine key, Account content key, or fabricated substitute.
...
- Keyless auto-provisioning requires keyless accounts and a storage policy that allows
  plaintext (see [Server Auth](/self-hosting/auth) for the current prerequisites).
- The complete token-only external-auth/UI/daemon flow is still activation-closed.
  Keep auto-provisioning disabled in production until the Server Auth guide removes
  that rollout warning; env-schema presence is not activation evidence.
...
- `HAPPIER_FEATURE_ENCRYPTION__PLAIN_ACCOUNT_CREDENTIALS_AT_REST` (default `server_sealed`)
  Values: `none` | `server_sealed`
- `HAPPIER_FEATURE_ENCRYPTION__PLAIN_ACCOUNT_ARTIFACTS_AT_REST` (default `server_sealed`)
  Values: `none` | `server_sealed`

`none` stores canonical plain values directly in the database. `server_sealed` is
server-owned persistence protection derived from `HANDY_MASTER_SECRET`; public wire
responses remain plain and no client key is involved.

Back up the database and exact matching master secret together. The current server has
no multi-key read or automatic re-seal rotation path, so do not replace
`HANDY_MASTER_SECRET` in place. A restore with a different secret, or a direct secret
change, can leave server-sealed values and other master-secret-derived auth/token
material unavailable. See [Server → Encryption & plaintext storage](/self-hosting/encryption).

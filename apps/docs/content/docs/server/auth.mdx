...
- [GitHub auth](/server/auth-github)
- [OIDC auth](/server/auth-oidc)
- [mTLS auth](/server/auth-mtls)
- [Custom auth providers](/server/auth-custom-providers)
...
- GitHub: [GitHub auth](/server/auth-github)
- OIDC: [OIDC auth](/server/auth-oidc)
...
- for mTLS keyless login: see [mTLS auth](/server/auth-mtls)
...
See [GitHub auth](/server/auth-github) for full setup steps.

### OIDC-only signup (example: Okta) + OIDC required for login

See [OIDC auth](/server/auth-oidc) for provider configuration.

...
- If you disable E2EE (encryption opt-out / plaintext storage), the server can read and index session content (enabling features like server-side search). See: [Encryption & plaintext storage](/server/encryption).
...
Direct mode would have the Happier API terminate TLS itself and verify the client

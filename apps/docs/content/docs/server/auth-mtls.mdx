...
## Keyless behavior contract (activation pending)

mTLS is intended to be a **keyless** login method for plain accounts:

- A genuinely keyless mTLS credential is token-only and contains zero Account E2EE
  material.
- After activation, mTLS provisioning creates a token-only/plain account without
  fabricating a device key.
...
The complete token-only UI/daemon path remains activation-closed while account-data
migrations and composed checks finish. Keep mTLS keyless auto-provisioning disabled in
production until the [Server Auth](/server/auth) rollout warning is removed.
Configuration-schema presence is not activation evidence.

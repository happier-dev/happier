...
## Keyless OIDC login contract (activation pending)

The target plaintext-account flow can use an OIDC provider as a **keyless login
method**. Keyless means a bearer-token credential with zero Account E2EE material,
not a device-generated replacement key.
...
Notes:
- After activation, keyless OIDC login is **separate** from E2EE OIDC signup.
...
- After activation, an `optional` server may offer a first-login **choice** (E2EE vs
  plaintext) when the provider supports both E2EE provisioning and token-only
  provisioning.

...
- On `HAPPIER_FEATURE_ENCRYPTION__STORAGE_POLICY=optional`, the client may offer a **choice** on first login (E2EE vs plaintext) when GitHub is enabled for both keyed provisioning and keyless auto-provisioning. See [Server Auth](/server/auth).

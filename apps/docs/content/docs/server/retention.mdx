...
When a client is connected to a server with a finite policy represented by the released v1 capability,
Happier surfaces that policy in the UI so users understand what the server keeps and what it eventually
deletes. Message-level policies are not represented by v1 and therefore are not currently shown in the
app; operators should communicate those policies separately.

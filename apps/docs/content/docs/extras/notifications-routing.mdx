...
- `Allow` and `Deny` execute the permission response directly when Happier can verify the target server/session and direct actions are allowed for that surface
- if direct execution is not safe, `Allow` and `Deny` fall back to opening the session for review
...
If the notification targets the server that is already active and the action target can be verified:
...
If the notification targets a saved server profile that is not currently active and can be switched to safely:

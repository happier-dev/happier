...
1. Install your Relay (examples):
   ...
   - Any machine (managed service): install the CLI, then run `happier relay host install` (or remote: `happier relay host install --ssh user@host`). To install as a system-wide service, use `--mode system` (requires root/sudo).
...
If you need a CLI fallback for the daemon host:

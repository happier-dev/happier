...
export function startCaffeinate(): boolean {
    ...
    if (configuration.disableCaffeinate) {
        logger.debug('[caffeinate] Caffeinate disabled via HAPPIER_DISABLE_CAFFEINATE environment variable');
        return false;
    }
    ...
    if (caffeinateProcess && !caffeinateProcess.killed) {
        ...
        try {
            ...
            await new Promise((resolve) => {
                const timeout = setTimeout(resolve, 1000) as unknown as { unref?: () => void };
                timeout.unref?.();
            });

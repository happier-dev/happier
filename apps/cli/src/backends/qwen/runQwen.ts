...
import { waitForNextQwenMessage } from './utils/waitForNextQwenMessage';

export async function runQwen(opts: {
  ...
  connectionState.setBackend('Qwen Code');
  ...
  if (typeof opts.existingSessionId === 'string' && opts.existingSessionId.trim()) {
    ...
    logger.debug(`[qwen] Attaching to existing Happy session: ${existingId}`);
    ...
    try {
      ...
      if (!snapshot) {
        logger.debug('[qwen] Failed to fetch session metadata snapshot before attach startup update; continuing without metadata write (non-fatal)');
      }
    } catch (error) {
      logger.debug(
    ...
    primeAgentStateForUi(session, '[Qwen]');
  ...
  if (hasTTY) {
    console.clear();
    inkInstance = render(React.createElement(QwenTerminalDisplay, {
      messageBuffer,
  ...
  const handleAbort = async () => {
    logger.debug('[Qwen] Abort requested');
    session.sendAgentMessage('qwen', { type: 'turn_aborted', id: randomUUID() });
    ...
    try {
      ...
      logger.debug('[Qwen] Failed to cancel current operation (non-fatal)', e);
    }
  };
  abortRequestedCallback = handleAbort;

  const handleKillSession = async () => {
    logger.debug('[Qwen] Kill session requested');
    shouldExit = true;
    await handleAbort();
    try {
      if (session) {
  ...
  const sendReady = () => {
    ...
    try {
      api.push().sendToAllDevices("It's ready!", 'Qwen Code is waiting for your command', { sessionId: session.sessionId });
    } catch (pushError) {
      logger.debug('[Qwen] Failed to send ready push', pushError);
    }
  };
  ...
  try {
    ...
    while (!shouldExit) {
      ...
      if (!message) {
        const next = await waitForNextQwenMessage({
          messageQueue,
      ...
      if (wasStarted && currentModeHash && message.hash !== currentModeHash) {
        ...
        messageBuffer.addMessage('Restarting Qwen Code session (permission settings changed)…', 'status');
        await runtime.reset();
        wasStarted = false;
        permissionHandler.reset();
        thinking = false;
      ...
      if (special.type === 'clear') {
        messageBuffer.addMessage('Resetting Qwen Code session…', 'status');
        await runtime.reset();
        wasStarted = false;
        permissionHandler.reset();
        thinking = false;
      ...
      try {
        ...
        if (!wasStarted) {
          ...
          if (resumeId) {
            ...
            try {
              ...
              logger.debug('[Qwen] Resume failed; starting a new session instead', e);
              messageBuffer.addMessage('Resume failed; starting a new session.', 'status');
              session.sendAgentMessage('qwen', { type: 'message', message: 'Resume failed; starting a new session.' });
        ...
        logger.debug('[Qwen] Error during prompt:', error);
        session.sendAgentMessage('qwen', { type: 'message', message: `Error: ${error instanceof Error ? error.message : String(error)}` });
      } finally {
        runtime.flushTurn();

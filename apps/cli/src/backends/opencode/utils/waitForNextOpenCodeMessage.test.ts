...
type OpenCodeSessionFixture = Pick<ApiSessionClient, 'popPendingMessage' | 'waitForMetadataUpdate'>;

function createQueue(): MessageQueue2<QueueMode> {
  return new MessageQueue2<QueueMode>(() => 'hash');
}

function asSessionClient(session: OpenCodeSessionFixture): ApiSessionClient {
...
describe('waitForNextOpenCodeMessage', () => {
  it('wakes on metadata update and then processes a pending-queue item', async () => {
    ...
    const session: OpenCodeSessionFixture = {
    ...
    const resultPromise = waitForNextOpenCodeMessage({
      messageQueue: queue,

...
import { createMainTranscriptStore } from './mainTranscriptStore';

describe('createMainTranscriptStore', () => {
    let workingDirectory: string;
    let claudeConfigDir: string;
    let projectDir: string;
    let lifecycle: ReturnType<typeof createMainTranscriptStore> | null = null;
    ...
    it('uses the store for canonical main transcripts and fences explicit noncanonical transcript paths as exceptions', async () => {
        ...
        lifecycle = createMainTranscriptStore({
            workingDirectory,
            claudeConfigDir,
            invalidate: () => {},
        });

...
async function waitForTranscriptToSettle(params: Readonly<{ transcriptPath: string }>): Promise<boolean> {
    ...
    const settleWindowMs = Math.min(
        timeoutMs,
        Math.max(effectivePollIntervalMs * 2, 150),
    );
    ...
    try {
        ...
        while (Date.now() < deadline) {
            ...
            if (size === lastSize && mtimeMs === lastMtimeMs) {
                if (Date.now() - stableSinceMs >= settleWindowMs) {
                    return true;
                }
                continue;
            }
...
export async function repairClaudeTranscriptAfterInterrupt(params: Readonly<{
    ...
    const settledAfterWait = await waitForTranscriptToSettle({ transcriptPath: path }).catch(() => false);
    if (!settledAfterWait) return;

...
export class Session {
    ...
    readonly queue: ClaudeEnhancedModeMessageQueue;
    ...
    private abortCurrentTurnHandler: (() => Promise<void>) | null = null;
    ...
    setPushSender(pushSender: SessionPushSender | null): void {
    ...
    setAbortCurrentTurnHandler(handler: (() => Promise<void>) | null): void {
        this.abortCurrentTurnHandler = handler;
    }

    async abortCurrentTurn(): Promise<void> {
        await this.abortCurrentTurnHandler?.();
    }
    ...
    cleanup = (): void => {
        ...
        this.abortCurrentTurnHandler = null;
    ...
    consumeOneTimeFlags = (): void => {
        ...
        for (let i = 0; i < this.claudeArgs.length; i++) {
            ...
            if (arg === '--session-id') {
                if (i + 1 < this.claudeArgs.length) {
                    ...
                    if (!nextArg.startsWith('-')) {
                        i++; // Skip the value
                        logger.debug('[Session] Consumed --session-id flag with explicit value');
                    } else {
            ...
            if (arg === '--resume' || arg === '-r') {
                ...
                if (nextArg && !nextArg.startsWith('-')) {
                    i++; // Skip the value
                    logger.debug(`[Session] Consumed ${arg} flag with explicit session ID`);
                } else {
        ...
        logger.debug('[Session] Consumed one-time flags', {
            remainingArgCount: this.claudeArgs?.length ?? 0,
        });
    }
}

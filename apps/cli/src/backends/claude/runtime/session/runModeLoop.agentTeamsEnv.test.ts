...
vi.mock('../remote/launcher', () => ({
  launchClaudeRemoteSession: vi.fn(async () => 'exit' as const),
}));
...
describe('loop (Agent Teams env)', () => {
  it('seeds Session.claudeCodeExperimentalAgentTeamsEnabled from loop options for the initial terminal spawn', async () => {
    const client = createSessionClientStub();
    const queue = new MessageQueue2<any>(() => 'mode');

    const { runClaudeModeLoop: loop } = await import('./runModeLoop');
    ...
    await loop({
      ...
      launchTerminal: async (params: { session: ClaudeLauncherSession; options?: { entry?: LaunchEntry } }) =>
        await mockClaudeLocalLauncher(params.session, params.options),

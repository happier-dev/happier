...
import { isClaudeCliJavaScriptFile, resolveClaudeCliPath } from '@/backends/claude/utils/resolveClaudeCliPath';
import { getClaudeSystemPrompt } from '@/backends/claude/utils/systemPrompt';
...
export async function runClaudeTerminalSession(opts: {
    abort: AbortSignal,
    ...
    try {
        ...
        await new Promise<void>((r, reject) => {
            ...
            logger.debug('[ClaudeLocal] Prepared launch args', {
                argCount: args.length,
                flagCount: args.filter((arg) => typeof arg === 'string' && arg.startsWith('-')).length,
                positionalCount: positionalArgs.length,
                usesNodeLauncher: shouldUseNodeLauncher,
            });

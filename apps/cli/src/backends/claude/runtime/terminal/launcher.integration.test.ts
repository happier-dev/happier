...
type LocalLaunchOptions = Parameters<(typeof import('./runTerminalSession'))['runClaudeTerminalSession']>[0];
...
describe('Claude terminal launcher', () => {
  ...
  it('surfaces Claude process errors to the UI', async () => {
    ...
    const { launchClaudeTerminalSession: claudeLocalLauncher } = await import('./launcher');
  ...
  it('exits via host abort before scanner startup finishes and does not spawn local Claude', async () => {
    const { session, client } = createLocalHarness();
    const scannerReady = createDeferred<SessionScannerResult>();

    session.onSessionFound('sess_1', hookWithTranscript('/tmp/sess_1.jsonl'));
    mockCreateSessionScanner.mockReturnValueOnce(scannerReady.promise);

    const { launchClaudeTerminalSession: claudeLocalLauncher } = await import('./launcher');
    const launcherPromise = claudeLocalLauncher(session);

    expect(mockCreateSessionScanner).toHaveBeenCalledTimes(1);
    ...
    const abortPromise = abortCurrentTurn();

    scannerReady.resolve(createSessionScannerStub());
  ...
  it('passes through user --mcp-config args and does not parse/merge them into happierMcpConfigJson', async () => {
    ...
    const loggedArgs = vi.mocked(logger.debug).mock.calls
      .flat()
      .map((value) => (typeof value === 'string' ? value : JSON.stringify(value)))
      .join('\n');
    expect(loggedArgs).not.toContain(userMcpConfig);
    expect(loggedArgs).not.toContain('--mcp-config');
    expect(result).toEqual({ type: 'exit', code: 0 });
  });
  ...
  it('adopts permission mode metadata updates during local mode for future spawns', async () => {
    ...
    const { session, client } = createLocalHarness({ metadataSnapshot });
  ...
  it('returns switch (not exit) when Claude is terminated during app-triggered local→remote switch', async () => {
    ...
    const { ExitCodeError } = await import('./runTerminalSession');

...
export class ClaudeLocalPermissionBridge {
    ...
    activate(): void {
        this.session.getOrCreatePermissionRpcRouter().registerConsumer({
            ...
            tryHandlePermissionRpc: (payload: PermissionRpcPayload) => this.tryHandlePermissionRpc(payload),
        });
    ...
    private syncPermissionModeFromMetadataSnapshot(): PermissionMode | null {
        let appliedMode: PermissionMode | null = null;
        applyPermissionIntentFromMetadataIfNewer({
            metadata: this.session.client.getMetadataSnapshot?.() ?? null,
            currentPermissionModeUpdatedAt: this.permissionModeUpdatedAt,
            apply: ({ intent, updatedAt }) => {
                this.permissionModeUpdatedAt = updatedAt;
                if (intent === this.permissionMode) {
                    return;
                }

                this.permissionMode = intent;

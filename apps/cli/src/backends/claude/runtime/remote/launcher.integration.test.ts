...
vi.mock('./dispatch', () => ({
  dispatchClaudeRemoteSession: mockClaudeRemoteDispatch,
}));
...
function getSessionAbortController(session: Session): () => Promise<void> {
  const candidate = session as Session & Partial<{
    abortCurrentTurn: () => Promise<void>;
  }>;
  if (typeof candidate.abortCurrentTurn !== 'function') {
    throw new Error('Expected Claude session to expose an abortCurrentTurn callback');
  }
  return candidate.abortCurrentTurn.bind(session);
}
...
function createRemoteHarness(options?: {
  ...
  const client: SessionClientStub = {
    ...
    getMetadataSnapshot: () => options?.metadata ?? null,
  ...
  return {
	...
	describe.sequential('claudeRemoteLauncher', () => {
	  ...
	  beforeEach(() => {
        ...
        mockClaudeRemoteDispatch.mockReset();
        mockResetParentChain.mockReset();
        mockUpdateSessionId.mockReset();
        mockConvert.mockReset();
        mockConvertSidechainUserMessage.mockReset();
        mockGenerateInterruptedToolResult.mockReset();
        vi.doUnmock('@/ui/tty/resolveHasTTY');
        vi.doUnmock('@/ui/remoteControl/remoteModeControl');
    ...
    mockInkRender.mockReturnValue({ unmount: vi.fn() });
  ...
  it('exits instead of relaunching when the remote input queue closes during dispatch teardown', async () => {
    ...
    const { launchClaudeRemoteSession: claudeRemoteLauncher } = await import('./launcher');
  ...
  it('keeps abort ownership off the provider RPC surface while exposing a session abort callback for remote turn interrupts', async () => {
    const waitWithin = async <T,>(promise: Promise<T>, label: string, ms: number = 5000): Promise<T> => {
      return await Promise.race([
        promise,
        new Promise<T>((_resolve, reject) => {
    ...
    await waitWithin(dispatchObserved.promise, 'remote dispatch mock did not run');
    expect(harness.client.rpcHandlerManager.registerHandler).not.toHaveBeenCalledWith('abort', expect.any(Function));
    const abortCurrentTurn = getSessionAbortController(harness.session);
  ...
  it('marks the session as abort-requested when the session abort callback is invoked', async () => {
    const waitWithin = async <T,>(promise: Promise<T>, label: string, ms: number = 5000): Promise<T> => {
      return await Promise.race([
        promise,
        new Promise<T>((_resolve, reject) => {
  ...
  it('falls back to process abort when no turn interrupt handler is installed', async () => {
    const waitWithin = async <T,>(promise: Promise<T>, label: string, ms: number = 5000): Promise<T> => {
      return await Promise.race([
        promise,
        new Promise<T>((_resolve, reject) => {
    ...
    const dispatchObserved = createDeferred<void>();
    const signalAbortedObserved = createDeferred<void>();
    ...
    mockClaudeRemoteDispatch.mockImplementationOnce(async (opts: any) => {
      ...
      dispatchObserved.resolve(undefined);
      await waitForAbort(opts.signal);
      signalAbortedObserved.resolve(undefined);
    });
    ...
    const abortCurrentTurn = getSessionAbortController(harness.session);
    await abortCurrentTurn();
    await waitWithin(signalAbortedObserved.promise, 'remote abort did not propagate to the dispatch signal');
  ...
  it('does not strip assistant text/thinking blocks when the Agent SDK runner is selected even if the enablement flag is disabled', async () => {
    const waitWithin = async <T,>(promise: Promise<T>, label: string, ms: number = 5000): Promise<T> => {
      return await Promise.race([
        promise,
        new Promise<T>((_resolve, reject) => {
    ...
    // Seed a first queued message that disables the Agent SDK flag, but the canonical remote path still uses Agent SDK.
    ...
    mockClaudeRemoteDispatch.mockImplementationOnce(async (opts: any) => {
      ...
      // Emit a plain assistant message (no tool blocks) as the Agent SDK runner would.
      opts.onMessage?.({
        type: 'assistant',
    ...
    // The Agent SDK runner must keep text/thinking content intact when converting.
  ...
  it('creates a streamed transcript writer even when sendAgentMessageCommitted is unavailable (best-effort)', async () => {
    ...
    const harness = createRemoteHarness();
    delete (harness.client as Partial<Pick<SessionClientStub, 'sendAgentMessageCommitted'>>).sendAgentMessageCommitted;
    harness.session.queue.push('hello', { permissionMode: 'default', claudeRemoteAgentSdkEnabled: true } as any);
  ...
  it('flushes agent-team inbox messages on shutdown', async () => {
    ...
    try {
      ...
      const { dispatchClaudeRemoteSession: claudeRemoteDispatch } = await import('./dispatch');
  ...
  it('clears the Claude session id after an abort-triggered exit code 1 so the next launch does not keep resuming a dead session', async () => {
    ...
    const { session, client, switchHandlerReady } = createRemoteHarness({ sessionId: 'dead-session-id' });
    session.queue.push('hello', { permissionMode: 'default' } satisfies EnhancedMode);
    ...
    try {
      ...
      expect(client.rpcHandlerManager.registerHandler).not.toHaveBeenCalledWith('abort', expect.any(Function));
      const abortCurrentTurn = getSessionAbortController(session);
      await abortCurrentTurn();
  ...
  it('does not mount Ink UI for daemon-started sessions even when a TTY is available', async () => {
    ...
    const { session, switchHandlerReady } = createRemoteHarness({ sessionId: 'sess_0', startedBy: 'daemon' });
  ...
  it('starts the static remote-control surface for daemon-started tmux sessions', async () => {
    ...
    const startRemoteModeStaticControl = vi.fn((params: unknown) => {
      void params;
      return { stop: stopStaticControl };
    });
    ...
    await vi.waitFor(() => {
      expect(startRemoteModeStaticControl).toHaveBeenCalledTimes(1);
    });
    expect(startRemoteModeStaticControl.mock.calls[0]?.[0]).toMatchObject({
      providerName: 'Claude',

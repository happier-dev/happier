 ...
 * - HAPPIER_USE_BUNDLED_CLAUDE=1: Force use of node_modules version (skip global search)
 * - HAPPIER_USE_GLOBAL_CLAUDE=1: Force use of global version (if available)
 */
export function getDefaultClaudeCodePath(): string {
    ...
    if (process.env.HAPPIER_CLAUDE_PATH) {
        logger.debug(`[Claude SDK] Using HAPPIER_CLAUDE_PATH: ${process.env.HAPPIER_CLAUDE_PATH}`)
        return process.env.HAPPIER_CLAUDE_PATH
    }

    // Force bundled version if requested
    if (process.env.HAPPIER_USE_BUNDLED_CLAUDE === '1') {

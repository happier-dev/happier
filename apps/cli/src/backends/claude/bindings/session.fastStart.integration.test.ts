...
vi.mock('@/backends/claude/runtime/session/runModeLoop', async (importOriginal) => {
  ...
  return {
    ...
    runClaudeModeLoop: vi.fn(async (opts: any) => {
      ...
      loopStarted.resolve();
      if (autoSessionReady) {
        opts?.onSessionReady?.(currentLoopSessionOverride ?? {
          cleanup: vi.fn(),
          setPushSender: vi.fn(),
...
let createClaudeSessionBindingFn: typeof import('./session').createClaudeSessionBinding;
let runHostSessionRuntimeFn: typeof import('@/agent/runtime/sessionLoop/runHostSessionRuntime').runHostSessionRuntime;
...
describe('Claude session binding fast-start', () => {
  ...
  beforeAll(async () => {
    ...
    ({ createClaudeSessionBinding: createClaudeSessionBindingFn } = await import('./session'));
    ({ runHostSessionRuntime: runHostSessionRuntimeFn } = await import('@/agent/runtime/sessionLoop/runHostSessionRuntime'));
...
afterEach(() => {
  ...
  async function runClaudeViaLiveHostPath(
    ...
    const plan = await createClaudeSessionBindingFn({
      credentials,
      ...options,
    });
  ...
  it('emits a startup timing summary line after session attach when enabled', async () => {
    ...
    const timingLine = await waitFor(

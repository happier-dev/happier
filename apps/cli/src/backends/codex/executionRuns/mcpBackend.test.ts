...
import type { CodexSessionConfig, CodexToolResponse } from '@/backends/codex/mcp/types';
...
async function loadBackendWithFakeClient(state: FakeClientState) {
  vi.doMock('@/backends/codex/mcp/sessionClient', () => ({
  ...
  const mod = await import('./mcpBackend');
  return mod.mcpBackend({
    cwd: process.cwd(),
...
describe('mcpBackend', () => {
  ...
  it('does not apply a default response timeout when timeoutMs is omitted', async () => {
    ...
    if (!backend.waitForTurnCompletion) {
      throw new Error('Expected waitForTurnCompletion to be defined');
    }
    const waiting = backend.waitForTurnCompletion();
  ...
  it('defers vendor session creation until the first prompt when startSession has no initial prompt', async () => {
    ...
    const unsubscribe = backend.subscribeMessages(onMessage);
  ...
  it('continues the same vendor session after canceling a turn', async () => {
    const state: FakeClientState = {
  ...
  it('includes the execution-run cwd in the first MCP start config', async () => {
    ...
    const { mcpBackend } = await import('./mcpBackend');
  ...
  it('queues a replacement prompt until the in-flight start settles and then continues the same session after cancel', async () => {
    ...
    expect(state.startCalls).toHaveLength(1);
    expect(state.continueCalls).toEqual([{ prompt: 'replacement prompt', signal: expect.any(AbortSignal) }]);
  });

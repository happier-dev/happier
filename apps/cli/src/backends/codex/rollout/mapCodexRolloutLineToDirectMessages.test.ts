...
describe('mapCodexRolloutLineToDirectMessages', () => {
  it('filters harness blobs and preserves stable direct transcript rows for rollout actions', () => {
    const actions: readonly CodexRolloutAction[] = [
      { type: 'user-text', text: '# AGENTS.md instructions\nignore this blob' },
      { type: 'user-text', text: 'Investigate direct transcript paging parity' },
    ...
    expect(items[0]?.raw).toEqual({
      role: 'user',
      content: { type: 'text', text: 'Investigate direct transcript paging parity' },
    });

...
describe('runCodex app-server MCP wiring behavior', () => {
  beforeEach(async () => {
  ...
  it('wires Happier MCP servers into the future app-server runtime', async () => {
    ...
    expect(startOrLoad?.mock.calls[0]?.[0]).toMatchObject({ resumeId: 'vendor-thread-existing-123', importHistory: false });
    expect(outcome).toMatchObject({ ok: false });
    if (outcome.ok) throw new Error('expected runCodex to fail in test');
    const failedOutcome = outcome;
    expect((failedOutcome.error as Error).name).toBe('StrictInitialResumeError');
    await expect(failedOutcome.error).toEqual(expect.objectContaining({
      cause: expect.objectContaining({ message: expect.stringMatching(/appServer-startOrLoad-called/) }),
    }));
  });

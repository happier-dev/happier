...
vi.mock('@/agent/runtime/createRuntimeOverrideSynchronizers', () => ({
  createRuntimeOverrideSynchronizers: (...args: any[]) => createRuntimeOverrideSynchronizersSpy(...args),
  createAcpRuntimeOverrideSynchronizers: (...args: any[]) => createRuntimeOverrideSynchronizersSpy(...args),
}));
...
describe('runCodex app-server attach behavior', () => {
  beforeEach(async () => {
  ...
  it('flushes pending remote override synchronizers after app-server attach startup', async () => {
    ...
    createCodexAppServerRuntimeSpy.mockImplementationOnce(() => createFakeCodexAppServerRuntime({
    ...
    expect(createRuntimeOverrideSynchronizersSpy).toHaveBeenCalledTimes(1);

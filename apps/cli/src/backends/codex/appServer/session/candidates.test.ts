...
describe('candidates', () => {
    ...
    it('emits canonical runtimeDescriptorV1 only for app-server thread candidates', async () => {
        await withTempDir('happier-codex-direct-app-server-runtime-descriptor-', async (root) => {
            ...
            expect(result[0]?.details).not.toHaveProperty('agentRuntimeDescriptorV1');
            expect(result[0]?.details).not.toHaveProperty('runtimeDescriptor');
        });
    });
});

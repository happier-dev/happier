...
describe('runCodex CodexACP resume behavior', () => {
  ...
  it('does not probe Codex ACP capabilities during startup for --resume sessions', async () => {
    ...
    const startOrLoadSession = createdRuntime?.startOrLoadSession as ReturnType<typeof vi.fn> | undefined;
    expect(startOrLoadSession?.mock.calls[0]?.[0]).toMatchObject({ resumeId: 'resume-123', importHistory: false });
    expect(outcome.ok).toBe(false);
  });

  it('does not pass attach metadata cleanup keys through the shared startup contract', async () => {
    const experiments = await import('@/backends/codex/experiments');
    ...
    expect(initializeOpts.metadataKeysToUnsetOnAttach).toBeUndefined();
  });
  ...
  it('fails closed for explicit --resume when Codex ACP loadSession fails', async () => {
    ...
    const createdRuntime = createCodexAcpRuntimeSpy.mock.results[0]?.value as any;
    const startOrLoadSession = createdRuntime?.startOrLoadSession as ReturnType<typeof vi.fn> | undefined;
    expect(startOrLoadSession).toBeTruthy();
    expect(startOrLoadSession?.mock.calls.length).toBe(1);
    expect(startOrLoadSession?.mock.calls[0]?.[0]).toMatchObject({ resumeId: 'resume-123', importHistory: false });
    await expect(startOrLoadSession?.mock.results?.[0]?.value).rejects.toThrow(/startOrLoad-called/);

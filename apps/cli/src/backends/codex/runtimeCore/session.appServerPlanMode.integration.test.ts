...
vi.mock('@/agent/runtime/runtimeOverridesSynchronizer', () => ({
  ...
  setupRuntimeMetadataDrivenOverridesSync: vi.fn(async () => ({
    runtimeControlSync: null,
    syncOverridesFromMetadata: vi.fn(),
  })),
}));

vi.mock('@/agent/runtime/mode/switching/createModeController', () => ({
  createTerminalRemoteModeController: vi.fn(() => ({
    publishModeState: vi.fn(async () => {}),
    registerRemoteSwitchHandler: vi.fn(),
  })),
}));

vi.mock('../runtime/remote/createRemoteTerminalUi', () => ({
  createCodexRemoteTerminalUi: vi.fn(() => ({
    mount: vi.fn(),
    unmount: vi.fn(async () => {}),
    setAllowSwitchToTerminal: vi.fn(),
  })),
}));
...
vi.mock('../utils/resolveCodexStartupRuntimeMode', () => ({
  resolveCodexStartupRuntimeMode: vi.fn(() => 'remote'),
}));
...
describe('runCodex app-server startup plan mode', () => {
  ...
  it('prefers explicit codexBackendMode=appServer over ACP-only attach behavior', async () => {
    const root = await createTempDir('happier-codex-run-explicit-appserver-attach-');
    tempRoots.add(root);
    ...
    const fakeAppServer = await writeFakeCodexAppServerScript({
      ...
      bodyLines: [
        ...
        '    process.stdout.write(JSON.stringify({ id: msg.id, result: [{ id: "gpt-5.4", displayName: "GPT-5.4", isDefault: true, supportedReasoningEfforts: [{ reasoningEffort: "medium", description: "Balances speed and reasoning depth for everyday tasks" }], defaultReasoningEffort: "medium" }] }) + "\\n");',
        '    continue;',
        '  }',
    ...
    initializeBackendRunSessionImpl = async () => ({
      session,
    ...
    const stopAfterFirstTurn = new Error('test-loop-stop');
    let waitCallCount = 0;
    waitForMessagesOrPendingSpy.mockImplementation(async () => {
      waitCallCount += 1;
      if (waitCallCount === 1) {
        return {
          message: { text: 'resume the attached session', localId: null },
          mode: {
            permissionMode: 'read-only',
            permissionModeUpdatedAt: 1,
          },
          isolate: false,
          hash: 'hash-explicit-app-server-1',
        };
      }
      throw stopAfterFirstTurn;
    });
    ...
    if (!outcome.ok && outcome.error !== stopAfterFirstTurn) {
      throw outcome.error;
    }
      ...
      expect.arrayContaining([
        expect.objectContaining({
          ...
          params: expect.objectContaining({
            threadId: 'vendor-thread-existing-123',
          }),
        }),
      ]),
    );
  ...
  it('threads startup plan, model, reasoning, and Fast overrides into the first app-server turn/start payload', async () => {
    ...
    waitForMessagesOrPendingSpy.mockImplementation(async () => {
      ...
      if (waitCallCount === 1) {
        return {
          message: { text: 'plan this change', localId: null },
          mode: {
            permissionMode: 'default',
            permissionModeUpdatedAt: 1,
          },
  ...
  it('seeds fresh app-server thread/start with selected model and Fast overrides before the first turn', async () => {
    ...
    waitForMessagesOrPendingSpy.mockImplementation(async () => {
      ...
      if (waitCallCount === 1) {
        return {
          message: { text: 'plan this fresh change', localId: null },
          mode: {
            permissionMode: 'default',
            permissionModeUpdatedAt: 1,
          },

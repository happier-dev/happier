...
vi.mock('@/agent/terminalRuntime/providers/requireTerminalRuntimeLaunch', () => ({
  requireTerminalRuntimeLaunch: vi.fn(async () => (opts: any) => terminalRuntimeLaunchSpy(opts)),
}));
...
vi.mock('@/backends/codex/terminalRuntime/launchTerminalRuntime', () => ({
  launchCodexTerminalRuntime: vi.fn(async () => {
    throw new Error('legacy-launchCodexTerminalRuntime-called');
  }),
}));
vi.mock('@/agent/runtime/startup/readRequiredStartupMachineId', () => ({
  readRequiredStartupMachineId: vi.fn(async () => 'machine_1'),
}));
...
vi.mock('@/backends/codex/utils/resolveCodexStartupRuntimeMode', () => ({
  resolveCodexStartupRuntimeMode: vi.fn(() => 'terminal'),
}));
...
describe('runCodex fast-start', () => {
  ...
  it('forwards Codex ACP fallback messages to the session', async () => {
    ...
    try {
      await expect(waitFor(localStarted.promise, 500)).resolves.toBeUndefined();
    } finally {

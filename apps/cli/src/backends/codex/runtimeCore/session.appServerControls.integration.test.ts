...
describe('runCodex app-server control behavior', () => {
  beforeEach(async () => {
  ...
  it('cancels the app-server runtime when the session abort RPC is invoked mid-turn', async () => {
    ...
    if (typeof abortHandler !== 'function') {
      throw new Error('expected abort handler to be registered');
    }
  ...
  it('steers mid-turn user messages for app-server sessions and publishes inFlightSteer=true', async () => {
    ...
    const appServerRuntime = createFakeCodexAppServerRuntime({

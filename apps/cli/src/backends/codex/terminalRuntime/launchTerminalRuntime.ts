...
import { CodexTerminalRuntimeMirror } from './codexTerminalRuntimeMirror';
...
function buildCodexTuiChildEnv(): NodeJS.ProcessEnv {
  ...
  // file and breaks terminal-control discovery + switching.
...
export async function launchCodexTerminalRuntime<TMode>(opts: {
  path: string;
  api: unknown;
  ...
  // Publish terminal-control state immediately so UIs can render the terminal/remote banner
  ...
  updateAgentStateBestEffort(
    ...
    '[codex]',
    'codex_terminal_runtime_launcher_start',
  );
  ...
  const publishRemoteControlState = (tag: 'switch' | 'exit' | 'launch_error'): void => {
    ...
    updateAgentStateBestEffort(
      ...
      '[codex]',
      `codex_terminal_runtime_launcher_${tag}`,
    );
  };
  ...
  try {
    // Terminal-control: any incoming UI message triggers a mode switch to remote.
    ...
    if (!candidateFile) {
      ...
      if (!interactive && bufferedStderr.trim().length > 0) {
        console.error(`[codex] Codex terminal runtime exited before rollout file was found. stderr:\n${bufferedStderr}`);
      }
      try {
        opts.session.sendSessionEvent({ type: 'switch', mode: 'remote' });
      } catch {
    ...
    logger.debug('[codex] bindTranscript: awaiting', { filePath: candidateFile.filePath });
    ...
    logger.debug('[codex] bindTranscript: ready', { filePath: candidateFile.filePath });

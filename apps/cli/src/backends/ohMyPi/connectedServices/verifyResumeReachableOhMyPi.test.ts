...
import { verifyResumeReachableOhMyPi } from './verifyResumeReachableOhMyPi';
...
describe('verifyResumeReachableOhMyPi', () => {
  ...
  it('does not accept a persisted candidate file whose name does not match the vendor resume id', async () => {
    const root = await mkdtemp(join(tmpdir(), 'happier-ohmypi-reachability-stale-candidate-'));
    const candidate = join(root, 'sessions', '--tmp-project--', '2026-05-28T00-00-00-000Z_omp-session-B.jsonl');
    try {
      await mkdir(join(root, 'sessions', '--tmp-project--'), { recursive: true });
      await writeFile(candidate, '{}\n');

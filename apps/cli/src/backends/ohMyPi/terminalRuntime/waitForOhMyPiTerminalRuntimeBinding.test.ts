...
import { resolveOhMyPiLocalHostedDirectTranscriptBinding } from './resolveOhMyPiLocalHostedDirectTranscriptBinding';
...
describe('waitForOhMyPiTerminalRuntimeBinding', () => {
    it('waits for the breadcrumb session file to materialize before returning a binding', async () => {
        ...
        expect(resolveOhMyPiLocalHostedDirectTranscriptBinding({
            cwd,
            env,
            terminalId: 'pts-3',
        })).toEqual({
        ...
        const bindingPromise = waitForOhMyPiTerminalRuntimeBinding({
            cwd,
            env,
            terminalId: 'pts-3',
            timeoutMs: 500,
    ...
    it('returns undefined when the session file never materializes before the timeout', async () => {
        ...
        await expect(waitForOhMyPiTerminalRuntimeBinding({
            cwd,

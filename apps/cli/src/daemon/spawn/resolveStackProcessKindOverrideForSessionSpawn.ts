export function resolveStackProcessKindOverrideForSessionSpawn(
  ...
  const stackName = String(processEnv.HAPPIER_STACK_STACK ?? '').trim();
  const repoDir = String(processEnv.HAPPIER_STACK_REPO_DIR ?? '').trim();
  if (!envFile && !stackName && !repoDir) return {};

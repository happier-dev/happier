  ...
  return await withInProcessDestinationLock(destinationHome, async () => {
    ...
    try {
      return await withJsonOwnerFileLock({
        lockPath: join(destinationHome, '.happier-state-sharing.lock'),
        ...
        // Compatibility prepare phase for the exact Remote 490a27a / Dev 877ee97a predecessor
        // reader. Remove only after those processes can no longer coexist with this shared home.
        predecessorCompatibleDirectoryOwner: {

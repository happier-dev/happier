...
function createFastAcpTransportHandler() {
  return createAcpTestTransportHandler({
    agentName: 'opencode',
    idleTimeoutMs: 1,
    preToolCallIdleTimeoutMs: 1,
    idleWithoutAssistantMessageTimeoutMs: 1,
    postToolCallIdleTimeoutMs: 1,
...
describe('runPermissionModePromptLoop with ApiSessionClient idle snapshot refresh', () => {
  ...
  it('applies an ACP session mode override via the real ACP runtime after a wake-triggered snapshot refresh', async () => {
    ...
    const backend = new AcpBackend({
      ...
      args: [scriptPath],
      transportHandler: createFastAcpTransportHandler(),
    });
    ...
    const localSessionSocketStub = sessionSocketStub;
    const localUserSocketStub = userSocketStub;
    if (!localSessionSocketStub || !localUserSocketStub) {
      throw new Error('Missing session socket stub');
    }
    (localSessionSocketStub.emitWithAck as any).mockImplementation(async (event: string, payload: any) => {
    ...
    const loopPromise = runPermissionModePromptLoop({
      ...
      sendReady: () => {
        ...
        localUserSocketStub.disconnect();
        setTimeout(() => {
          localUserSocketStub.connect();
        }, 25);
      },
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => {
            ...
            reject(
              new Error(
                `Timed out waiting for ACP runtime mode override (readyCount=${readyCount}, sessionFetchCount=${sessionFetchCount}, servedSessionMetadataVersions=${servedSessionMetadataVersions.join(',') || 'none'}, metadata=${JSON.stringify(session.getMetadataSnapshot())}, modeLog=${JSON.stringify(modeLog)}, setSessionModeCalls=${setSessionModeSpy.mock.calls.length}, updateMetadataCalls=${updateMetadataSpy.mock.calls.length}, metadataAckHistory=${metadataAckHistory.join('|') || 'none'}, localMetadataVersion=${String((session as any).metadataVersion)}, waitForMetadataUpdateCalls=${waitForMetadataUpdateSpy.mock.calls.length}, refreshSessionSnapshotCalls=${refreshSessionSnapshotSpy.mock.calls.length}, userSocketConnected=${String(localUserSocketStub.connected ?? null)}, userSocketConnectCalls=${String((localUserSocketStub.connect as any)?.mock?.calls?.length ?? 0)})`,
              ),
            );
          }, 10_000),
        ),
    ...
    const loopPromise = runPermissionModePromptLoop({
      ...
      onAfterStart: () => {
        ...
        void (async () => {
          ...
          await session.updateMetadata((prev) => ({
            ...(prev as any),
      ...
      await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(() => {
            ...
            reject(
              new Error(
                `Timed out waiting for ACP runtime mode override with onAfterStart publish (readyCount=${readyCount}, metadata=${JSON.stringify(session.getMetadataSnapshot())}, modeLog=${JSON.stringify(modeLog)}, localMetadataVersion=${String((session as any).metadataVersion)}, serverMetadataVersion=${serverMetadataVersion}, waitForMetadataUpdateCalls=${waitForMetadataUpdateSpy.mock.calls.length}, refreshSessionSnapshotCalls=${refreshSessionSnapshotSpy.mock.calls.length}, userSocketConnected=${String(localUserSocketStub.connected ?? null)}, userSocketConnectCalls=${String((localUserSocketStub.connect as any)?.mock?.calls?.length ?? 0)})`,
              ),
            );
          }, 10_000),
        ),

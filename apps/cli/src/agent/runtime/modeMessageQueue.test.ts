...
describe('MessageQueue2', () => {
    ...
    it('should resolve pending wait with null when reset is called', async () => {
        const queue = new MessageQueue2<string>(mode => mode);
        ...
        const result = await Promise.race([
            waitPromise,
            new Promise<null>((_, reject) => setTimeout(() => reject(new Error('waitForMessagesAndGetAsString hung on reset')), 100)),
        ]);
        expect(result).toBeNull();
    });
    ...
    it('should stop batching when hitting isolated message', async () => {
        ...
        queue.pushIsolateAndClear('isolated', { type: 'A' });
        queue.unshift('message2', { type: 'A' });
        queue.unshift('message1', { type: 'A' });
        queue.push('message3', { type: 'A' });

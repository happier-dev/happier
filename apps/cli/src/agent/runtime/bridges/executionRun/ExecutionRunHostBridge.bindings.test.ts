...
function createStaticBackend(responseText: string): AgentBackend {
  ...
  const sessionId: SessionId = 'runtime-bindings-session-1' as SessionId;
  return {
...
describe('ExecutionRunHostBridge bindings consumption', () => {
  it('creates execution-run backends through bindings when no compatibility factory is injected', async () => {
    ...
    const bridge = new ExecutionRunHostBridge({
      ...
      cwd: '/tmp/runtime-bindings-bridge',
      sendAcp: () => {},
  ...
  it('routes prompt-capable permission requests through the parent-session envelope', async () => {
    ...
    const bridge = new ExecutionRunHostBridge({
      ...
      getNowMs: () => 1_700_000_000_000,
      createBackend: () => createExecutionRunHostRuntimeFromRuntimeTurnOperations(operations),
    });
  ...
  it('denies fail-closed permission requests through the runtime when parent prompting is unavailable', async () => {
    const sent: Array<{
      provider: string;
      body: AgentMessage;
      opts?: { meta?: Record<string, unknown> };
      ...
      async sendPrompt() {
        handlers.forEach((handler) => {
          handler({
            ...
            payload: {
              toolName: 'write',
              input: { path: '/tmp/fail-closed-deny.txt' },
            },
          } as AgentMessage);
        });
      },
    ...
    expect(calls).toContain('respondToPermission:provider-request-1:false');
    expect(sent.filter((entry) => entry.body.type === 'permission-request')).toHaveLength(0);
  });

  it('fails the run when fail-closed permission requests have no compatible responder', async () => {
    const sent: Array<{
      provider: string;
      body: AgentMessage;
      opts?: { meta?: Record<string, unknown> };
      ...
      async sendPrompt() {
        handlers.forEach((handler) => {
          handler({
            ...
            payload: {
              toolName: 'write',
              input: { path: '/tmp/fail-closed-terminate.txt' },
            },
          } as AgentMessage);
        });
      },
    ...
    expect(sent.filter((entry) => entry.body.type === 'permission-request')).toHaveLength(0);
    expect(bridge.get(started.runId)).toEqual(expect.objectContaining({
      status: 'failed',
      error: {
        code: 'execution_run_failed',
        message: 'Execution-run permission request cannot be surfaced or denied',
      },
    }));
  });

  it('fails the run when fail-closed permission requests omit the provider request id', async () => {
    const sent: Array<{
      provider: string;
      body: AgentMessage;
      opts?: { meta?: Record<string, unknown> };
      ...
      async sendPrompt() {
        handlers.forEach((handler) => {
          handler({
            ...
            payload: {
              toolName: 'write',
              input: { path: '/tmp/fail-closed-missing-id.txt' },
            },
          } as AgentMessage);
        });
      },
  ...
  it('fails the run when fail-closed permission denial cannot be delivered to the runtime', async () => {
    const sent: Array<{
      provider: string;
      body: AgentMessage;
      opts?: { meta?: Record<string, unknown> };
    ...
    const runtime = {
      ...
      async sendPrompt() {
        handlers.forEach((handler) => {
          handler({
            ...
            payload: {
              toolName: 'write',
              input: { path: '/tmp/fail-closed-deny-rejection.txt' },
            },
          } as AgentMessage);
        });
      },
      ...
      async respondToPermission(requestId: string, approved: boolean) {
        calls.push(`respondToPermission:${requestId}:${approved}`);
        throw new Error('deny transport failed');
      },
      async dispose() {},
    };
    ...
    expect(calls).toContain('respondToPermission:provider-request-3:false');
    expect(sent.filter((entry) => entry.body.type === 'permission-request')).toHaveLength(0);
    expect(bridge.get(started.runId)).toEqual(expect.objectContaining({
      status: 'failed',
      error: {

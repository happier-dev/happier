...
const getExecutionRunBackendDescriptorMock = vi.fn(() => {
  throw new Error('legacy executionRunBackendRegistry must not be used for opencode once bindings exist');
});
...
describe('engineRegistry (opencode bindings)', () => {
  beforeEach(() => {
    getExecutionRunBackendDescriptorMock.mockClear();
  });

  it('creates an execution-run backend through engine bindings without consulting the legacy execution-run registry', async () => {

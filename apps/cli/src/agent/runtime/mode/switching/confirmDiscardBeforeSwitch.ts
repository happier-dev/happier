...
export async function confirmDiscardBeforeSwitchToTerminal(opts: {
    queuedCount: number;
    queuedPreview: readonly string[];
    pendingCount: number;
    ...
    const answer = await io.question('Discard these messages and switch to terminal mode? (y/N) ');
    const normalized = answer.trim().toLowerCase();
    return normalized === 'y' || normalized === 'yes';
}

export async function confirmDiscardQueuedMessagesForSwitchToTerminal(
    opts: ConfirmDiscardQueuedMessagesParams,
): Promise<boolean> {
    return confirmDiscardBeforeSwitchToTerminal({

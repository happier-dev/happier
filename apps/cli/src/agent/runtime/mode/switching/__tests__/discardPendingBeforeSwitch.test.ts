...
describe('discardPendingBeforeSwitchToTerminal', () => {
  ...
  it('returns failed and sends an error when discarding server pending messages fails', async () => {
    ...
    expect(sendStatusMessage).toHaveBeenCalledWith(
      'Failed to discard pending messages before switching to terminal mode: server boom',
    );
  });
  ...
  it('returns failed and sends an error when marking queued messages fails', async () => {
    ...
    expect(result).toBe('failed');
    expect(sendStatusMessage).toHaveBeenCalledWith(
      'Failed to mark queued messages as discarded before switching to terminal mode: queue boom',
    );
  });
  ...
  it('discards server and queued messages and emits a summary on success', async () => {
    ...
    expect(sendStatusMessage).toHaveBeenCalledWith(
      'Discarded 2 pending UI messages and 3 queued remote messages to switch to terminal mode. Please resend them from this terminal if needed.',
    );
  });
});

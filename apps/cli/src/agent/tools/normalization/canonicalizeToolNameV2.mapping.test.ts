...
describe('canonicalizeToolNameV2 mappings', () => {
  ...
  // in the SDK's generated tool schemas) that act on a headless background task, not on a subagent
  // roster entry. The `task*` prefix rule above must not swallow them.
  it.each([

export type DiscardPendingBeforeSwitchToTerminalResult = 'proceed' | 'cancelled' | 'failed';

export async function discardPendingBeforeSwitchToTerminal(opts: {
  queuedCount: number;
  ...
  onCancelled?: () => void;
}): Promise<DiscardPendingBeforeSwitchToTerminalResult> {
  ...
  try {
    ...
    opts.sendStatusMessage(
      `Failed to discard pending messages before switching to terminal mode: ${opts.formatError(error)}`,
    );
    return 'failed';
  }
  ...
  try {
    ...
    opts.sendStatusMessage(
      `Failed to mark queued messages as discarded before switching to terminal mode: ${opts.formatError(error)}`,
    );
    return 'failed';
  }
  ...
  if (parts.length > 0) {
    opts.sendStatusMessage(
      `Discarded ${parts.join(' and ')} to switch to terminal mode. Please resend them from this terminal if needed.`,
    );
  }

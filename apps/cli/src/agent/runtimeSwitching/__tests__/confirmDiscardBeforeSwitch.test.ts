...
import { confirmDiscardBeforeSwitchToTerminal } from '../confirmDiscardBeforeSwitch';
...
describe('confirmDiscardBeforeSwitchToTerminal', () => {
    it('does not prompt when there is nothing to discard in TTY mode', async () => {
        let prompted = 0;
        const ok = await confirmDiscardBeforeSwitchToTerminal({
            queuedCount: 0,
            queuedPreview: [],
            pendingCount: 0,
            pendingPreview: [],

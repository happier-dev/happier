...
import { discardQueuedAndPendingForTerminalSwitch } from '../discardQueuedAndPendingForSwitch';

describe('discardQueuedAndPendingForTerminalSwitch', () => {
  it('returns proceed without calling discard when queue and server are empty', async () => {
    ...
    const result = await discardQueuedAndPendingForTerminalSwitch({
      queue,

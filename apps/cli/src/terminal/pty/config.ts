...
export function readDaemonTerminalPtyConfig(env: NodeJS.ProcessEnv): DaemonTerminalPtyConfig {
  ...
  const bufferRetentionMs = parseIntEnv(env.HAPPIER_DAEMON_TERMINAL_BUFFER_RETENTION_MS, 10 * 60_000, {
    min: 1000,
    max: 24 * 60 * 60_000,
  });
  ...
  const urlDedupeLimit = parseIntEnv(env.HAPPIER_DAEMON_TERMINAL_URL_DEDUPE_LIMIT, 1024, {
    min: 1,
    max: 100_000,
  });

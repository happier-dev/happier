...
export const ACTION_SPEC_RPC_EXCEPTIONS = Object.freeze([
    ...
    ...A12_VOICE_TRANSPORT_EXCEPTIONS,
    {
        ...
        ownerPacket: 'ESR legacy direct-session link compatibility',
        rationale: 'The canonical link builder writes only the externalSessionV1 metadata envelope, which released direct-session clients cannot recognize, so this compatibility-only alias validates the shipped request shape and returns the released failure envelope before ActionSpec dispatch, Session creation, or any metadata write.',
        abiProof: 'ui-web-v0.2.0@dc5203145 and ui-web-v0.2.2-preview.1775585938.1@4913c1e5 read metadata.directSessionV1 (sessionStorageKind.ts, directSessions/readDirectSessionLink.ts); packages/protocol/src/sessions/external/linkedSessionMetadata.ts#buildLinkedExternalSessionMetadataV1 writes externalSessionV1 and deletes the released key, so a dispatched link would answer ok:true with an unopenable Session.',
        retirement: 'Remove this exception and wire alias when every supported UI/mobile/desktop client and rollback target no longer sends daemon.directSessions.link.ensure.',
    },
    {
        method: RPC_METHODS.DAEMON_DIRECT_SESSION_TAKEOVER_LEGACY,
        actionId: 'sessions.external.takeover',
        reason: 'custom_result_envelope',
        ownerPacket: 'ES-2026-07-31.14 A13 legacy takeover compatibility',
        rationale: 'Compatibility-only direct-session takeover validates the shipped request shape, then returns the released failure envelope with invalid_request/upgrade_required before ActionSpec dispatch or domain effects.',
        abiProof: 'Legacy direct-session takeover accepts { machineId, sessionId, forceStop? }; malformed requests return invalid_request, while valid unphased requests return invalid_request/upgrade_required and never map into ExternalSessionTakeoverInputV1.',
        retirement: 'Remove this exception and wire alias when every supported UI/mobile/desktop client and rollback target no longer sends daemon.directSessions.takeover.',
    },
    {
        ...
        reason: 'legacy_alias',
        ownerPacket: 'ES-2026-07-31.14 A13 legacy takeover compatibility',
        rationale: 'Compatibility-only direct-session takeoverPersist validates the shipped request shape, then returns the released failure envelope with invalid_request/upgrade_required before ActionSpec dispatch or domain effects.',
        abiProof: 'Legacy direct-session takeoverPersist accepts { machineId, sessionId, forceStop? }; malformed requests return invalid_request, while valid unphased requests return invalid_request/upgrade_required and never map into ExternalSessionTakeoverInputV1.',
        retirement: 'Remove this exception and wire alias when every supported UI/mobile/desktop client and rollback target no longer sends daemon.directSessions.takeoverPersist.',
    },
    {

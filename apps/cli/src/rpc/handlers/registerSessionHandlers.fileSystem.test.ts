...
describe('registerSessionHandlers (file system)', () => {
  it('keeps filesystem browsing, mutation, and bulk transfer handlers out of session scope', () => {
    ...
    expect(handlers.has(RPC_METHODS.DAEMON_BULK_TRANSFER_DOWNLOAD_INIT)).toBe(false);
    expect(handlers.has(RPC_METHODS.DAEMON_BULK_TRANSFER_DOWNLOAD_CHUNK)).toBe(false);
    expect(handlers.has(RPC_METHODS.DAEMON_BULK_TRANSFER_DOWNLOAD_FINALIZE)).toBe(false);
    expect(handlers.has(RPC_METHODS.DAEMON_BULK_TRANSFER_DOWNLOAD_ABORT)).toBe(false);

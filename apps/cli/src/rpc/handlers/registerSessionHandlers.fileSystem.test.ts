...
describe('registerSessionHandlers (file system)', () => {
  it('keeps filesystem browsing, mutation, and bulk transfer handlers out of session scope', () => {
    ...
    expect(handlers.has(RPC_METHODS.DAEMON_TRANSFER_DOWNLOAD_INIT)).toBe(false);
    expect(handlers.has(RPC_METHODS.DAEMON_TRANSFER_DOWNLOAD_CHUNK)).toBe(false);
    expect(handlers.has(RPC_METHODS.DAEMON_TRANSFER_DOWNLOAD_FINALIZE)).toBe(false);
    expect(handlers.has(RPC_METHODS.DAEMON_TRANSFER_DOWNLOAD_ABORT)).toBe(false);
    expect(handlers.has(['daemon.bulkTransfer.', 'upload.init'].join(''))).toBe(false);
    expect(handlers.has(['daemon.bulkTransfer.', 'upload.chunk'].join(''))).toBe(false);
    expect(handlers.has(['daemon.bulkTransfer.', 'upload.finalize'].join(''))).toBe(false);
    expect(handlers.has(['daemon.bulkTransfer.', 'download.init'].join(''))).toBe(false);
    expect(handlers.has(['daemon.bulkTransfer.', 'download.chunk'].join(''))).toBe(false);
    expect(handlers.has(['daemon.bulkTransfer.', 'download.finalize'].join(''))).toBe(false);

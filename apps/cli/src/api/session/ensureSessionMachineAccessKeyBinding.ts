...
export async function ensureSessionMachineAccessKeyBinding(params: Readonly<{
    ...
    const bindingPromise = ensureSessionMachineAccessKeyBindingUncached({
        serverUrl: params.serverUrl,
        token: params.token,
        sessionId: params.sessionId,
        machineId: params.machineId,
...
async function ensureSessionMachineAccessKeyBindingUncached(params: Readonly<{
    ...
    if (isAuthenticationStatus(existingStatus)) {
        throw createAuthenticationHttpStatusError(existingStatus, 'Authentication failed while binding session machine control');
    }
    ...
    if (isAuthenticationStatus(createdStatus)) {
        throw createAuthenticationHttpStatusError(createdStatus, 'Authentication failed while binding session machine control');
    }

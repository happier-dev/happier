...
describe('ApiMachineClient SCM handlers', () => {
  it('registers SCM RPCs as machine-scoped handlers', () => {
    ...
    expect(rpc.hasHandler(RPC_METHODS.SCM_PULL_REQUEST_CHECKOUT)).toBe(true);
    expect(rpc.hasHandler(RPC_METHODS.SCM_PULL_REQUEST_PREPARE_WORKTREE)).toBe(true);
    expect(rpc.hasHandler(RPC_METHODS.SCM_PULL_REQUEST_RUN_STACKED)).toBe(true);

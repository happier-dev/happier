type DirectSessionTranscriptReadAfterResult<TItem> = Readonly<{
  items: readonly TItem[];
  nextCursor?: string | null;
  truncated: boolean;
}>;

type DirectSessionPollingFollowLeaseParams<TItem> = Readonly<{
  readAfterTranscript: (params: Readonly<{
    cursor: string;
    maxBytes: number;
    maxItems: number;
  }>) => Promise<DirectSessionTranscriptReadAfterResult<TItem>>;
  env?: NodeJS.ProcessEnv;
}>;

type DirectSessionTranscriptUpdate<TItem> = Readonly<{
  items: TItem[];
  nextCursor: string | null;
  truncated: boolean;
}>;

type DirectSessionTranscriptUpdateListener<TItem> = (
  update: DirectSessionTranscriptUpdate<TItem>,
) => void | Promise<void>;

type PollingDirectSessionFollowLease<TItem> = Readonly<{
  release: () => Promise<void>;
  getTailCursor: () => string | null;
  subscribeToTranscriptUpdates: (listener: DirectSessionTranscriptUpdateListener<TItem>) => () => void;
}>;
...
async function notifyTranscriptListeners<TItem>(
  listeners: ReadonlySet<DirectSessionTranscriptUpdateListener<TItem>>,
  update: DirectSessionTranscriptUpdate<TItem>,
): Promise<void> {
...
export async function createPollingDirectSessionFollowLease<TItem>(
  params: DirectSessionPollingFollowLeaseParams<TItem>,
): Promise<PollingDirectSessionFollowLease<TItem>> {
  const env = params.env ?? process.env;
  const pollIntervalMs = resolvePollIntervalMs(env);
  const maxBytes = resolveMaxBytes(env);
  const maxItems = resolveMaxItems(env);
  const listeners = new Set<DirectSessionTranscriptUpdateListener<TItem>>();

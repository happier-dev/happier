...
These example packages document the **current** plugin/extension contract used by the CLI runtime.

All examples in this folder are **V2-only** (`schemaVersion: 2`). No V1/compat authoring fixtures remain.

Extension package topology is **domain-organized**:
...
```bash
happier plugins install /absolute/path/to/apps/cli/src/extensions/testkit/fixtures/authoring-examples/action-plugin

...
describe('marketplace source registry store', () => {
  ...
  it('boots a curated marketplace source when the file does not exist', async () => {
    ...
    expect(JSON.parse(readFileSync(join(happyHomeDir, 'extensions', 'plugins', 'state', 'marketplace-source-registry.v1.json'), 'utf8'))).toMatchObject({
      t: 'happier_marketplace_source_registry_v1',
      schemaVersion: 1,
      sources: [
        {
  ...
  it('persists and resolves marketplace sources by id and URL', async () => {
    ...
    const registryPath = join(happyHomeDir, 'extensions', 'plugins', 'state', 'marketplace-source-registry.v1.json');

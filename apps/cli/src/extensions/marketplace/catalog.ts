...
import { installPluginFromSource } from '../install/source';
import { fetchRemoteJsonWithLimits } from '../remote/fetch';
import { createPluginStateStore } from '../store/state';
import type { PluginCatalogContributionSummary } from '../catalog/summary';

const REMOTE_MARKETPLACE_CACHE_TTL_MS = 5 * 60 * 1000;

const MarketplaceCatalogEntrySchema = ExtensionMarketplaceEntryV1Schema;
...
function inferMarketplaceArchiveTrustPolicy(locator: string): ExtensionSourceSpecV1['trustPolicy'] {
  const parsedLocatorUrl = tryParseUrl(locator);
  if (parsedLocatorUrl) {
    if (parsedLocatorUrl.protocol === 'http:' || parsedLocatorUrl.protocol === 'https:') {
      return 'prompt';
...
function buildProtocolMarketplaceEntry(
  descriptor: z.infer<typeof ExtensionMarketplaceEntryV1Schema>,
  params?: Readonly<{ catalogSourceUrl?: string }>,
...
function readMarketplaceCatalogEntry(
  raw: z.infer<typeof ExtensionMarketplaceEntryV1Schema>,
  params?: Readonly<{ catalogSourceUrl?: string }>,
): MarketplaceCatalogEntry {
...
type MarketplaceCatalogDocumentV1 = Readonly<{
  ...
  title: string;
  description?: string;
  entries: readonly z.infer<typeof ExtensionMarketplaceEntryV1Schema>[];
}>;

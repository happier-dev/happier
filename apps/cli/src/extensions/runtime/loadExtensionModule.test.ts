...
import type { ExtensionModuleNamespace } from './loadExtensionModule';
...
describe('loadExtensionModule', () => {
    it('loads a trusted file-backed daemon entry and caches repeated loads by entry path + fingerprint', async () => {
        ...
        const first = await loadExtensionModule({
            source: { kind: 'file_backed', entryPath, trustPolicy: 'local_trusted' },
        });
        const second = await loadExtensionModule({
            source: { kind: 'file_backed', entryPath, trustPolicy: 'local_trusted' },
        });

        expect(typeof (first as ExtensionModuleNamespace).bindTranscript).toBe('function');
    ...
    it('fails clearly when a file-backed daemon entry path does not exist', async () => {
        ...
        await expect(loadExtensionModule({
            source: { kind: 'file_backed', entryPath, trustPolicy: 'local_trusted' },
    ...
    it('loads a bundled activation source via the provided loader and caches by moduleId + cacheKey', async () => {
        ...
        const source = {
            ...
            moduleId: '@happier-dev/extensions-acme/daemon:test-cache-1',
            load: async () => {
                loads += 1;
                return { version: 1 } as unknown as ExtensionModuleNamespace;
            },
        };

        const first = await loadExtensionModule({ source, cacheKey: 'gen:1' });
        const second = await loadExtensionModule({ source, cacheKey: 'gen:1' });
    ...
    it('invalidates bundled module caches when cacheKey changes', async () => {
        ...
        const source = {
            kind: 'bundled' as const,
            moduleId: '@happier-dev/extensions-acme/daemon:test-cache-2',
            load: async () => {
                loads += 1;
                return { version: loads } as unknown as ExtensionModuleNamespace;
            },
        };

        const first = await loadExtensionModule({ source, cacheKey: 'gen:1' });
        const second = await loadExtensionModule({ source, cacheKey: 'gen:2' });

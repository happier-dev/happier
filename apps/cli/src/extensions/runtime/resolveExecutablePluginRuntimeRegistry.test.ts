...
import { resolveBuiltInContributions } from '@/extensions/registry/resolveBuiltInContributions';
import { writePluginReloadStateSnapshot } from '@/extensions/reload/state';
...
describe('resolveExecutablePluginRuntimeRegistry', () => {
    it('activates the bundled opencode extension by default and exposes a backend engine registration', async () => {
        const contributions = createResolvedContributionRegistry(resolveBuiltInContributions());
    ...
    it('merges activation-time providers, backends, actions, resources, and UI descriptors into the authoritative contribution snapshot', async () => {
        ...
        const manifestPath = await writeActivationManifest(pluginRoot, {
            id: 'acme.activated',
            runtimeCapabilities: ['providers', 'backends', 'actions', 'resources', 'uiDescriptors', 'hooks'],
        ...
        await writeFile(
            ...
            [
                ...
                '    kindVersion: 1,',
                '    id: "acme.activated.provider",',
                '    providerAgentId: "claude",',
                '    display: {',
                '      name: "Activated Provider",',
                '      subtitle: "From activation",',
                '      tags: ["plugin"],',
                '    },',
                '    ownedBackendIds: ["acme.activated.backend"],',
                '  });',
                '  api.registerBackend({',
                '    kindVersion: 1,',
                '    id: "acme.activated.backend",',
                '    providerId: "acme.activated.provider",',
                '    runtimeKind: "native",',
                '    title: "Activated Backend",',
        ...
        expect(runtimeRegistry.contributions.providerDefinitionsById.get('acme.activated.provider')).toMatchObject({
            pluginId: 'acme.activated',
            provenance: 'external',
            source: { kind: 'path' },
            definition: {
        ...
        expect(runtimeRegistry.contributions.backendDefinitionsById.get('acme.activated.backend')).toMatchObject({
            pluginId: 'acme.activated',
            provenance: 'external',
            source: { kind: 'path' },
            definition: {
        ...
        expect(runtimeRegistry.contributions.actionsById?.get('acme.activated.action')).toMatchObject({
            pluginId: 'acme.activated',
            definition: {
                id: 'acme.activated.action',
                title: 'Activated Action',
        ...
        expect(runtimeRegistry.contributions.resourcesById?.get('acme.activated.prompt')).toMatchObject({
            pluginId: 'acme.activated',
            definition: {
                id: 'acme.activated.prompt',
                path: 'resources/prompt.md',
        ...
        expect(runtimeRegistry.contributions.uiDescriptorsById?.get('acme.activated.settings')).toMatchObject({
            pluginId: 'acme.activated',
            definition: {
                id: 'acme.activated.settings',
                surface: 'settings',
    ...
    it('loads the ui-descriptor authoring example through the activation-time runtime contract', async () => {
        ...
        expect(runtimeRegistry.contributions.resourcesById?.get('examples.ui.prompt')).toMatchObject({
            pluginId: 'examples.ui-descriptor-plugin',
            definition: {
        ...
        expect(runtimeRegistry.contributions.uiDescriptorsById?.get('examples.ui.settings')).toMatchObject({
            pluginId: 'examples.ui-descriptor-plugin',
            definition: {
    ...
    it('normalizes activation-time tools and commands through the authoritative runtime contribution snapshot', async () => {
        ...
        const projected = runtimeRegistry.contributions as typeof runtimeRegistry.contributions & Readonly<Record<string, unknown>>;

        expect(listBuiltInHappierTools({
            surface: 'cli',
            registry: runtimeRegistry.contributions,
        }).map((tool) => tool.name)).toContain('acme_activated_tool');
        expect(runtimeRegistry.contributions.actionsById?.get('acme.activated.tool')).toMatchObject({
            pluginId: 'acme.activated',
    ...
    it('reuses caller-provided contribution ingress when resolving executable runtime hooks', async () => {
        ...
        expect(reused.contributions).toBe(initial.contributions);

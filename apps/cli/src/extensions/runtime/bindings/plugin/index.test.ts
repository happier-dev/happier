...
import { createPluginBindings } from './index';
...
describe('createPluginBindings', () => {
  it('returns an explicit binding envelope instead of a bare runtime object', () => {
    const binding = createBindingFactory({
  ...
  it('rejects raw RuntimeTurnOperations launch results and requires an explicit runtime payload envelope', async () => {
    ...
    if (!('bindings' in binding)) {
      throw new Error('expected plugin bindings to return a bindings envelope');
    }

    const plan = await binding.bindings.createSessionRuntime({
      credentials,
  ...
  it('passes only the plugin session launch request to plugin terminal launch', async () => {
    ...
    const binding = createBindingFactory({ launch });

import type {
    ExtensionProjectionDiagnosticV2,
    ExtensionProjectionInstalledPackageV2,
    ExtensionProjectionV2,
} from '@happier-dev/protocol';
import { getExtensionHookDefinitionV1 } from '@happier-dev/protocol';

import type { PluginCatalogEntry } from '@/extensions/catalog/installed';
...
function resolveActionSurfaces(
    ...
    if (
        ...
        || surfaces.ui_slash_command === true
        || surfaces.voice_tool === true
        || surfaces.voice_action_block === true
    ) {
...
function resolveUiDescriptorSurface(
    surface: string,
): 'settings' | 'setup' | 'status' | 'providerSettings' | 'backendSettings' {
    switch (surface) {
        case 'setup':
            return 'setup';
        case 'status':
...
function pushPluginDiagnostics(
    bucket: Map<string, ExtensionProjectionDiagnosticV2[]>,
    pluginId: string,
...
function buildDiagnostics(params: Readonly<{
    ...
    const diagnosticsByPluginId = new Map<string, ExtensionProjectionDiagnosticV2[]>();
...
function buildInstalledPackagesById(params: Readonly<{
    ...
    installedPackages: readonly PluginCatalogEntry[];
    pluginDiagnosticsByPluginId: Readonly<Record<string, readonly PluginCompatibilityDiagnostic[]>>;
}>): ExtensionProjectionV2['installedPackagesById'] {
    const installedPackagesById: ExtensionProjectionV2['installedPackagesById'] = {};
...
function buildProvidersById(
    registry: ResolvedContributionRegistry,
): ExtensionProjectionV2['providersById'] {
    const providersById: ExtensionProjectionV2['providersById'] = {};
    for (const provider of registry.providers) {
...
function buildBackendsById(
    registry: ResolvedContributionRegistry,
): ExtensionProjectionV2['backendsById'] {
    const backendsById: ExtensionProjectionV2['backendsById'] = {};
    for (const backend of registry.backends) {
...
function buildHooksById(
    registry: ResolvedContributionRegistry,
): NonNullable<ExtensionProjectionV2['hooksById']> {
    const hooksById: NonNullable<ExtensionProjectionV2['hooksById']> = {};
    ...
    for (const hook of sortedHookRegistrations) {
        const hookDefinition = getExtensionHookDefinitionV1(hook.definition.id);
...
function buildActionsById(
    registry: ResolvedContributionRegistry,
): ExtensionProjectionV2['actionsById'] {
    const actionsById: ExtensionProjectionV2['actionsById'] = {};
    for (const action of registry.actions) {
        if (!action.pluginId) {
            continue;
        }
...
function buildToolsById(
    registry: ResolvedContributionRegistry,
): ExtensionProjectionV2['toolsById'] {
    const toolsById: ExtensionProjectionV2['toolsById'] = {};
    for (const tool of registry.tools ?? []) {
        if (!tool.pluginId) {
            continue;
        }
...
function buildCommandsById(
    registry: ResolvedContributionRegistry,
): ExtensionProjectionV2['commandsById'] {
    const commandsById: ExtensionProjectionV2['commandsById'] = {};
    for (const command of registry.commands ?? []) {
        if (!command.pluginId) {
            continue;
        }
...
function buildResourcesById(
    registry: ResolvedContributionRegistry,
): ExtensionProjectionV2['resourcesById'] {
    const resourcesById: ExtensionProjectionV2['resourcesById'] = {};
    for (const resource of registry.resources) {
...
function buildUiDescriptorsById(
    registry: ResolvedContributionRegistry,
): ExtensionProjectionV2['uiDescriptorsById'] {
    const uiDescriptorsById: ExtensionProjectionV2['uiDescriptorsById'] = {};
    for (const descriptor of registry.uiDescriptors) {
...
export function buildExtensionProjectionV2(params: Readonly<{
    registry: ResolvedContributionRegistry;
    generation: number;
    installedPackages?: readonly PluginCatalogEntry[];
    pluginDiagnosticsByPluginId?: Readonly<Record<string, readonly PluginCompatibilityDiagnostic[]>>;

...
function aggregateHookResults(
  ...
  aggregation: ExtensionHookAggregationKindV1 | null,
...
function shouldStopDispatchAfterOutcome(params: Readonly<{
  aggregation: ExtensionHookAggregationKindV1 | null;
  failureMode: ExtensionHookFailureModeV1 | null;
...
export async function dispatchPluginHookEvent(params: Readonly<{
  ...
  const hookDefinition = getExtensionHookDefinitionV1(envelope.eventId);

...
function collectRegistryPluginIds(registry: ResolvedExecutablePluginRuntimeRegistry): readonly string[] {
    ...
    for (const contribution of registry.contributions.providers) {
        if (contribution.pluginId) pluginIds.add(contribution.pluginId);
    }
    for (const contribution of registry.contributions.backends) {
        if (contribution.pluginId) pluginIds.add(contribution.pluginId);
    }
    for (const contribution of registry.contributions.actions) {
        if (contribution.pluginId) pluginIds.add(contribution.pluginId);
    }
    for (const contribution of registry.contributions.resources) {
        if (contribution.pluginId) pluginIds.add(contribution.pluginId);
    }
    for (const contribution of registry.contributions.uiDescriptors) {
        if (contribution.pluginId) pluginIds.add(contribution.pluginId);
    }
    for (const target of registry.contributions.activationTargets) {
        pluginIds.add(target.pluginId);
    }
    for (const contribution of registry.contributions.hookRegistrations) {
...
function resolveActiveGenerationId(
    ...
    return registry.contributions.generationId ?? `reload:${generation}`;
}

...
import { composeJsonRpcDiagnosticHooks, createExecClientRpcLogger } from './diagnostics/rpcLog';
import { spawnSupervisedPluginProcess } from './processSupervisor';
import type { PluginExecSystemToolDefinition } from './system/tools/definitions';
import {
    createPluginExecSystemToolGrantStore,
    type PluginExecSystemToolGrantStore,
} from './system/tools/grants';
import { createPluginExecSystemToolResolver } from './system/tools/resolveGrant';
import { isDeniedPathOnlyRuntimeName, normalizePathOnlyRuntimeName } from './system/tools/runtimeDeny';
...
function buildProviderCliProcessEnv(
    ...
    return buildScopedProcessEnv({
        baseEnv: process.env,
        explicitEnv: { ...(params.baseEnv ?? {}), ...(input.env ?? {}) },
        unsetEnvKeys: input.unsetEnvKeys,
    }) as Record<string, string>;
}

async function resolveAgentCliLaunch(
    input: Extract<ExecLaunchInputV1, { kind: 'agent-cli' }>,
    params: CreatePluginExecServiceParams,
    ...
    const launch = requireAgentCliLaunchSpec(input.agentId as CatalogAgentLookupId, {
        processEnv,
    });
    return {
...
async function resolveAllowedLaunch(
    ...
    const launch = input.kind === 'agent-cli'
        ? await resolveAgentCliLaunch(input, params)
    ...
    const activeSystemToolGrant = systemToolGrantStore.hasActivePathGrant(launch.executablePath);
    if (isDeniedPathOnlyRuntimeName(executableName)) {
        ...
        if (!explicitlyAllowed && !activeSystemToolGrant) {
            throw new PluginExecError(
                'PLUGIN_EXEC_PATH_ONLY_RUNTIME_DENIED',
    ...
    if (!explicitManifestGrant && !activeSystemToolGrant) {
        throw new PluginExecError(
            'PLUGIN_EXEC_PERMISSION_DENIED',
...
async function createSpawnedPluginProcess(
    ...
    const env = finalizeSessionChildEnvironment({
        environment: buildScopedProcessEnv({
            ...
            unsetEnvKeys: resolvedLaunch.unsetEnvKeys,
        }),
    ...
    const supervised = spawnSupervisedPluginProcess({
        ...
        env,
        stdin: resolvedLaunch.stdin === undefined
            ? undefined
            : typeof resolvedLaunch.stdin === 'string'
                ? new Uint8Array(Buffer.from(resolvedLaunch.stdin, 'utf8'))
                : resolvedLaunch.stdin,
        timeoutMs: options?.timeoutMs,
        signals: [options?.signal, params.signal].filter((signal): signal is AbortSignal => signal !== undefined),
    ...
    const outputSubscription = outputTee
        ? supervised.handle.onOutput((chunk) => outputTee.onChunk(chunk.stream, chunk.data))
        : null;
    const exit = supervised.handle.wait().then((result): ExecRunResultV1 => {
        if (result.termination.observed.kind === 'failed') {
            throw new Error(result.termination.observed.diagnostic.message ?? result.termination.observed.diagnostic.code);
        }
        return Object.freeze({
            exitCode: result.termination.observed.kind === 'exit' ? result.termination.observed.exitCode : null,
            signal: result.termination.observed.kind === 'signal' ? result.termination.observed.signal : null,
    ...
    const handle: ExecProcessHandleV1 = Object.freeze({
        ...
        async writeStdin(inputChunk) {
            await supervised.handle.write(typeof inputChunk === 'string'
                ? new Uint8Array(Buffer.from(inputChunk, 'utf8'))
                : inputChunk);
        },
        kill() {
    ...
    return {
        child,
        handle,
        readStderrPreview: () => sanitizeExecDiagnosticText(
...
export function createPluginExecService(params: CreatePluginExecServiceParams = {}): ExecRuntimeServiceV1 {
    ...
    async function spawnClient(
        ...
        let terminalExitResult: ExecRunResultV1 | null = null;
        const publishExit = (result: ExecRunResultV1): void => {
            terminalExitResult ??= result;
            for (const listener of [...exitListeners]) {
                try {
                    listener(terminalExitResult);
                } catch {
                    // One lifecycle observer cannot suppress later observers or process cleanup.
                }
            }
        };
        ...
        const handle: ExecClientHandleV1<ExecProtocolClientV1> = Object.freeze({
            ...
            onExit(listener) {
                if (terminalExitResult) {
                    try {
                        listener(terminalExitResult);
                    } catch {
                        // Late observers have the same isolation guarantee as live observers.
                    }
                    return () => undefined;
                }

...
export async function reconcileReactNativeCrashStateBindings(input: Readonly<{
    ...
    const state = await input.store.update((current) => {
        ...
        for (const binding of bindings) {
            ...
            // The durable slot is keyed by the canonical Protocol mount key,
            // which carries every immutable generation, so each exact
            // mount/generation binding owns its own counts, disablement, and
            // epoch. A generation change therefore opens a fresh clean slot;
            // the predecessor's exact token keeps fencing only its own
            // historical record and never reaches the successor's slot.
            if (

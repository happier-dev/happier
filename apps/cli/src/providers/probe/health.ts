...
export function providerProbeFailureHealthState(
  ...
  timing: Readonly<{ observedAt: number; retryAt?: number }>,
  ...
  switch (error.code) {
    case 'provider_endpoint_unreachable':
      return { ...common, status: 'unreachable', errorCode: error.code, ...(timing.retryAt === undefined ? {} : { retryAt: timing.retryAt }) };
    case 'provider_endpoint_unavailable':
      return { ...common, status: 'temporarily_unavailable', errorCode: error.code, ...(timing.retryAt === undefined ? {} : { retryAt: timing.retryAt }) };
    case 'provider_endpoint_rate_limited':
      return { ...common, status: 'rate_limited', errorCode: error.code, ...(timing.retryAt === undefined ? {} : { retryAt: timing.retryAt }) };

 ...
 * transcripts and lifecycle rows as `event`
 * (`packages/protocol/src/sessions/messages/sessionMessageRole.ts`), and none of
 * those is a conversational turn. Asking the server for the other two is what
 * turns a fixed page of rows into a page of CONVERSATION: the observed
 * unfiltered window spent 500 rows to yield 165 lines and zero user turns.
 *
...
export async function hydrateReplayDialogFromForkChain(params: Readonly<{
   ...
   *
   * The framer's omission line counts items it was GIVEN and dropped; rows this
   ...
   *
   * This owner used to ask for `dataKey` material only, so a legacy-secret
   * home marked every e2ee segment unavailable and the whole hydration
   * answered `null`. The same-Session Agent transition asks this AFTER it has
  ...
  if (wantSynopsisText && startingCtx !== 'unavailable') {
  ...
  const accepted: Array<Readonly<{
    dialog: HappierReplayDialogItem & { role: 'User' | 'Assistant' };
    referencedSessionMediaWorkspacePaths: readonly string[];
  }>> = [];
  let usedChars = 0;
  ...
  walk: for (const segment of segments) {
    ...
    while (!segmentExhausted) {
       ...
       */
      const pageDialog = slice.dialog.map((item, index) => ({
        dialog: { ...item, sessionId: segment.sessionId } as HappierReplayDialogItem & { role: 'User' | 'Assistant' },
        referencedSessionMediaWorkspacePaths: slice.dialogReferencedSessionMediaWorkspacePaths[index] ?? [],
      }));

      let budgetMet = false;
      for (let index = pageDialog.length - 1; index >= 0; index -= 1) {
        const candidate = pageDialog[index]!;
        const item = candidate.dialog;
        ...
        const previouslyAccepted = accepted[accepted.length - 1]?.dialog;
        ...
        for (const path of candidate.referencedSessionMediaWorkspacePaths) {
          referencedSessionMediaWorkspacePaths.add(path);
        }
        // The newest turn alone can exceed the whole budget. Keeping it is
        // right — the framer truncates its TEXT with a marker — but nothing
        // after it can fit, so the walk stops instead of paging for room that
        // does not exist.
  ...
  if (!lastUserDialogItem && startingCtx !== 'unavailable') {
   ...
   */
  const dialog = [...accepted]
    .reverse()
    .sort((a, b) => a.dialog.createdAt - b.dialog.createdAt)

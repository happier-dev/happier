...
export async function resolveReplaySeedDraft(params: Readonly<{
  ...
  maxTextChars?: number;
  /**
   * The departing Agent's work state, for the same-Session Agent transition
   * whose cutover clears the field (section 8). A replay-seeded NEW Session has
   * no departing Agent and passes nothing. The framing owner beneath bounds and
   * escapes it inside the same total cap as the transcript.
   */
  ...
  const seedDraft = buildHappierReplayPromptFromDialog({
    ...
    // history the seed does not carry — and the framer can only count the items
    // it was GIVEN, so it cannot see that loss unless it is told.

...
function hasCompletePersistedTakeoverAttemptIdentity(
  record: ExternalSessionOperationRecordV1,
): boolean {
...
function resolvePassiveRepairMode(
  ...
  if (record.phase === 'spawning') {
    return hasCompletePersistedTakeoverAttemptIdentity(record)
  ...
  if (
    !hasCompletePersistedTakeoverAttemptIdentity(record)
...
export async function repairExternalSessionOperationProgressProjections(
  ...
  const selected =
    selectExternalSessionOperationRecordsForPassiveRepair(records);
  ...
  for (const selectedRecord of selected) {
    ...
    if (record && repairMode) {
      const repaired = await mutateExternalSessionOperationRecordAtRevision(
        activeServerDir,
        record.operationId,
        record.revision,
        (current) => {
          ...
          if (repairMode === 'hosted_offline') {
            return {
              ...
              error: {
                code: 'spawn_failed',
                message:
                  'Persisted takeover runtime was interrupted before runtime binding completed.',
                retryable: true,
                occurredAtMs: nowMs(),
              },
            };
      ...
      if (repaired.ok) {
        ...
        if (!latest) {
          ...
          // A current-revision action won the race. Publish only those newer
          // durable bytes; the metadata binding rejects revision regression.
          record = latest;
        }
      }
    }
...
export function selectExternalSessionOperationProgressMetadata(
  ...
  const parsedCurrent = ExternalSessionOperationStateV1Schema.safeParse(
    metadata.externalSessionOperationV1,
  );
  const current = parsedCurrent.success ? parsedCurrent.data.progress : null;
  let selectedMetadata: SessionMetadata = metadata;
  if (current && current.operationId !== progress.operationId) {
    if (
      !REPLACEABLE_TERMINAL_STATUSES.has(current.status)
...
export function selectExternalSessionOperationPresentationMetadata(
  ...
  const parsedOwner = ExternalSessionOperationStateV1Schema.safeParse(
  ...
  const publishUnderAdmissionLock = async () => {
    ...
    while (true) {
      ...
      await updateSessionMetadataWithRetry({
        ...
        rawSession,
        updater: (metadata) =>
          snapshot.metadataLayoutVersion === 0
            ? selectExternalSessionOperationPresentationMetadata(
                metadata,
                selectedProgress,
                {
      ...
      return await acknowledgeExternalSessionOperationProgressProjection({

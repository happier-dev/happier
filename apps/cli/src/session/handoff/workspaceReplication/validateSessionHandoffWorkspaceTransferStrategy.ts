...
function readIgnoredIncludeGlobs(workspaceTransfer: SessionHandoffWorkspaceTransferInput | undefined): readonly string[] {
  const value = workspaceTransfer?.ignoredIncludeGlobs;
...
export function validateSessionHandoffWorkspaceTransferStrategy(params: Readonly<{
  ...
  if (
    ...
    && readIgnoredIncludeGlobs(params.workspaceTransfer).length > 0
  ) {
    return {
      ok: false,
      errorCode: 'unsupported_workspace_transfer_strategy',

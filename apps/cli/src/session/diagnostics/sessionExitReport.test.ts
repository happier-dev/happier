...
describe('writeSessionExitReport', () => {
  ...
  it('preserves runner termination metadata when daemon later records clean process exit', async () => {
    const { writeSessionExitReport, writeSessionExitReportSync } = await import('./sessionExitReport');
    const dir = await mkdtemp(join(tmpdir(), 'happy-exit-report-merge-'));
    ...
    const outPath = writeSessionExitReportSync({
      ...
      report: {
        ...
        terminationRequestedAt: 10,
      },
    });

    await writeSessionExitReport({
      baseDir: dir,
      sessionId: 'sess_signal',
      pid: 999,
      report: {

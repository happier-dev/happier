...
import type {
  CanResumeFromMaterializedStateInput,
  CanResumeFromMaterializedStateResult,
} from '@/daemon/connectedServices/stateSharing/canResumeFromMaterializedState';

export type MaterializedStateReachabilityInput = CanResumeFromMaterializedStateInput;
export type MaterializedStateReachabilityVerifier = (
  input: MaterializedStateReachabilityInput,
...
export function createMaterializedStateReachabilityDelegate(
  verifyMaterializedState: MaterializedStateReachabilityVerifier,
): HostRuntimeControlReachabilityDelegateV1 {
  return {
    verifyMaterializedState: async (input) => {
      ...
      return {
        ok: true,
        value: await verifyMaterializedState(reachabilityInput),
      };
    },
  };
}

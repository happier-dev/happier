...
import { buildBoundedActivationBrief } from './buildSessionAgentTransitionActivationBrief';
...
// brief builder still stops at the canonical Replay seed owner, so what it
 ...
 * cutoff the divider recorded. A preview composed separately could show a brief
 * the target Agent was never sent, and a surface whose whole claim is "this is
 * what it received" cannot be free to disagree with the thing that sent it.
 */
...
function buildDeps(overrides?: Partial<Parameters<typeof previewSessionAgentTransitionBrief>[0]['deps']>) {
    return {
        ...
        decryptOwnerMetadataView: vi.fn(() => ({ path: '/w' })) as never,
        buildActivationBrief: vi.fn(() => ({
            status: 'available' as const,
            seed: {
                v: 1 as const,
                seedText: 'REBUILT SEED',
                sourceSessionId: 'sess_resolved',
...
describe('previewSessionAgentTransitionBrief', () => {
    it('runs the transition’s own brief builder, bounded by the divider cutoff', async () => {
        ...
        // reader is the Agent that arrived, not whichever runs the Session
        // today. Reading head instead would replay rows the target never saw;
        // naming today's Agent would address the brief to the wrong reader.
        expect(deps.buildActivationBrief).toHaveBeenCalledWith(
    ...
    it('never reports an unreadable source as empty', async () => {
        ...
        // Collapsing them tells the reader nothing crossed a boundary that a
        // whole conversation may have crossed.
        const preview = await previewSessionAgentTransitionBrief({
            credentials: CREDENTIALS,
            request: REQUEST,
            deps: buildDeps({
    ...
    it('does not build a brief for a Session this machine cannot address', async () => {
        const buildActivationBrief = vi.fn(() => ({ status: 'available' as const, seed: null }));
     ...
     * live state — or, after a switch back, that Agent's newer native session —
     * as though it had crossed the boundary. Nothing records them per boundary
     * (the divider carries only the cutoff and the Agent pair), so the honest
     * rebuild omits them and the card says so.
     */
    it('rebuilds without the departing Agent’s current projections, which no longer belong to the boundary', async () => {
        ...
        const preview = await previewSessionAgentTransitionBrief({
            ...
            request: REQUEST,
            deps: buildDeps({
                // Today's view: the Session has been running since, and whatever
                // Agent runs it now has republished both keys.
     ...
     * pass, because "the preview forwards a field" is not the contract; "the
     * pass runs against the same lower bound the transition used" is.
     */
    it('rebuilds a native return against the away-delta the boundary actually sent', async () => {
        ...
        const preview = await previewSessionAgentTransitionBrief({
            ...
            deps: buildDeps({ buildActivationBrief: buildBoundedActivationBrief }),
        });
    ...
    /**
     * Discriminating control: a fresh target's boundary had no lower bound at
     * all, so none may be invented. The key stays ABSENT rather than becoming a
    ...
    it('survives a throwing brief builder without claiming an empty source', async () => {
        const preview = await previewSessionAgentTransitionBrief({
            credentials: CREDENTIALS,
            request: REQUEST,
            deps: buildDeps({
                buildActivationBrief: vi.fn(() => { throw new Error('boom'); }),
            }),
        });

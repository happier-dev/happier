...
export async function awaitSpawnedSessionId(params: Readonly<{
  ...
  if (result.sessionId) return { type: 'success', sessionId: result.sessionId };
  ...
  switch (settled.status) {
    case 'success': return { type: 'success', sessionId: settled.sessionId };
...
export async function abandonSpawnedSessionUntilCompleted(params: Readonly<{
  ...
  if (resolution.status !== 'success') return { status: resolution.status };
  try {

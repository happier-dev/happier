...
} from '@/agent/acp/catalog/configured/resolveConfiguredAcpBackendFromAccountSettings';
...
function readConfiguredAcpVendorSessionId(metadata: Record<string, unknown>, backendId: string): string | null {
  ...
  if (!isLegacyCustomAcpId(providerId) && providerId !== `acp:${backendId}`) {
    return null;
  }
...
export async function resolveSessionForkBackendTarget(params: Readonly<{
  ...
  if (candidateConfiguredBackendId) {
    ...
    const resolvedConfiguredBackend = metadataConfiguredBackend
      ? null
      : await resolveConfiguredAcpBackendFromAccountSettingsOrPlugins({
    ...
    if (metadataConfiguredBackend || resolvedConfiguredBackend) {
      if (candidateConfiguredBackendId === 'customAcp') {
        return {
          ok: false,
      ...
      const backendTargetRefs = resolveConcreteBackendTargetRefs(backendTarget);
  ...
  return {
    ...
    backendTargetV2: resolveConcreteBackendTargetRefs({ kind: 'builtInAgent', agentId: agentRaw })!.backendTargetV2,

...
} from '@/workspaces/replication/state/workspaceReplicationJobLease';
import { startWorkspaceReplicationJobLeaseHeartbeat } from '@/workspaces/replication/state/workspaceReplicationJobLeaseHeartbeat';
import {
...
} from '@/workspaces/replication/state/workspaceReplicationScopeLease';
import { startWorkspaceReplicationScopeLeaseHeartbeat } from '@/workspaces/replication/state/workspaceReplicationScopeLeaseHeartbeat';
import {
  abortIfCancellationRequested,
  abortJobAndReturn,
  abortRecord,
  isCancelRequestedError,
  markJobFailedAndRethrow,
  resolveNowMs,
} from './aborts';
import type { ExecuteWorkspaceReplicationJobParams } from './types';
import { runWithWorkspaceReplicationJobStatusHeartbeat } from './statusHeartbeat';
...
export async function executeWorkspaceReplicationJob(
  params: ExecuteWorkspaceReplicationJobParams,
  ...
  if (!isCheckpointAtOrAfter(latest.status.checkpoint, 'apply_completed')) {
    ...
    try {
      applyResult = await runWithWorkspaceReplicationJobStatusHeartbeat({
        heartbeatIntervalMs: resolveWorkspaceReplicationJobStatusHeartbeatIntervalMs(),
        jobStore: params.jobStore,
        jobId: params.jobId,
        now: params.now,
        run: async () =>
          await params.applyPlan({
            job: latest,
            offer,
          }).catch(async (error: unknown) => {
            if (isCancelRequestedError(error)) {

...
import {
    buildScmWorkspaceIntegrationWorkspaceExportArtifactsWithBlobProviderFromTransferEntries,
...
describe('workspaceIntegration workspace export artifacts', () => {
    afterEach(async () => {
        await Promise.all(tempRoots.splice(0, tempRoots.length).map((dir) => rm(dir, { recursive: true, force: true })));
    });

    it('builds SCM export artifacts from workspace-integration transfer entries', async () => {
        ...
        const { workspaceExportArtifacts: artifacts, blobProvider } = await buildScmWorkspaceIntegrationWorkspaceExportArtifactsWithBlobProviderFromTransferEntries({
            entries: [
    ...
    it('skips unreadable transfer entries when the caller marks the access error as ignorable', async () => {
        ...
        try {
            const { workspaceExportArtifacts: artifacts } = await buildScmWorkspaceIntegrationWorkspaceExportArtifactsWithBlobProviderFromTransferEntries({
                entries: [
                    { relativePath: 'README.md', sourcePath: join(root, 'README.md') },
    ...
    it('clones the manifest and omits null workspace-integration metadata for manifest-only inputs', () => {
        ...
        const artifacts = createScmWorkspaceIntegrationWorkspaceExportArtifacts({
        ...
        expect(artifacts).not.toHaveProperty('workspaceIntegrationMetadata');
    });
});

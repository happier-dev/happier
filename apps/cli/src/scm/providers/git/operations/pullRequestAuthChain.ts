...
export function matchesBranchHeadContext(input: Readonly<{
    ...
    const expectedHeadRepository = input.provider.nameWithOwner?.trim();
    const actualHeadRepository = readHeadRepositoryNameWithOwner(input.pullRequest);
    if (actualHeadRepository && expectedHeadRepository && actualHeadRepository !== expectedHeadRepository) {
        return false;
    }
    return true;
}

...
import { createScmHostingProviderRegistry } from '../hostingProviders/registry';
...
function normalizeUrlSafety(
    provider: NonNullable<ResolvedExecutablePluginRuntimeRegistry['contributes']['scmHostingProviders']>[number]['definition'],
...
export function createPullRequestStatusProjectionRegistry(
    runtimeRegistry: Pick<ResolvedExecutablePluginRuntimeRegistry, 'contributes' | 'scmHostingProvidersById'>,
): ResolvedScmHostingProviderRegistry {
...
export async function resolveDefaultPullRequestStatusProjectionRegistry(): Promise<ResolvedScmHostingProviderRegistry> {
    defaultRegistryPromise ??= resolveExecutablePluginRuntimeRegistry()
        .then((runtimeRegistry) => createPullRequestStatusProjectionRegistry(runtimeRegistry));
...
export function projectPullRequestStatus(input: Readonly<{
    ...
    const baseBranch = normalizeUpstreamBranch(snapshot.branch.upstream);

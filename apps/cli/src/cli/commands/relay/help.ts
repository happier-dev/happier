...
export function showRelayHelp(): void {
  console.log(renderHelpPage({
    ...
    usage: [
      ...
      { label: 'happier relay access <status|configure|disable> [--ssh <user@host>] [--ssh-auth agent|keyfile|password] [--identity-file <path>] [--ssh-config-file <path>] [--upstream-url <url>] [--provider <provider-id>] [--url <url>] [--hostname <hostname>] [--token <token>] [--yes] [--json]', description: 'Manage relay access method config (share URL strategy)' },
    ],
    notes: [

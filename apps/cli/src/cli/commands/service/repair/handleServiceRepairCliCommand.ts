...
function buildDoctorRepairJsonSnapshot(snapshot: DoctorSnapshot | null): Readonly<{
  ...
  return {
    ...
    relays: snapshot?.localRelays?.relays
      ?? (snapshot as { relays?: { happier?: { relays?: readonly unknown[] } } } | null)?.relays?.happier?.relays
      ?? [],
...
export async function handleServiceRepairCliCommand(params: Readonly<{
  ...
  assertDaemonServiceModeSupported(runtimePreview.platform, parsed.mode);
  if (parsed.modeExplicit && parsed.mode === 'system' && runtimePreview.platform === 'linux' && runtimePreview.uid !== 0) {

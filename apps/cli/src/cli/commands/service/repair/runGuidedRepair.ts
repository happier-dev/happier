...
function buildChannelSwitchUninstallArgs(
  startup: NonNullable<Extract<RepairFinding, { kind: 'channel_switch_recommended' }>['fromStack']['automaticStartup']>,
): readonly string[] {
  const args = ['service', 'uninstall'] as string[];
  if (startup.mode === 'system') {
    args.push('--mode', 'system');
  }
  if (startup.releaseChannel) {
    args.push('--ring', String(startup.releaseChannel));
  }
  if (startup.serverId) {
    args.push('--instance', String(startup.serverId));
  }
  args.push('--yes');
  return args;
}
  ...
  switch (choice) {
    ...
    case 'replace': {
      ...
      if (finding.fromStack.automaticStartup) {
        runCliCommand(buildChannelSwitchUninstallArgs(finding.fromStack.automaticStartup));
      }
      const started = runCliCommand(['daemon', 'start']);
      return started;
    }

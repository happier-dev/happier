...
const {
  ...
  readDoctorRuntimeInventoryMock: vi.fn(async () => ({
    settings: {
      activeServerId: 'cloud',
      servers: {},
      knownAccountIds: ['acct_123'],
...
describe('handleServiceRepairCliCommand', () => {
  ...
  afterEach(() => {
    ...
    readDoctorRuntimeInventoryMock.mockClear();
  ...
  it('rejects doctor repair --server --yes when the target server is not the active runtime server', async () => {
    await withTempDir('happier-service-repair-server-execute-scope-', async (homeDir) => {
      envScope.patch({
        HAPPIER_HOME_DIR: homeDir,
        HAPPIER_ACTIVE_SERVER_ID: 'cloud',
        HAPPIER_PUBLIC_RELEASE_CHANNEL: 'stable',
      ...
      await expect(handleServiceRepairCliCommand({
        argv: ['repair', '--server', 'https://other.example.test', '--yes', '--json'],
        commandPath: 'happier doctor',
      })).rejects.toThrow('Target-scoped repair execution is only supported for the active server profile');

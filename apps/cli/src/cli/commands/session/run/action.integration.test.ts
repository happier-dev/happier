...
describe('happier session run action (integration)', () => {
  ...
  beforeEach(async () => {
    happyHomeDir = await mkdtemp(join(tmpdir(), 'happier-cli-session-run-action-'));
    ...
    mockIo.mockImplementation(() => {
      ...
      const emit = vi.fn((event: string, data: any, cb?: (...args: any[]) => void) => {
        ...
        expect(decrypted).toMatchObject({ runId: 'run_1', actionId: 'review.triage', input: { accept: ['f1'] } });
        cb?.({
          ok: true,
          result: encodeBase64Rpc(encrypt(dek, 'dataKey', { ok: true, updatedToolResult: { ok: true } }), 'base64'),
        });
      });

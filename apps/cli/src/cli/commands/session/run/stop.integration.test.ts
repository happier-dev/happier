...
describe('happier session run stop (integration)', () => {
  ...
  beforeEach(async () => {
    happyHomeDir = await mkdtemp(join(tmpdir(), 'happier-cli-session-run-stop-'));

    const sessionId = 'sess_integration_run_stop_123';
    ...
    mockIo.mockImplementation(() => {
      ...
      const emit = vi.fn((event: string, data: any, cb?: (...args: any[]) => void) => {
        ...
        expect(String(data.method ?? '')).toBe(`${sessionId}:${SESSION_RPC_METHODS.EXECUTION_RUN_STOP}`);
  ...
  it('returns a session_run_stop JSON envelope', async () => {
    ...
    try {
      const machineKeySeed = new Uint8Array(32).fill(8);
      await handleSessionCommand(['run', 'stop', 'sess_integration_run_stop_123', 'run_1', '--json'], {
      ...
      expect(parsed.kind).toBe('session_run_stop');
      expect(parsed.data?.sessionId).toBe('sess_integration_run_stop_123');

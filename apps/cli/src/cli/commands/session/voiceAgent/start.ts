...
export async function cmdSessionVoiceAgentStart(
  ...
  if (!credentials) {
    if (json) {
      printJsonEnvelope({ ok: false, kind: 'session_voice_agent_start', error: { code: 'not_authenticated' } });
      return;
    }
  ...
  if (!rawSession) {
    if (json) {
      printJsonEnvelope({ ok: false, kind: 'session_voice_agent_start', error: { code: 'session_not_found', sessionId } });
      return;
    }
  ...
  if (!started.ok) {
    if (json) {
      printJsonEnvelope({ ok: false, kind: 'session_voice_agent_start', error: { code: started.errorCode } });
      return;
    }

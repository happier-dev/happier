...
if (!modulePath) {
  ...
  try {
    ...
    // An ESM specifier is a URL, not a filesystem path: a Windows path parses as a
    // `c:` scheme and a `#` in any path segment starts a fragment. Both discard the
    // real module location, so the argv path is converted here.
    const imported = await tsImport(pathToFileURL(modulePath).href, import.meta.url);

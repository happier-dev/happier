 ...
 */
function verifyArchiveChecksum(archivePath, manifestPath = path.join(path.dirname(archivePath), 'checksums.sha256')) {
    const archiveName = path.basename(archivePath);
    const checksums = readChecksumManifest(manifestPath);
...
async function unpackTools(options = {}) {
    try {
        ...
        const checksums = readChecksumManifest(path.join(archivesDir, 'checksums.sha256'));
        for (const entry of entries) {

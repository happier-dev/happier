...
- **Do not validate the repo root / stack package by default**.
  - In multi-session development, the repo root worktree may be `gitDirty` with unrelated, in-flight work.
  - Treat root `git status` / `git diff` as **non-blocking context** unless the task explicitly targets the stack package/root.
...
When reviewing code changes, do **not** rely on `git diff` from the repo root.
...
- **What `command-task-diff.txt` represents**:
  ...
  - The diff base is the stack’s configured remote **default branch** (derived from `HAPPIER_STACK_STACK_REMOTE` and that remote’s `HEAD`/default branch).
...
This ensures validators review the change set **as a PR diff on top of the targeted base branch**, rather than whatever happens to be dirty in the repo root.

---

## Git command ban for validators (MANDATORY)

- **Do not run `git status` / `git diff` / `git log` / `git range-diff`** during validation in this repo.

...
## hstack Overlay (MANDATORY)

- Do **not** start servers manually (`yarn dev`, `expo`, etc.). The stack lifecycle must be managed by Edison’s `web_server` integration, which should run via `hstack`.
- Always run validation stack-scoped via the wrapper:
  - `hstack edison --stack=<stack> -- qa validate <task-id> --execute`
...
- Instead, always log in to the seeded dev account:
  ...
  - **Preferred (avoid leaking the key into logs)**:
    - copy the restore key to clipboard:
      - `hstack auth dev-key --print | pbcopy`

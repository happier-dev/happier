...
test('monorepo port --skip-applied skips patches that are already present in the target', async (t) => {
  ...
  await writeFile(join(target, 'apps', 'ui', 'package.json'), '{}\n', 'utf-8');
  await writeFile(join(target, 'apps', 'cli', 'package.json'), '{}\n', 'utf-8');
  await writeFile(join(target, 'apps', 'cli', 'hello.txt'), 'v2\n', 'utf-8');
  await writeFile(join(target, 'apps', 'server', 'package.json'), '{}\n', 'utf-8');
  await run('git', ['add', '.'], { cwd: target, env: gitEnv() });
  await run('git', ['commit', '-q', '-m', 'chore: init monorepo'], { cwd: target, env: gitEnv() });
...
test('monorepo port accepts monorepo sources without double-prefixing paths', async (t) => {
  ...
  await writeFile(join(target, 'apps', 'cli', 'package.json'), '{}\n', 'utf-8');
  await writeFile(join(target, 'apps', 'server', 'package.json'), '{}\n', 'utf-8');
  await writeFile(join(target, 'apps', 'ui', 'hello.txt'), 'v1\n', 'utf-8');
  await run('git', ['add', '.'], { cwd: target, env: gitEnv() });
  await run('git', ['commit', '-q', '-m', 'chore: init monorepo'], { cwd: target, env: gitEnv() });
  ...
  await mkdir(join(source, 'apps', 'ui'), { recursive: true });
  await mkdir(join(source, 'apps', 'cli'), { recursive: true });
  await mkdir(join(source, 'apps', 'server'), { recursive: true });
  await writeFile(join(source, 'apps', 'ui', 'package.json'), '{}\n', 'utf-8');
  await writeFile(join(source, 'apps', 'cli', 'package.json'), '{}\n', 'utf-8');
  await writeFile(join(source, 'apps', 'server', 'package.json'), '{}\n', 'utf-8');
  await writeFile(join(source, 'apps', 'ui', 'hello.txt'), 'v1\n', 'utf-8');

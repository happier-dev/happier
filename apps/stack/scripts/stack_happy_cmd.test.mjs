...
test('hstack stack happier <name> runs CLI under that stack env', async () => {
  ...
  const res = await runNode([join(rootDir, 'bin', 'hstack.mjs'), 'stack', 'happier', stackName], { cwd: rootDir, env: baseEnv });
...
test('hstack stack happier <name> --identity=<name> uses identity-scoped HAPPIER_HOME_DIR', async () => {
  ...
  const res = await runNode(
    [join(rootDir, 'bin', 'hstack.mjs'), 'stack', 'happier', stackName, `--identity=${identity}`],
...
test('hstack <stack> happier ... shorthand runs CLI under that stack env', async () => {
  const scriptsDir = dirname(fileURLToPath(import.meta.url));
  const rootDir = dirname(scriptsDir);
  ...
  const res = await runNode([join(rootDir, 'bin', 'hstack.mjs'), stackName, 'happier'], { cwd: rootDir, env: baseEnv });
  assert.equal(res.code, 0, `expected exit 0, got ${res.code}\nstdout:\n${res.stdout}\nstderr:\n${res.stderr}`);
...
test('hstack stack happier <name> does not print wrapper stack traces on CLI failure', async () => {
  const scriptsDir = dirname(fileURLToPath(import.meta.url));
  const rootDir = dirname(scriptsDir);
  ...
  const res = await runNode([join(rootDir, 'bin', 'hstack.mjs'), 'stack', 'happier', stackName, 'attach', 'abc'], {
    cwd: rootDir,
    env: baseEnv,
  });

...
import { ensureReactNativeLibsodiumNativeBuild } from './utils/expo/libsodiumNativeBuild.mjs';
import { ensureReactNativeSkiaIosBinaries } from './utils/expo/skiaPrebuiltBinaries.mjs';
...
async function main() {
  ...
  if (wantsHelp(argv, { flags })) {
    printResult({
      ...
      text: [
        ...
        '- If you explicitly set HAPPIER_PUBLIC_SERVER_URL (or HAPPIER_STACK_SERVER_URL as a fallback), it bakes that URL into the app via EXPO_PUBLIC_HAPPY_SERVER_URL.',
      ].join('\n'),
    });
    return;
  }
  ...
  const dependencyInstallEnv = {
    ...process.env,
    SKIP_SKIA_DOWNLOAD: process.env.SKIP_SKIA_DOWNLOAD || '1',
  };
  await ensureDepsInstalled(uiRepoDir, 'happier-ui', { env: dependencyInstallEnv });
  ...
  if (shouldPrebuild) {
    ...
    if (platform === 'ios' || platform === 'all') {
      ...
      const duplicateTargetRepairs = await repairDuplicateNamedIosTargets({ uiDir, targetName: 'ExpoWidgetsTarget' });
      for (const repair of duplicateTargetRepairs) {
        console.log(`[mobile] repaired ${repair.removedTargetCount} duplicate ExpoWidgetsTarget entries in ${repair.project}.xcodeproj`);
      }

      await ensureReactNativeSkiaIosBinaries({ runnerDir: happyDir, projectDir: uiDir, env });
      await ensureReactNativeLibsodiumNativeBuild({ runnerDir: happyDir, projectDir: uiDir, env });

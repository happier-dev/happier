...
async function loadDesktopInstallModule() {
  try {
    return await import('./desktop_install.mjs');
  } catch (error) {
    assert.fail(`desktop install module should exist: ${error instanceof Error ? error.message : String(error)}`);
  }
}

test('installMacOsDesktopApp replaces an existing stack-scoped macOS app bundle', async (t) => {
  const { installMacOsDesktopApp } = await loadDesktopInstallModule();
...
test('resolveMacOsDesktopInstallPlan sanitizes app bundle path separators', async () => {
  const { resolveMacOsDesktopInstallPlan } = await loadDesktopInstallModule();

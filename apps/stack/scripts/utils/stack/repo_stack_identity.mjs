...
function resolveGitDir(repoRoot, fileOps) {
  try {
    ...
    const commonDirPath = pathApi.join(linkedGitDir, 'commondir');
    const commonDir = readTextFile(commonDirPath, fileOps);
    return commonDir
      ? resolveGitPath(commonDir, { baseDir: linkedGitDir, pathApi })
      : linkedGitDir;
  } catch {
    return null;
  }

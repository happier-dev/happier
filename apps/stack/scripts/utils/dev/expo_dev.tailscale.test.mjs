...
test('ensureDevExpoServer restarts when recorded Tailscale forwarder is dead', async () => {
  const { mkdtemp, mkdir, rm, writeFile, chmod, readFile } = await import('node:fs/promises');
  const { tmpdir } = await import('node:os');
  const { dirname, join } = await import('node:path');
  const { ensureDevExpoServer } = await import('./expo_dev.mjs');
  ...
  const tmp = await mkdtemp(join(tmpdir(), 'hstack-expo-ts-dead-forwarder-'));
  const children = [];
...
if (process.argv.slice(2).join(' ') === 'status --json') {
  console.log(JSON.stringify({ BackendState: 'Running', Self: { TailscaleIPs: ['127.0.0.1'] } }));
  process.exit(0);
}
if (process.argv.slice(2).join(' ') === 'ip -4') {
  console.log('127.0.0.1');
...
`,
      ...
      JSON.stringify(
        {
          ...
          tailscaleForwarderPid: 999_999_999,
    ...
    const result = await ensureDevExpoServer({
      ...
      stackMode: true,
      runtimeStatePath: null,
      stackName: 'qa-agent-tailscale-dead-forwarder',
      envPath: join(tmp, 'stack.env'),
      children,
      expoTailscale: true,
      quiet: true,
    ...
    let spawnRaw = '';
    const deadline = Date.now() + 2_000;
    while (!spawnRaw && Date.now() < deadline) {
      try {
        spawnRaw = await readFile(spawnOut, 'utf-8');
      } catch {
        await new Promise((resolve) => setTimeout(resolve, 25));
      }
    }
    assert.equal(spawnRaw, 'spawned');
  } finally {
    killChildren(children);
    await rm(tmp, { recursive: true, force: true });
  }

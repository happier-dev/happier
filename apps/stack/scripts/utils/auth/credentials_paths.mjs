...
export function resolveStackCredentialPaths({ cliHomeDir, serverUrl = '', env = process.env }) {
  ...
  // Runtime identity is stack-owned. A URL-matching settings profile is only a credential-migration
  // source; letting it replace the stable scope splits daemon state, machine identity, and auth.
...
export function resolveStackDaemonStatePaths({ cliHomeDir, serverUrl = '', env = process.env }) {
  ...
  const stableScopeServerId = resolveActiveServerIdOverride(env);
  const daemonLifecycleScopeId = resolveDaemonLifecycleScopeIdOverride(env);
  // Daemon state is lifecycle state, not a credential-profile artifact. Endpoint/profile settings
  // may select credentials but never select or alias daemon ownership state.

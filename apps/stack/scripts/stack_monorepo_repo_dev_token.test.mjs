...
test('hstack stack new --repo=dev pins workspace dev checkout', async () => {
  const scriptsDir = dirname(fileURLToPath(import.meta.url));
  const rootDir = dirname(scriptsDir);
  const tmp = await mkdtemp(join(tmpdir(), 'hstack-stack-dev-token-'));

  const workspaceDir = join(tmp, 'workspace');
  const storageDir = join(tmp, 'storage');
  const homeDir = join(tmp, 'home');
  const sandboxDir = join(tmp, 'sandbox');
  const stackName = 'exp-dev-token';

  const mainRoot = join(workspaceDir, 'main');
  const devRoot = join(workspaceDir, 'dev');
  for (const monoRoot of [mainRoot, devRoot]) {
  ...
  const res = await runNode([join(rootDir, 'scripts', 'stack.mjs'), 'new', stackName, '--repo=dev', '--no-copy-auth', '--json'], {
    cwd: rootDir,
    env,
  });

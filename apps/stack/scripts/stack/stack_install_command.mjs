...
async function resolveInstallPort({ stackName, explicitPortRaw, stackAlreadyExists, env = process.env }) {
  const explicitPort = coercePort(explicitPortRaw);
  if (explicitPort) {
    await assertPortCanBePinned({ stackName, port: explicitPort, stackAlreadyExists, env });
    return explicitPort;
  }
  if (stackAlreadyExists) {
    const { env: stackEnv } = await readStackEnv(stackName);
    const existing = coercePort(stackEnv.HAPPIER_STACK_SERVER_PORT);
    if (existing) return existing;
  }

  const reservedPorts = await collectReservedStackPorts({ excludeStackName: stackName, env });
  return await pickNextFreePort(getDefaultPortStart(stackName), { reservedPorts });
}

async function assertPortCanBePinned({ stackName, port, stackAlreadyExists, env = process.env }) {
  const { env: stackEnv } = stackAlreadyExists ? await readStackEnv(stackName) : { env: {} };
  const existing = coercePort(stackEnv.HAPPIER_STACK_SERVER_PORT);
  if (existing === port) return;
  const reservedPorts = await collectReservedStackPorts({ excludeStackName: stackName, env });
  if (reservedPorts.has(port)) {
    throw new Error(`[stack install] port ${port} is already reserved by another stack env.`);
  }
...
export async function buildStackInstallPlan({
  ...
  if (desktopMode !== 'none' && desktopPlatform !== 'darwin') {
    throw new Error('[stack install] desktop build/install is only supported on macOS in v1. Re-run with --no-desktop on this platform.');
  }
...
export async function runStackInstallCommand({
  ...
  if (!plan.dryRun) {
    await assertPortCanBePinned({ stackName, port: plan.port, stackAlreadyExists: stackExistsSync(stackName), env });

...
For a headless development worker whose repository will live on the guest disk,
create a bounded VM without host filesystem mounts or containerd:
...
LIMA_CONTAINERD=none \
  ./scripts/provision/macos-lima-vm.sh happier-dev-worker
```

`LIMA_DISK`, `LIMA_MOUNT_NONE`, and `LIMA_CONTAINERD` are creation-time
settings. Recreate an existing instance if you need to change them. Keep the
hot repository inside the guest filesystem; a host-mounted checkout measures
the host filesystem plus mount translation rather than guest-local Linux I/O.

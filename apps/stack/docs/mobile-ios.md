...
- **Shared dev-client app** (recommended for development):
  - Install *one* Hapsta dev-client app on your phone.
...
If you ran `hapsta setup --profile=dev`, the setup wizard can optionally offer to install the dev-client for you.

Install the dedicated Hapsta dev-client app on your iPhone (USB).
...
```bash
hapsta stack mobile-dev-client <stack> --install
```

Optional:

```bash
hapsta mobile-dev-client --install --device="Your iPhone"
hapsta mobile-dev-client --install --clean
```

Then run any stack with mobile enabled:

```bash
hapsta stack dev <stack> --mobile
# or:
hapsta dev --mobile
```
...
- **LAN requirement**: for physical iPhones, Metro must be reachable over LAN.
  - Hapsta defaults to `lan` for mobile, and will print a QR code + deep link.
...
hapsta stack mobile:install <stack> --name="Happier PR 272" --device="Your iPhone"
```
...
```bash
hapsta mobile --prebuild
# (optional) fully regenerate ios/:
hapsta mobile --prebuild --clean
```
...
- **You usually don’t need to run this manually** because both:
  - `hapsta mobile-dev-client --install`
  - `hapsta stack mobile:install <stack>`
  already include `--prebuild`.
- Legacy alias: `hapsta mobile:prebuild` exists (hidden), but prefer `hapsta mobile --prebuild`.

## Manual `hapsta mobile` usage (advanced)

If you want to work on the embedded Expo app directly (outside `hapsta dev --mobile`), `hapsta mobile` supports:
...
hapsta mobile --prebuild --run-ios --device="Your iPhone"
hapsta mobile --prebuild --run-ios --configuration=Release --no-metro
```

## Notes / troubleshooting

- **QR opens the wrong app**:
  - The dev-client QR uses the `HAPPIER_STACK_DEV_CLIENT_SCHEME` (default: `hapsta-dev`).
...
```bash
hapsta mobile:devices
```

- **Code signing weirdness on a real iPhone**:
  - Hapsta will try to “un-pin” signing fields in the generated `.pbxproj` so Expo/Xcode can reconfigure signing
    (this avoids failures where automatic signing is disabled because `DEVELOPMENT_TEAM`/profiles were pinned).
  - If you want to manage signing manually, pass `--no-signing-fix` to `hapsta mobile ...` / `hapsta stack mobile <stack> ...`.

## Bake the default server URL into the app (optional)

If you want the built app to default to your Hapsta server URL, set this **when building**:

```bash
HAPPIER_STACK_SERVER_URL="https://<your-machine>.<tailnet>.ts.net" hapsta mobile-dev-client --install
```
...
```bash
HAPPIER_STACK_IOS_BUNDLE_ID="com.yourname.hapsta.dev" hapsta mobile --prebuild --run-ios --no-metro
```

- **App name (what shows on the home screen)**:

```bash
HAPPIER_STACK_IOS_APP_NAME="Hapsta" hapsta mobile --prebuild --run-ios --no-metro
```
...
- a stack pinned to that worktree (`hapsta stack wt <stack> -- use ...`)
...
```bash
hapsta wt new happy happier --from=origin --base=origin/happier
```

Pin the `happier` stack to that worktree:

```bash
hapsta stack wt happier -- use happier-dev/happier
```
...
# or:
hapsta stack mobile happier --prebuild --platform=all --clean --no-metro
```
...
With Hapsta helpers (stack-scoped):

```bash
# Inspect:
hapsta stack eas happier env:list --environment production

# Create (use env:update if it already exists):
hapsta stack eas happier env:create --name EXPO_APP_NAME --value "Happier" --environment production --visibility plainText
hapsta stack eas happier env:create --name EXPO_APP_BUNDLE_ID --value "dev.happier.app" --environment production --visibility plainText
hapsta stack eas happier env:create --name EXPO_APP_SCHEME --value "happier" --environment production --visibility plainText
hapsta stack eas happier env:create --name EXPO_APP_OWNER --value "happier-dev" --environment production --visibility plainText
hapsta stack eas happier env:create --name EXPO_APP_SLUG --value "happier" --environment production --visibility plainText
```
...
```bash
hapsta stack eas happier ios --profile production --interactive
```

After credentials are set up, you can run non-interactive builds:

```bash
hapsta stack eas happier ios --profile production --no-wait --non-interactive
hapsta stack eas happier android --profile production --no-wait --non-interactive
```

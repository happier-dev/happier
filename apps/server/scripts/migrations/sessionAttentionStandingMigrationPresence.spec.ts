...
// Shared with the predecessor client: a Happier build writing attention standings against this
// server must find the same table, columns, and migration identity, so the id is part of the
// contract and not a local naming choice.
...
describe('session attention standing migration', () => {
    it.each([
        ...
        expect(schema).toMatch(/^\s+standing\s+Boolean$/mu);

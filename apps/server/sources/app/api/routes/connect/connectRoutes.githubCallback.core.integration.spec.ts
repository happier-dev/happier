...
describe("connectRoutes (GitHub callback)", () => {
    ...
    it("keeps external connect params available when the retired master env is off", async () => {
        applyGithubConnectCallbackEnv(harness, {
        ...
        expect(paramsRes.statusCode).toBe(200);
        expect(new URL((paramsRes.json() as { url: string }).url).searchParams.get("state")).toBeTruthy();

        await app.close();
    });

    it("completes an in-flight connect callback when the retired master env is turned off", async () => {
        applyGithubConnectCallbackEnv(harness);

...
export async function initGithub() {
    if (
        ...
        // Use the App's built-in webhooks instance for type-safe event processing.
...
function registerWebhookHandlers() {
    ...
    // Catch-all for unhandled events (avoid duplicate logs for handled ones).
    const handledEventBases = new Set(["push", "pull_request", "issues", "star", "repository"]);
    webhooks.onAny(async ({ id, name }: EmitterWebhookEvent) => {
        const base = String(name).split(".")[0];
        if (handledEventBases.has(base)) {
            return;
        }
        log({ module: 'github-webhook', event: name as string }, 
            `Received unhandled webhook event: ${name}`, { id });
    });

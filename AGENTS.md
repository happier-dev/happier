...
2. Use repository skills when relevant:
   ...
   - For issue-linked implementation and release follow-up, use the correction lifecycle in `docs/issue-triage.md`: distinguish source integration from dev/preview/stable availability, require the next exact GitHub mutation preview to include `stage:source` once an open issue's complete correction is integrated and verified on canonical `dev`, make reporter follow-up depend on their stated channel, keep detailed public reasoning separate from private evidence, never use a closing commit keyword when default-branch integration does not satisfy the issue's closure gate, and preserve material issue-author/commenter contributions with a verified `Co-authored-by:` trailer.

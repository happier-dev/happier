...
- Ordinary commits use the checkout's existing current-user `git config user.name` and `git config user.email`. Verify both before the first commit, never rewrite them to a GitHub bot or another contributor, and stop rather than inventing a missing identity. GitHub comments and repository mutations use `happier-bot` through `skills/happier-github-ops`; PR-stewardship pushes use its isolated bot Git transport. A foreign author's rebase is the narrow exception: preserve original authors, use the verified bot only as committer and push actor for the rewritten history, and never alter persistent Git identity.

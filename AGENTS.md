...
2. Use repository skills when relevant:
   ...
   - `skills/happier-pr-steward` when the user asks to assess and shepherd a pull request through an evidence-backed recommendation, approved follow-up commits, Remote Dev -> Dev intent ports, public review requests, and current-head review/CI monitoring. It composes `happier-review` rather than replacing its review standard and preserves every `happier-github-ops` human mutation gate.

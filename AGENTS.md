...
2. Use repository skills when relevant:
   ...
   - `skills/happier-plugin-authoring` before scaffolding or changing a Happier plugin through the public authoring workflow. It owns the public API/capability lookup, author loop, diagnostics, review-rejoin, and package-reference routing; generated scaffold copies reuse this same source.
   ...
   - `skills/happier-slopo` for embedding-assisted duplicate-implementation discovery in production source. It supplies advisory candidates to `happier-review`; similarity alone is never a defect, refactor order, plan-completeness verdict, or CI gate.
   - `skills/happier-pr-steward` when the user asks to assess and shepherd a 0.3 pull request through an evidence-backed recommendation, approved follow-up commits, public review requests, and current-head review/CI monitoring. It composes `happier-review` rather than replacing its review standard and preserves every `happier-github-ops` human mutation gate.
   ...
   - `skills/happier-controlled-stack-qa` only after a user or human explicitly asks for a dedicated, isolated, stable, controlled, snapshot-backed, or manual-restart QA stack. Do not infer this authorization from an ordinary testing/QA request. Once invoked, reuse and remember the same stack throughout the current session unless the user explicitly requests multiple stacks or replacement.
   - `skills/happier-remote-work` after the user explicitly authorizes remote compute; authorization lasts for the user session unless revoked or narrowed. The preferred-execution rule below applies independently and lets operator configuration choose local or remote execution.

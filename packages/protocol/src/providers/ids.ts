...
export const ProviderConnectionIdSchema = canonicalBoundedId(256).brand<'ProviderConnectionId'>();
export type ProviderConnectionId = z.infer<typeof ProviderConnectionIdSchema>;

export const ProviderContributionKeySchema = canonicalBoundedId(512);
export type ProviderContributionKey = z.infer<typeof ProviderContributionKeySchema>;

export const ProviderLocalIdSchema = canonicalBoundedId(128).regex(/^[A-Za-z0-9][A-Za-z0-9._-]*$/u);

export const ProviderMachineIdSchema = canonicalBoundedId(256);

export const ProviderAgentTargetKeySchema = canonicalBoundedId(256);

export const ProviderModelIdSchema = z.string().trim().min(1).max(512).refine(
  (value) => !/[\u0000-\u001f\u007f]/u.test(value),

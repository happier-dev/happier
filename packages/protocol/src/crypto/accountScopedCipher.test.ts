...
/**
 * Generated by the actual `remote-dev` predecessor writer at
 * 9b097966a35e643b51e84af987a1f30869696416 from
 * `packages/protocol/src/crypto/accountScopedCipher.ts`, using the exact
 * payload and deterministic nonce from its Account draft test. This is a
 * prospective predecessor input, not a current-reader reconstruction.
 */
const REMOTE_DEV_9B097966_SESSION_DRAFT_PRIVATE_PAYLOAD_VECTOR = {
  ciphertext: 'oQoBAgMEBQYHCAkKCwwNDg8QERITFBUWFxgovLMWw7gaf7P4ig25WqdbYFEBiBal69Yx4x7M0j9+ErMDSka7zzhe7kxnV6w+oGkIYQDK561sb32BbaU86iQomQZQaDYOJFyQB+d2vgnGgrI6T9o2fApFOovAvYQGI1LO64hpp+pMWQ==',
  payload: {
    v: 1,
    address: {
      kind: 'newSession',
...
const FROZEN_CANONICAL_ACCOUNT_SCOPED_VECTORS = [
  ...
  },
  {
    ...
    kindByte: 10,
    ciphertext: 'oQohIiMkJSYnKCkqKywtLi8wMTIzNDU2Nzj6YCLJPQRfP2Fj1AYTdAVYvTvN2OvQ8BVcXutUYtFgwwkUWKcsngmfktmYCEP52Ldn9s6FbUWVuA==',
...
describe('accountScopedCipher', () => {
  ...
  it('opens the exact remote-dev@9b097966 Account draft ciphertext under its predecessor domain', () => {
    ...
    expect(readAccountScopedCiphertextKindByte(
      REMOTE_DEV_9B097966_SESSION_DRAFT_PRIVATE_PAYLOAD_VECTOR.ciphertext,
    )).toBe(10);
    expect(openAccountScopedBlobCiphertext({
      kind: 'account_session_draft_private_payload',
      material,
      ciphertext: REMOTE_DEV_9B097966_SESSION_DRAFT_PRIVATE_PAYLOAD_VECTOR.ciphertext,
    })).toMatchObject({
      format: 'account_scoped_v1',
      kindTag: 'canonical',
      value: REMOTE_DEV_9B097966_SESSION_DRAFT_PRIVATE_PAYLOAD_VECTOR.payload,
    });
  });
   ...
   * approved r4.4.6 allocation using the same v1 wire algorithm. Slot 10 preserves
   * the moving-predecessor Account session-draft domain. Slots 11-12 freeze the approved
   ...
   * domain. Slot 26 freezes the approved r4.4.8 owner-metadata domain after its
   * undeployed allocation relocation.
   */
  ...
  it('freezes canonical allocated kinds 10 and above with no alias or cross-domain admission', () => {
    const material: AccountScopedCryptoMaterial = {
      type: 'dataKey',
      machineKey: FROZEN_ACCOUNT_SCOPED_VECTOR_MACHINE_KEY,
    };
    const ownerVector = FROZEN_CANONICAL_ACCOUNT_SCOPED_VECTORS.find(
      (vector) => vector.kind === 'session_owner_metadata',
    );
    if (!ownerVector) throw new Error('expected the frozen owner-metadata vector');
    const noAliasVectors = FROZEN_CANONICAL_ACCOUNT_SCOPED_VECTORS.filter(
      (vector) => vector.kindByte >= 10,
    );
  ...
  it('keeps the complete canonical Account-cipher allocation collision-free', () => {
    const canonicalKindBytes = FROZEN_CANONICAL_ACCOUNT_SCOPED_VECTORS
      .map((vector) => getAccountScopedBlobKindByte(vector.kind));

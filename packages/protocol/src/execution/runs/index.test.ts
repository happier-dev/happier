...
describe('executionRuns protocol', () => {
  ...
  it('accepts a typed optional connectedServices selection on start requests and rejects malformed selections', () => {
    const parsed = ExecutionRunStartRequestSchema.parse({
      ...
      instructions: 'Run with connected services.',
    ...
    const omitted = ExecutionRunStartRequestSchema.parse({
      intent: 'review',
      backendTarget: { kind: 'builtInAgent', agentId: 'claude' },
      permissionMode: 'read_only',
      retentionPolicy: 'ephemeral',
    ...
    expect(omitted.connectedServices).toBeUndefined();

    const malformed = ExecutionRunStartRequestSchema.safeParse({
      intent: 'delegate',
      backendTarget: { kind: 'builtInAgent', agentId: 'codex' },
      permissionMode: 'default',
      retentionPolicy: 'resumable',
      ...
      connectedServices: {
        v: 1,
        bindingsByServiceId: {
          'openai-codex': { source: 'connected', selection: 'profile' },
        },
      },
    });
  ...
  it('accepts legacy vendorSessionId input on resume handles as provider session identity compatibility', () => {
    ...
    expect(Object.prototype.hasOwnProperty.call(parsed.resumeHandle, 'vendorSessionId')).toBe(false); // legacy vendorSessionId read-compat
  });

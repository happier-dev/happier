 ...
 *
 * v1 ships the two kinds carried by the one activity record this repository publishes today —
 * `activity/workflow_run.v1`, written by
 * `packages/plugins/claude/src/agent/workflowRecords/publishWorkflowActivitySnapshot.ts`, whose
 * snapshot holds both the run and its agents. `agentActivityKindV1.test.ts` names that producer for
 * each member, so a member added without one fails there.
 *

 ...
 * `../activityHeadlineOrdering.ts`. The value is generous enough that the transport is never the
 * thing that starves a roster pane's own finished-section cap. Producers may lower it; nothing may
 * cap the active side. Terminal detail stays durable in the records the entries point at.
 */
 ...
 * `../activityHeadlineOrdering.ts`, which the workflow headline calls with its own accessors.
 ...
 *
 * The single shared builder for this key: the producer, any test fixture and every consumer that
 * assembles a headline go through it, so ordering, bounding and the count-only projection cannot
 * diverge between the producer and whatever else assembles one.
 */

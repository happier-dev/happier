 ...
 *
 * A CROSS-REPO WIRE LITERAL: this repository and `../remote-dev` both write and read this exact
 * string, so it is protocol-owned rather than restated at each producer and consumer, and it is
 * pinned by a hand-written literal in `agentActivityHeadlineMetadataKey.test.ts`. It is published
 * ALONGSIDE `sessionWorkflowActivityHeadlineV1`, which keeps its name, its shape and its own parity
 * lock — this is the expand step only; removing the older key is out of scope.
 */
...
/**
 * The unified agent-activity headline: a compact live pointer published into session metadata.
 *
 * Shaped deliberately like `SessionWorkflowActivityHeadlineV1` — same envelope, same active/recent
 * split, same derived-primary hint — because it is DERIVED from the same committed snapshots rather
 * than parallel to it. What it adds is the ability to describe kinds of work beyond a workflow run,
 * and per-entry rows so a cold-opened client can name every agent without fetching a transcript
 * page; what it deliberately does not add is detail.
 *
 * `activeEntries` is never bounded: a roster that is quietly incomplete is the failure this key
 * exists to prevent. Only `recentEntries` is bounded, and `truncated` says so out loud.
 */
...
/**
 * Read a headline written by any build of either repository, tolerating entries this build cannot
 * parse.
 *
 * The tolerance is the point, and it is not defensive habit: `AgentActivityKindV1` grows a member
 * every time a kind gains a proven writer, and statuses can follow. With a whole-array parse, the
 * first entry from a newer producer would take the ENTIRE roster down to `null` on every client that
 * predates it, turning a forward-compatible addition into a regression for everyone else. Dropping
 * the rows this build genuinely cannot render, and keeping the ones it can, is the required degrade.
 *
 ...
 *
 * Every consumer goes through this rather than reaching for the key itself, so the cross-repo
 * literal has exactly one spelling and the parity lock actually locks something. Returns `null` for
 * metadata written by a producer that predates the key, which is the shape every session on an
 * older daemon still has.
 */

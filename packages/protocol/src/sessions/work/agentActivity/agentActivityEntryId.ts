 ...
 *
 * A roster unions the headline the producer publishes with the entries a client derives from the
 * transcript, and it unions them BY ID. Two spellings of one agent — `workflow_agent:wf_1:a1` on the
 * producer against `wf_1/a1` on the consumer — therefore render that agent twice, with every schema
 * valid, every test green and nothing failing loudly. That is the failure this module exists to make
 * impossible: the producer projection and the consumer merge both import these functions, so there
 * is one spelling by construction, and the hand-written literals in the test are the lock on it.
 *
 ...
 *   recorded two-run workflow labels an agent `workflow-agent:1` in BOTH of its runs.
 * - **The agent handle** is the provider-side tool-use id underneath. It is the ONLY identifier the
 *   two sides can independently produce for the same work: the producer sees the SDK's `tool_use` id
 *   (the agent's snapshot id), and a client sees the transcript tool call whose id doubles as the
 *   sidechain id. A client cannot know which run the producer attached an agent to, so the merge
 *   joins on the handle and adopts the headline's id.
 */
 ...
 * rather than a "components must not contain ':'" rule: a synthesized run id is literally
 * `implicit:agent-activity`, and a workflow journal's fallback agent id is literally
 * `workflow-agent:1`. A scheme that forbade either would push the producer into either throwing on
 * real data or minting an id that reads back as a different agent — which, joined on, merges two
 * agents into one row or splits one into two.
 *

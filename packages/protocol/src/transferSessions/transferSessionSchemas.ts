...
import { TransferChunkEnvelopeSchema, type TransferChunkEnvelope } from '../machineTransfer/transferStream.js';
...
function boundedBase64String(maxLength: number): z.ZodString {
  return boundedString(maxLength).regex(/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/, {

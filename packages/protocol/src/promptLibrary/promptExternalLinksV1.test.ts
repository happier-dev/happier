...
describe('promptExternalLinksV1 schemas', () => {
  it('preserves additive fields on external link payloads', () => {
      ...
      links: [
        {
          ...
          lastSyncAtMs: 123,
          futureLinkField: 'keep-me',
        },
      ],
      futureLinksEnvelope: {
        kind: 'prompt_external_links.v2',
      },
    });

    expect((parsed as any).futureLinksEnvelope).toEqual({
      kind: 'prompt_external_links.v2',
    });
    expect((parsed.links[0] as any)?.futureLinkField).toBe('keep-me');
  });

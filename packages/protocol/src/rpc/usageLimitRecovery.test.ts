...
import { RPC_METHODS, SESSION_RPC_METHODS } from './index.js';
...
describe('RPC_METHODS usage-limit recovery surface', () => {
  it('declares daemon and session-scoped usage-limit recovery controls', () => {
    ...
    expect((RPC_METHODS as any).DAEMON_SESSION_USAGE_LIMIT_CONSUME_RESET_CREDIT).toBe('daemon.sessionUsageLimit.consumeResetCredit');
    ...
    expect((SESSION_RPC_METHODS as any).SESSION_USAGE_LIMIT_CONSUME_RESET_CREDIT).toBe('session.usageLimit.consumeResetCredit');
  });
});

...
Strict TERM-7b evidence uses two separately governed Ed25519 registries. `device-evidence-capture-authorities.json` controls keys allowed to attest loaded-device captures and embedded build identities, while `release-approval-authorities.json` controls independent Android release approval. Capture policy schema v2 binds every authority to exact renderer/build IDs and an inclusive validity window; the internal `internal-term-device-qa-20260828` authority is limited to the two listed QA builds from `2026-08-28T11:30:00Z` through `2026-08-30T23:59:59Z`. Its private key is an external local operational secret at `/home/leeroy.guest/.happier/term-qa-authority-20260828/private.pem`; never copy it into the repository, evidence, logs, or reports.

The checked-in `release-approval-authorities.json` remains empty. Internal capture signatures prove only controlled QA provenance and never constitute Android legal/product release approval. Evidence builds receive `HAPPIER_TERMINAL_NATIVE_CAPTURE_AUTHORITY_ID` and `HAPPIER_TERMINAL_NATIVE_CAPTURE_PRIVATE_KEY_PATH` only in the controlled build environment and embed the signed identity in the retained app ZIP/APK; ordinary development builds remove stale generated identities.
...
The source extractor and package probes provide engineering evidence about the exact pinned source closure; they do not provide legal advice. `HAPPIER_TERMINAL_NATIVE_ANDROID_LEGAL_ACCEPTED=1` is only an assertion supplied by an authorized release process after its external product/legal decision has been recorded against that exact revision.

Internal device QA before that decision must use `HAPPIER_TERMINAL_NATIVE_ANDROID_ENGINEERING_QA=1` with the explicit `APP_ENV=internaldev` or `APP_ENV=internalpreview` allowlist. Missing, unknown, `publicdev`, `preview`, and `production` environments fail closed; the override is not release approval evidence.

`yarn workspace @happier-dev/terminal-native evidence:android:artifact` compares candidate and optional baseline APK sizes and verifies the packaged ABI directories requested by `HAPPIER_TERMINAL_NATIVE_ANDROID_REQUIRED_ABIS`. This is static package evidence only; TERM-7b still requires installing and loading the candidate on each supported device ABI.

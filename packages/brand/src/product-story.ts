...
const seeds = [
    ...
    ['worktrees', 'worktrees', null, 'A worktree per session. Or not.', 'A worktree per session.\nOr not.', 'Give each session its own checkout of the repo, or start in the folder you are already in.', 'Start a session in its own Git worktree—a real checkout on its own branch—so several agents can work the same repository without stepping on each other. Pick a branch and Happier creates it, suggests a name, and reuses an existing worktree instead of duplicating it. Starting in the current folder stays one tap away.', 'primary'],
    ['handoff', 'handoff', null, 'Move a running session to another machine.', 'Move a running session\nto another machine.', 'Hand a session to another computer and pick it up where it stopped—optionally with the code.', 'Hand a running session to your desktop, a VPS, or a dev box and continue where it stopped. The working tree can travel with it as a full snapshot or as only the changes, writing a sibling copy rather than overwriting, and leaving ignored files behind unless you name them.', 'primary'],

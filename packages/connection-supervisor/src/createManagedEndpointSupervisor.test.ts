...
describe('createManagedEndpointSupervisor', () => {
  ...
  it('can move directly to auth_failed from an externally reported probe result', async () => {
    ...
    supervisor.reportProbeResult?.({ status: 'auth_failed', statusCode: 403, errorMessage: 'forbidden' }, probeReportScope);
  ...
  it('ignores externally reported probe results from an older endpoint scope', async () => {
    vi.useFakeTimers();
    ...
    expect(supervisor.getState()).toEqual(expect.objectContaining({ phase: 'online', attempt: 0 }));

    supervisor.reportProbeResult?.({ status: 'auth_failed', statusCode: 403, errorMessage: 'forbidden' }, firstScope);

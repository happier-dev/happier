...
class OfflineTtsEngineCache {
 public:
  ...
  /**
   * Admit one native initialize request before its platform worker starts.
   * `admissionId` is generated by the JS caller and remains immutable for that
   * request. Unlike `release`, cancelling this admission must not advance the
   * path epoch or retire the cached engine: a queued prewarm is not authority to
   * stop another synthesis already using the same pack.
   *
   * The record deliberately lives beside the cache epoch rather than in either
   * platform bridge. Only this cache owns the find/create/publish decision, so
   * it is the one place that can remember cancellation while a worker is queued
   * and still reject the same request both before and after model creation.
   */
  bool admitInitialization(const std::string &assetsDir, const std::string &admissionId) {
    if (assetsDir.empty() || admissionId.empty()) {
      return false;
    }

    std::lock_guard<std::mutex> lock(mutex_);
    auto &admissions = initializationAdmissionsByAssetsDir_[assetsDir];
    return admissions.emplace(admissionId, InitializationAdmission{epochs_.capture(assetsDir), false}).second;
  }

  /**
   * Refuse one admitted initializer without changing the pack's currentness or
   * touching any cached engine. Returns false after the admission has already
   * reached a terminal decision, which makes late JS cancellation a harmless
   * no-op instead of a retirement of an engine another caller may be using.
   */
  bool cancelInitialization(const std::string &assetsDir, const std::string &admissionId) {
    if (assetsDir.empty() || admissionId.empty()) {
      return false;
    }

    std::lock_guard<std::mutex> lock(mutex_);
    auto byAssetsDir = initializationAdmissionsByAssetsDir_.find(assetsDir);
    if (byAssetsDir == initializationAdmissionsByAssetsDir_.end()) {
      return false;
    }
    auto admission = byAssetsDir->second.find(admissionId);
    if (admission == byAssetsDir->second.end()) {
      return false;
    }
    admission->second.cancelled = true;
    return true;
  }
  ...
  EngineRef leaseOrCreate(const std::string &assetsDir, Create &&create) {
    ...
    CacheEpoch admission;
    {
      ...
      admission = epochs_.capture(assetsDir);
    }
    ...
    std::lock_guard<std::mutex> lock(mutex_);
    if (!epochs_.isCurrent(assetsDir, admission)) {
  ...
  /**
   * Continue exactly one admitted native initialization. The admission is
   * consumed at its terminal decision, so a fresh initialization for the same
   * path is independent. A cancellation arriving while `create` runs is checked
   * again before publication, but never retires an engine or its jobs.
   */
  template <typename Create>
  EngineRef leaseOrCreateInitialization(
      const std::string &assetsDir,
      const std::string &admissionId,
      Create &&create) {
    if (assetsDir.empty() || admissionId.empty()) {
      return EngineRef();
    }
    ...
    {
      std::lock_guard<std::mutex> lock(mutex_);
      auto admission = findInitializationAdmissionLocked(assetsDir, admissionId);
      if (!admission || admission->cancelled || !epochs_.isCurrent(assetsDir, admission->epoch)) {
        eraseInitializationAdmissionLocked(assetsDir, admissionId);
        return EngineRef();
      }
      auto cached = enginesByAssetsDir_.find(assetsDir);
      if (cached != enginesByAssetsDir_.end()) {
  ...
  EngineRef release(const std::string &assetsDir) {
    EngineRef released;
    {
      ...
      // A pack mutation is the lifecycle boundary for all its queued
      // initializers. They cannot publish after the bytes are retired, and their
      // records are now terminal even if a platform worker never starts.
      initializationAdmissionsByAssetsDir_.erase(assetsDir);
      auto it = enginesByAssetsDir_.find(assetsDir);
  ...
  std::vector<EngineRef> releaseAll() {
    std::unordered_map<std::string, EngineRef> released;
    {
      ...
      initializationAdmissionsByAssetsDir_.clear();
 ...
 private:
  struct InitializationAdmission {
    CacheEpoch epoch;
    bool cancelled;
  };

  InitializationAdmission *findInitializationAdmissionLocked(
      const std::string &assetsDir,
      const std::string &admissionId) {
    auto byAssetsDir = initializationAdmissionsByAssetsDir_.find(assetsDir);
    if (byAssetsDir == initializationAdmissionsByAssetsDir_.end()) {
    ...
    auto admission = byAssetsDir->second.find(admissionId);
    return admission == byAssetsDir->second.end() ? nullptr : &admission->second;
  }

  void eraseInitializationAdmissionLocked(const std::string &assetsDir, const std::string &admissionId) {
    auto byAssetsDir = initializationAdmissionsByAssetsDir_.find(assetsDir);
    if (byAssetsDir == initializationAdmissionsByAssetsDir_.end()) {
      return;
    }
    byAssetsDir->second.erase(admissionId);
    if (byAssetsDir->second.empty()) {
      initializationAdmissionsByAssetsDir_.erase(byAssetsDir);
    }
  }

  mutable std::mutex mutex_;
  std::unordered_map<std::string, EngineRef> enginesByAssetsDir_;
  std::unordered_map<std::string, std::unordered_map<std::string, InitializationAdmission>>
      initializationAdmissionsByAssetsDir_;
  CacheEpochs epochs_;
};

...
describe('buildHappierReplayPromptFromDialog', () => {
  ...
  it('shrinks the recent transcript tail to fit maxPromptChars', () => {
    const prompt = buildHappierReplayPromptFromDialog({
      ...
      // grew, so the budget that forces the tail to shrink without refusing the
      // seed outright moved up with it.
      maxPromptChars: 540,
      dialog: [
    ...
    expect(prompt.length).toBeLessThanOrEqual(540);
  });
});
...
describe('buildHappierReplayPromptFromDialog total budget', () => {
  ...
  it('keeps the newest turn and stays inside the total at the smallest configured cap', () => {
     ...
     *
     * MEASURED, not chosen. Every WRITER now clamps to at least 1024 — the
     * settings screen, the account settings catalog, the UI clamp, and the
     ...
     * is what that floor has to clear. Below the floor the builder returns
     * nothing, which is the contract: a frame announcing replayed context it did
     * not carry is the one output that loses the reader's trust in every other
     * line it does carry.
     */
    const maxPromptChars = 1_024;
    const prompt = buildHappierReplayPromptFromDialog({
      previousSessionId: 'sess_prev',
      strategy: 'summary_plus_recent',
      recentMessagesCount: 10,
  ...
  // untrusted-content statement, no `<recent_transcript>` marker — which is the one output the
  ...
  // second `<recent_transcript>` section and authored-looking `User:` / `Assistant:` turns, and
  ...
  it('holds the total and never emits a partial role prefix at any reachable budget', () => {
    ...
    for (let budget = 200; budget <= 900; budget += 1) {
      for (const strategy of ['recent_messages', 'summary_plus_recent'] as const) {
        ...
        if (prompt.length > 0 && !prompt.includes('<recent_transcript>')) {
...
describe('the range claim never outlives the lines it names', () => {
   ...
   *
   * Whole, not merely opened: the builder keeps a clipped fragment of the newest
   ...
   * truncated message as a delivered one, which is exactly what the target Agent
   * then declines to re-fetch. So each named row is matched by its entire
   * rendered line.
   */
   ...
   *
   * When the newest turn alone overflows the region, the builder keeps a clipped
  ...
  it('does not claim a row whose text the builder itself clipped, at the shipped default budget', () => {
    ...
    // newest message, which costs the target a re-read and lies about nothing.
  ...
  it('drops the claim, the paging instruction and the cursor with the rows they name', () => {
    ...
    expect(fitted).toContain('[Older context was omitted to fit the replay budget.]');
  });
  ...
  it('claims nothing when a dropped row of the pointer\u2019s own Session sits above the span', () => {
    ...
    const clipped = buildHappierReplayPromptFromDialog({
      ...
      retrieval: { sessionId, renderInvocation },
      // Re-derived after the container restructure.
  ...
  it('claims nothing when a foreign row renders exactly like the own row it stands in for', () => {
    ...
    const collided = buildAt(windowWith(collidingText), 1_900);
    ...
    const control = buildAt(windowWith(distinctText), 1_900);
 ...
 * 120 000 character budget while the user's last instruction sat 1 154 rows
 * before the cutoff. The target Agent CAN reach the rest — the transcript action
 * is on its tool surface — but nothing in the seed said so, said where, or said
 * which slice it was already holding, so it either worked from the tail alone or
 * paged the transcript from the start and re-read its own prompt.
 */
describe('happierReplayPrompt — retrieval pointer', () => {
  ...
  it('names the session, the range already inlined, and a backwards-paging invocation', () => {
    ...
    // The direction lives in the FRAME, where no fit can delete it, and the
...
describe('happierReplayPrompt — retrieval planning surface', () => {
  ...
  it('plans no more transcript room than the builder will have, reservation included', async () => {
    ...
    expect(prompt).not.toContain('[Older context was omitted to fit the replay budget.]');
  ...
  it('treats a null recentMessagesCount as "the character budget is the bound"', () => {
    ...
    // caller supplies one, and a drop the reader is not told about is a
    // truncated tail presented as the whole conversation.
    ...
    expect(capped).toContain('[Older context was omitted to fit the replay budget.]');
  });

  it('marks a window the retrieval stopped short of, which it cannot count', () => {
    ...
    expect(truncated).toContain('[Older context was omitted to fit the replay budget.]');
...
describe('happierReplayPrompt — session title and pinned last user instruction', () => {
  const baseDialogItem = () => ({ role: 'User' as const, createdAt: 1, text: 'hello there' });
  ...
  it('inlines the Session title as a header line, escaped like any untrusted text', () => {
    ...
    // One header line, and it cannot forge a section or open a turn.
    expect(prompt.split('<recent_transcript>\n')).toHaveLength(2);

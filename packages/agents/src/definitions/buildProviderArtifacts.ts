import { CANONICAL_AGENT_IDS, type AgentId, type CanonicalAgentId } from '../types.js';
...
function createProviderDefinition(agentId: CanonicalAgentId): ProviderCatalogDefinition {
  return {
    id: agentId,
    ...
    providerCliRuntime: CANONICAL_PROVIDER_CLI_RUNTIME_SPECS[agentId],
    providerSettings: getProviderSettingsDefinition(agentId),
  };
}

function createProviderDefinitionContract(agentId: CanonicalAgentId): ProviderDefinitionContractV1 {
  return {
    kindVersion: 1,
    id: agentId,
...
const PROVIDER_DEFINITIONS = Object.freeze(CANONICAL_AGENT_IDS.map((agentId) => createProviderDefinition(agentId)));
...
const PROVIDER_DEFINITION_CONTRACTS = Object.freeze(CANONICAL_AGENT_IDS.map((agentId) => createProviderDefinitionContract(agentId)));
...
export type ProviderArtifacts = Readonly<{
  ...
  providerCliRuntimeSpecs: typeof CANONICAL_PROVIDER_CLI_RUNTIME_SPECS;
}>;

export function buildProviderArtifacts(): ProviderArtifacts {
  return {
    defaultAgentId: DEFAULT_AGENT_ID,
    providerIds: CANONICAL_AGENT_IDS,
    ...
    providerCliRuntimeSpecs: CANONICAL_PROVIDER_CLI_RUNTIME_SPECS,
  };
}

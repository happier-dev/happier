/**
 * The one paged-detail-panel state machine the four forge sources share.
 *
 * It is here rather than in four copies because the rule it enforces is one
 * product contract, and four copies are four places for "nothing here" to start
 * looking like "we could not look". Four outcomes must stay apart, and every
 * forge detail body has all four:
 *
 ...
 *
 * What it deliberately does NOT own is anything the forges genuinely differ on.
 ...
 * `never`. Centralizing those would produce the abstraction the next forge
 * fights.
 *
...
export type ForgePagedStateV1<TRow, TFailure, TIncomplete = never> = Readonly<{
...
export type ForgePagedPageV1<TRow, TIncomplete = never> = Readonly<{
  rows: readonly TRow[];
  omittedRowCount: number;
  projectionTruncated: boolean;
  continuation: string | null;
...
export type ForgePagedEventV1<TRow, TFailure, TIncomplete = never> =
  | Readonly<{ kind: 'requestStarted'; token: number }>
  | Readonly<{ kind: 'pageSettled'; token: number; page: ForgePagedPageV1<TRow, TIncomplete> }>
  | Readonly<{ kind: 'pageFailed'; token: number; failure: TFailure }>
  /** The panel was left. Every forge detail plane discards, so nothing survives. */
...
export function forgePagedInitialState<TRow, TFailure, TIncomplete = never>():
ForgePagedStateV1<TRow, TFailure, TIncomplete> {
  return INITIAL as ForgePagedStateV1<TRow, TFailure, TIncomplete>;
}

export function forgePagedReducer<TRow, TFailure, TIncomplete = never>(
  state: ForgePagedStateV1<TRow, TFailure, TIncomplete>,
  event: ForgePagedEventV1<TRow, TFailure, TIncomplete>,
): ForgePagedStateV1<TRow, TFailure, TIncomplete> {
  switch (event.kind) {
    case 'panelLeft':
      return forgePagedInitialState<TRow, TFailure, TIncomplete>();
    case 'requestStarted':
      return {
        ...state,

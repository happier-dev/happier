...
function materializer(
  implementation: ForgeListedAccountMaterializer['materializeListedAccount'],
): ForgeListedAccountMaterializer {
  return { materializeListedAccount: implementation };
}

function headers(value: ConnectedAccountMaterialization): ForgeListedAccountMaterializer {
...
describe('materializeForgeAuthorization', () => {
  it('requests the exact bound account for the declared purpose and request origin', async () => {
    const materializeListedAccount = vi.fn<
      ForgeListedAccountMaterializer['materializeListedAccount']
    >(async () => ({ kind: 'httpHeaders' as const, headers: { authorization: 'Bearer t' } }));
    const signal = new AbortController().signal;

    const outcome = await materializeForgeAuthorization({
  ...
  it('omits the signal option entirely when the caller supplied none', async () => {
    ...
    await materializeForgeAuthorization({
      connectedAccounts: materializer(materializeListedAccount),
      purpose: PURPOSE,
      account: ACCOUNT,
      origin: ORIGIN,
...
describe('readForgeAuthorization', () => {
  it('admits a materialization the caller already holds by exactly the same rule', () => {
    expect(readForgeAuthorization({ kind: 'httpHeaders', headers: { Authorization: 'token t' } }))
      .toEqual({
        ok: true,
        headers: { Authorization: 'token t' },
        authorization: 'token t',
      });
    expect(readForgeAuthorization({ kind: 'files', files: {} }))
      .toEqual({ ok: false, reason: 'unsupportedMaterialization' });
    expect(readForgeAuthorization({ kind: 'httpHeaders', headers: { Accept: 'x' } }))

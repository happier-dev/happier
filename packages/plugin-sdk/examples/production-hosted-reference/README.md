...
Its real emitted graph is checked against the incumbent installed-Artifact
resolver; package/install rows must use the exact archive and Artifact digest
they exercise. That source-level adoption check does **not** claim a loaded
browser or native frame. Those rows run independently on their observed current
runtime/package/Artifact identities through the Artifact/frame and packed-live
owners; there is no global fixed candidate.
...
The packed command proves archive/install/action lifecycle only. It does not
replace the separate browser/frame evidence.

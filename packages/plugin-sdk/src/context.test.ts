...
import type { ManagedServerSpecV1 } from './managedServer';
import type {
...
describe('PluginContextV1 permission surface', () => {
    it('places permission operations on session-owned surfaces instead of flat ctx.permissions', () => {
        // @ts-expect-error A.5x removes flat ctx.permissions from the final public SDK context.
        type FlatPermissions = PluginContextV1['permissions'];
        void (undefined as unknown as FlatPermissions);
        expectTypeOf<PluginContextV1['session']['permissions']>().toEqualTypeOf<SessionPermissionsServiceV1>();
    ...
    it('exposes typed session-scoped runtime services without raw request/result escape hatches', () => {
        ...
        expectTypeOf<PluginContextV1['session']['permissions']['getMode']>()
            .toEqualTypeOf<() => SessionPermissionModeV1>();
        expectTypeOf<PluginContextV1['session']['mcp']['elicit']>()
            .toEqualTypeOf<(
                request: SessionMcpElicitRequestV1,
                options?: Readonly<{ signal?: AbortSignal }>,
            ) => Promise<SessionMcpElicitResultV1>>();
        expectTypeOf<PluginContextV1['session']['auth']['services']['refreshRuntimeAuth']>()
            .toEqualTypeOf<(
    ...
    it('types ACP custom runtime extension dispatch params without changing the public method shape', () => {
        const requestHandler: AcpRuntimeRequestHandlerV1 = async (params, context) => {
            ...
            expectTypeOf(context.backendId).toEqualTypeOf<string>();
        ...
        expectTypeOf<PluginContextV1['acp']['createRuntime']>()
            .toEqualTypeOf<(
                spec: PluginContextV1['acp']['defineAcpBackend'] extends (spec: infer TSpec) => unknown ? TSpec : never,
                params: CreateAcpRuntimeParamsV1,
            ) => Promise<PluginContextV1['acp']['createRuntime'] extends (...args: readonly unknown[]) => Promise<infer THandle> ? THandle : never>>();
    });
    ...
    it('exposes structured agent CLI readiness without widening exec system-tool grants', () => {
        expectTypeOf<PluginContextV1['agents']>()
            .toEqualTypeOf<AgentsRuntimeServiceV1>();
        expectTypeOf<PluginContextV1['agents']['cli']['checkReadiness']>()

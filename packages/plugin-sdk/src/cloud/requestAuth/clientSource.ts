...
export class ConnectedAccountRequestAuthTransportError extends Error {
...
export async function lookupConnectedAccountRequestAuth(input) {
...
export async function reportConnectedAccountAuthFailure(input) {
...
export async function reportConnectedAccountQuotaFailure(input) {
  if (!requestAuthHasExactKeys(input, ["credentialContext", "normalizedFailure"], ["signal"])) {
    throw new Error("happier_request_auth_schema_invalid");
  }
  const body = {

...
export function assertServerComponentDirMatches({ rootDir, serverComponentName, serverDir }) {
  ...
  const hint =
    mismatch.expected === 'happy-server-light'
      ? 'Fix: either switch flavor (`hapsta srv use happy-server`) or switch the active checkout for happy-server-light (`hapsta wt use happy-server-light default` or a worktree under .worktrees/happy-server-light/).'
      : 'Fix: either switch flavor (`hapsta srv use happy-server-light`) or switch the active checkout for happy-server (`hapsta wt use happy-server default` or a worktree under .worktrees/happy-server/).';

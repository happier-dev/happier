...
describe('Azure DevOps detail UI build configuration', () => {
  it('builds the exact artifact the manifest renderer declares', async () => {
    ...
    expect(config).toMatchObject({
      ...
      targets: [{
        rendererId: AZURE_DEVOPS_TRIAGE_DETAIL_ARTIFACT_ID,
        entry: 'src/ui/renderSurface.tsx',
        kind: 'reactNative',
        platforms: ['web', 'ios', 'android'],
      }, {
        rendererId: AZURE_DEVOPS_TRIAGE_SETTINGS_ARTIFACT_ID,
  ...
  it('mounts the built artifact for the detail surface the Triage contribution declares', () => {
    ...
    expect(declared).toContain(AZURE_DEVOPS_TRIAGE_DETAIL_RENDERER_ID);
  });
  ...
  it('declares one reachable Settings page bound to the settings renderer', () => {
    ...
    expect(PLUGIN_MANIFEST.contributes.ui.settingsPages[0].group)
      .toEqual({ kind: 'plugin', localId: AZURE_DEVOPS_TRIAGE_SETTINGS_GROUP_ID });
    expect(PLUGIN_MANIFEST.contributes.ui.settingsGroups)
      .toEqual([expect.objectContaining({ id: AZURE_DEVOPS_TRIAGE_SETTINGS_GROUP_ID })]);
  });
});

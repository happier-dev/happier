...
const PURPOSE = 'azure-devops-account-use';
...
function binding(accountId: string): JsonValue {
  return {
    purpose: PURPOSE,
    account: {
      service: { pluginId: AZURE_DEVOPS_PLUGIN_ID, localId: 'azure-devops-account' },
      accountId,
    },
  };
}
...
function createHarness(options: Readonly<{
  ...
  async function executeAction(
    ...
    if (ref.localId === AZURE_DEVOPS_TRIAGE_ACTION_IDS.listInstances) return options.discovery;
    if (
...
describe('the mounted Azure DevOps PRs & Issues settings page', () => {
  ...
  it('never asks the target to configure anything merely by opening the page', async () => {
    ...
    expect(harness.recorded.map((entry) => (entry.action as Readonly<{ localId?: string }>).localId))
      .toEqual([AZURE_DEVOPS_TRIAGE_ACTION_IDS.listInstances]);
  });
  ...
  it('reports a source that could not be reached rather than an empty list', async () => {
    ...
    await expect(page.getByText('Azure DevOps could not be read'))
      .resolves.toEqual({ content: 'Azure DevOps could not be read' });
    await expect(page.queryByText('No Azure DevOps scopes to add')).resolves.toBeUndefined();
  });
  ...
  it('tells the user a source the host no longer admits cannot be configured', async () => {
    ...
    await expect(page.getByText(
      'Azure DevOps is no longer an admitted PRs & Issues source on this account.',
    )).resolves.toBeDefined();
  });
});

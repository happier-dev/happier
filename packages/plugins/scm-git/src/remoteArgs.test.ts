...
describe('normalizeScmRemoteRequest', () => {
    ...
    it('accepts slash-delimited remote names for legacy fetch/pull/push requests', () => {
        expect(normalizeScmRemoteRequest({ remote: ' fork/alice ', branch: ' feature/x ' } as any)).toEqual({
            ok: true,
            request: {
                remote: 'fork/alice',
                branch: 'feature/x',

...
import { ensureGitWorktreeMaterialized } from './ensureGitWorktreeMaterialized.js';
import { repairGitWorktreeAdminReference } from './repairGitWorktreeAdminReference.js';
import { isGitWorkspaceTransferMetadata } from '../workspaceTransferMetadata.js';
...
export async function reconcileGitWorkspaceCheckout(input: Readonly<{
    ...
    const sourcePath = typeof input.sourcePath === 'string' ? input.sourcePath.trim() : '';
    const sourceMetadata = isGitWorkspaceTransferMetadata(input.workspaceIntegrationMetadata)

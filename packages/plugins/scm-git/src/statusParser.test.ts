...
describe('git status snapshot parser', () => {
    ...
    it('parses configured remotes from git remote verbose output', () => {
        const statusOutput =
            '# branch.oid 1111111111111111111111111111111111111111\0' +
            '# branch.head main\0';
        const input = {
            ...
            pendingNumStatOutput: '',
            remotesOutput:
                'upstream\tgit@example.com:upstream.git (fetch)\n' +
                'upstream\tssh://push.example.com/upstream.git (push)\n' +
                'origin\tgit@example.com:origin.git (fetch)\n' +
                'origin\tgit@example.com:origin.git (push)\n',
        };
        ...
        expect(snapshot.repo).toMatchObject({
            remotes: [
                {
                    name: 'upstream',
                    fetchUrl: 'git@example.com:upstream.git',
                    pushUrl: 'ssh://push.example.com/upstream.git',
                },
                {
                    name: 'origin',
                    fetchUrl: 'git@example.com:origin.git',
                    pushUrl: 'git@example.com:origin.git',
                },
            ],
        });
    });
    ...
    it('projects the default branch from the upstream remote HEAD ref', () => {
        ...
        expect(snapshot.repo.defaultBranch).toBe('release/2026');
    });

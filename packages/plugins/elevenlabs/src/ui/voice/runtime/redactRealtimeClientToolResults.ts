...
export type { VoiceToolResultRedactionPrefs } from './types.js';
export type VoiceToolResultRedactor = (value: unknown, prefs: VoiceToolResultRedactionPrefs) => unknown;
...
export function redactRealtimeClientToolResultString(
    ...
    redactValue: VoiceToolResultRedactor,
): string {
    try {
        let parsed: unknown;
        try {
            ...
            const redactedScalar = redactValue(rawResult, prefs);
            return typeof redactedScalar === 'string' ? redactedScalar : JSON.stringify(redactedScalar);
        }
        return JSON.stringify(redactValue(parsed, prefs));
    } catch {
...
export function redactRealtimeClientToolResults(
    ...
    resolvePrefs: () => VoiceToolResultRedactionPrefs,
    redactValue: VoiceToolResultRedactor,
    ...
    for (const [toolName, handler] of Object.entries(clientTools)) {
        wrapped[toolName] = async (parameters: unknown): Promise<string> => {
            ...
            return redactRealtimeClientToolResultString(rawResult, resolvePrefs(), redactValue);
        };
    }
    return wrapped;
}

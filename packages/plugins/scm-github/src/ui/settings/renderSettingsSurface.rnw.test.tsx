...
import { GITHUB_TRIAGE_ACTION_IDS_V1 } from '../../triage/contribution.js';
...
const PURPOSE = 'github-connected-account';
...
function createHarness(options: Readonly<{
  ...
  async function executeAction(
    ...
    if (ref.localId === GITHUB_TRIAGE_ACTION_IDS_V1.listInstances) return options.discovery;
    if (
...
describe('the mounted GitHub PRs & Issues settings page', () => {
  ...
  it('never asks the target to configure anything merely by opening the page', async () => {
    ...
    expect(harness.recorded.map((entry) => (entry.action as Readonly<{ localId?: string }>).localId))
      .toEqual([GITHUB_TRIAGE_ACTION_IDS_V1.listInstances]);
  });
  ...
  it('reports a source that could not be reached rather than an empty list', async () => {
    ...
    await expect(page.getByText('GitHub could not be read'))
      .resolves.toEqual({ content: 'GitHub could not be read' });
    await expect(page.queryByText('No GitHub scopes to add')).resolves.toBeUndefined();
  });
  ...
  it('tells the user a source the host no longer admits cannot be configured', async () => {
    ...
    await expect(page.getByText(
      'GitHub is no longer an admitted PRs & Issues source on this account.',
    )).resolves.toBeDefined();
  });
});

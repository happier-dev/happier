...
/**
 * GitHub's binding of the shared forge paged-panel state machine.
 *
 ...
 * all four forges and lives at `@happier-dev/scm-forge-adapter`, because four
 * copies are four places for "nothing here" to start looking like "we could not
...
export type GithubPagedStateV1<TRow> = ForgePagedStateV1<
  TRow,
  TriageSourceFailureV1,
  GithubDetailIncompleteReasonV1
>;
export type GithubPagedPageV1<TRow> = ForgePagedPageV1<TRow, GithubDetailIncompleteReasonV1>;
export type GithubPagedEventV1<TRow> = ForgePagedEventV1<
  TRow,
  TriageSourceFailureV1,
  GithubDetailIncompleteReasonV1
>;
...
export function githubPagedInitialState<TRow>(): GithubPagedStateV1<TRow> {
  return forgePagedInitialState<TRow, TriageSourceFailureV1, GithubDetailIncompleteReasonV1>();
}
...
export function githubPagedReducer<TRow>(
  ...
  return forgePagedReducer<TRow, TriageSourceFailureV1, GithubDetailIncompleteReasonV1>(
    state,
    event,
  );
}

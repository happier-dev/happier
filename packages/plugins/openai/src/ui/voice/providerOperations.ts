/** Provider-owned OpenAI request construction and bounded response parsing. */
import {
...
import { OpenAiRealtimeClientAuthResponseSchema } from '../../protocol/voice/clientAuth.js';
...
const AudienceSchema = z.object({
  ...
  instructions: z.string().trim().max(16_384).nullable().optional(),
  turnDetection: z.enum(['server_vad', 'semantic_vad', 'manual']).optional(),
  inputTranscriptionModel: z.string().trim().min(1).max(128).nullable().optional(),
}).strict();
  ...
  const audioInput: Record<string, unknown> = {
    turn_detection: settings.turnDetection === 'manual'
      ...
      : {
          ...
          // an in-flight answer before the backchannel/false-alarm gate.
          interrupt_response: false,
        },
    ...(settings.inputTranscriptionModel
      ? { transcription: { model: settings.inputTranscriptionModel } }
      : {}),
  };
...
function parseClientAuthArtifact(
  ...
  const parsed = OpenAiRealtimeClientAuthResponseSchema.safeParse(value);
  if (!parsed.success) {
    throw providerError('provider_response_invalid');
  }

...
/**
 * GitLab's binding of the shared Triage paged-panel state machine.
 *
 ...
 * Triage source and lives at `@happier-dev/triage-protocol`. What GitLab
...
export type GitlabPagedStateV1<TRow> = TriagePagedPanelStateV1<
  TRow,
  TriageSourceFailureV1,
  GitlabDetailIncompleteReasonV1
>;
export type GitlabPagedPageV1<TRow> = TriagePagedPanelPageV1<TRow, GitlabDetailIncompleteReasonV1>;
export type GitlabPagedEventV1<TRow> = TriagePagedPanelEventV1<
  TRow,
  TriageSourceFailureV1,
  GitlabDetailIncompleteReasonV1
>;
...
export function gitlabPagedInitialState<TRow>(): GitlabPagedStateV1<TRow> {
  return triagePagedPanelInitialState<TRow, TriageSourceFailureV1, GitlabDetailIncompleteReasonV1>();
}
...
export function gitlabPagedReducer<TRow>(
  ...
  return triagePagedPanelReducer<TRow, TriageSourceFailureV1, GitlabDetailIncompleteReasonV1>(
    state,
    event,
  );
}

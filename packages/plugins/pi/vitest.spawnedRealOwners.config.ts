...
const preferWorkspaceTypeScriptSources = {
  name: 'happier-pi-spawned-real-owners-typescript-sources',
  ...
  load(id: string) {
    ...
    if (
      ...
      throw new Error(
        `Spawned real-owner proof must not load an emitted CLI source sibling: ${sourcePath}`,
      );
    }
    return null;
  },
...
export default defineConfig({
  test: {
    ...
    minWorkers: 1,
    include: [
      'src/agent/auth/services/requestAuth/source.spawnedRealOwners.test-support.ts',
    ],
    testTimeout: 60_000,
    hookTimeout: 60_000,
  },

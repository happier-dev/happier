...
export function createAntigravityConnectedServiceRuntimeAuthAdapter(): AgentConnectedAccountRuntimeAuthAdapterV1 {
  return {
    ...
    async verifyActiveAccount() {
      return { status: 'unavailable' as const, retryable: true, reason: 'antigravity_provider_outcome_pending' };
    },
    async verifyProviderOutcome(input: Readonly<{ selections?: unknown; outcome?: unknown }>) {
      ...
      if (outcome?.kind !== 'provider_activity' || outcome.event !== 'assistant_message_end') {
        return { status: 'unavailable' as const, reason: 'antigravity_provider_activity_required' };
      }
      ...
      if (targets.length !== 1) {
        return { status: 'unavailable' as const, reason: 'antigravity_exact_epoch_required' };
      }
      return { status: 'verified' as const, source: 'antigravity_provider_activity', targets };
    },

 ...
 * the EFFECTIVE settings of the spawned config root (`CLAUDE_CONFIG_DIR`, then the Happier-managed
 * `HAPPIER_CLAUDE_CONFIG_DIR` fallback) and carried
...
export function resolveClaudeStatuslineOriginalCommand(params: Readonly<{
    ...
    try {
        const configRoot = resolveClaudeConfigDir(params.env);

/**
 * Claude extended-context (1M) facts and context-window token resolution.
 *
 ...
 * Agent SDK `model` option. Happier never mutates the suffixed id on a send path; the
 ...
 * - Always-1M models resolve 1M even with a BASE id — hooks/JSONL report the base id.
 * - Everything else resolves null: the caller decides between catalog facts and the
 *   200k default.
 */
 ...
 * assumption is stale (e.g. the user enabled 1M provider-side without Happier knowing).
 * Bump to the smallest known window that fits the observation; beyond every known
 * window, trust the observation itself so consumers never compute >100% usage.
 */

...
const PURPOSE = 'sentry-account-use';
...
function createHarness(options: Readonly<{
  ...
  async function executeAction(
    ...
    if (ref.localId === SENTRY_ACTION_IDS.listInstances) return options.discovery;
    if (
...
describe('the mounted Sentry PRs & Issues settings page', () => {
  it('lists what this source can reach and configures the one the user chose', async () => {
    const harness = createHarness({
      discovery: { kind: 'complete', candidates: [candidate('acme-org')], failures: [] },
    });
    const page = await mountSettings(harness);

    await expect(page.getByText('acme-org')).resolves.toEqual({ content: 'acme-org' });

    await pressControl(page, 'Add acme-org to PRs & Issues');
    ...
    expect(administrations[0]).toMatchObject({
      v: 1,
      kind: 'create',
      draft: { binding: binding('account-1'), localInstanceKey: 'acme-org' },
    });
  ...
  it('never asks the target to configure anything merely by opening the page', async () => {
    ...
    expect(harness.recorded.map((entry) => (entry.action as Readonly<{ localId?: string }>).localId))
      .toEqual([SENTRY_ACTION_IDS.listInstances]);
  });
  ...
  it('offers no exact-row control until the target has named the row', async () => {
    ...
    await expect(page.queryByRole('button', { name: 'Remove acme-org from PRs & Issues' }))
      .resolves.toBeUndefined();
    await expect(page.queryByRole('button', { name: 'Update acme-org from the provider' }))
      .resolves.toBeUndefined();
    await expect(page.queryByRole('button', { name: 'Restore acme-org to PRs & Issues' }))
      .resolves.toBeUndefined();
  });
  ...
  it('takes a configured source back out again, and can bring it back', async () => {
    ...
    await pressControl(page, 'Add acme-org to PRs & Issues');
    await pressControl(page, 'Update acme-org from the provider');
    await pressControl(page, 'Remove acme-org from PRs & Issues');
    ...
    await pressControl(page, 'Restore acme-org to PRs & Issues');
  ...
  it('reports a source that could not be reached rather than an empty list', async () => {
    ...
    await expect(page.getByText('Sentry could not be read'))
      .resolves.toEqual({ content: 'Sentry could not be read' });
    await expect(page.queryByText('No Sentry scopes to add')).resolves.toBeUndefined();
  });
  ...
  it('tells the user a source the host no longer admits cannot be configured', async () => {
    ...
    await expect(page.getByText(
      'Sentry is no longer an admitted PRs & Issues source on this account.',
    )).resolves.toBeDefined();
  });
});

...
describe('Sentry plugin manifest', () => {
  ...
  it('has its scan purpose binding enforced, not merely tolerated', () => {
    ...
    // The walker runs at manifest ingest, which is what a host puts every
    // manifest through before it may contribute anything. `definePlugin` is an
    // authoring projector that validates no contribution schema, so probing it
    // for a throw observed nothing and could not fail.

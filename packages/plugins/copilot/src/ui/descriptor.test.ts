...
import { COPILOT_UI_DESCRIPTOR } from './descriptor.js';
...
describe('COPILOT_UI_DESCRIPTOR', () => {
  it('is a versioned plugin UI descriptor', () => {
    expect(COPILOT_UI_DESCRIPTOR).toMatchObject({
      kind: 'plugin.ui.v1',
      pluginId: 'copilot',
      agentId: 'copilot',
      version: 1,
  ...
  it('is a data-only no-execute descriptor', () => {
    expect(collectNoExecuteViolations(COPILOT_UI_DESCRIPTOR)).toEqual([]);
    expect(JSON.parse(JSON.stringify(COPILOT_UI_DESCRIPTOR))).toEqual(COPILOT_UI_DESCRIPTOR);
  });
});

...
import { KIMI_UI_DESCRIPTOR } from './descriptor.js';
...
describe('KIMI_UI_DESCRIPTOR', () => {
  it('is a versioned plugin UI descriptor', () => {
    expect(KIMI_UI_DESCRIPTOR).toMatchObject({
      kind: 'plugin.ui.v1',
      pluginId: 'kimi',
      agentId: 'kimi',
      version: 1,
  ...
  it('is a data-only no-execute descriptor', () => {
    expect(collectNoExecuteViolations(KIMI_UI_DESCRIPTOR)).toEqual([]);
    expect(JSON.parse(JSON.stringify(KIMI_UI_DESCRIPTOR))).toEqual(KIMI_UI_DESCRIPTOR);
  });
});

...
describe('provider harness step gating', () => {
  ...
  test('accepts a newly completed canonical session turn when native ACP omits legacy task_complete', () => {
    expect(readCompletedTurnId({
      latestTurnId: 'turn-native-1',
      latestTurnStatus: 'completed',
    })).toBe('turn-native-1');
    expect(readCompletedTurnId({
      latestTurnId: 'turn-native-2',
    ...
    expect(
      shouldEnqueueNextStepAfterSatisfaction({
        ...
        decodedMessagesSeen: [],
        taskCompleteCountAtStepSatisfaction: 0,
        completedTurnIdAtStepStart: 'turn-native-1',
  ...
  test('does not mistake the prior completed turn for the current step boundary', () => {
    expect(
      shouldEnqueueNextStepAfterSatisfaction({
        providerProtocol: 'acp',
        traceEvents: [],

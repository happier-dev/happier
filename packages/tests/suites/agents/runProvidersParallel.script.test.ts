...
describe('providers parallel scenario registry filtering', () => {
  ...
  it('does not fall back to the full provider list when an explicit scenario matches no registry', async () => {
    const filtered = await filterProviderIdsByScenarioRegistry({
      providerIds: ['qwen', 'kilo', 'codex'],
      tier: 'extended',
      scenarioSelectionRaw: 'scenario_not_declared_by_any_provider',
    });

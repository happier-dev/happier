import { lstatSync, mkdtempSync, mkdirSync, rmSync, utimesSync, writeFileSync } from 'node:fs';
...
} from '../../src/testkit/process/workspacePackageResolution';
...
function writeCliBundledHostManifest(repoRoot: string) {
...
function writeSharedDepsOutputs(repoRoot: string) {
  writeCliBundledHostManifest(repoRoot);
  const outputs = CLI_SHARED_DEP_TEST_FIXTURE_PACKAGE_NAMES.map((packageName) =>
...
describe('providers: shared deps build lock', () => {
  ...
  it('rebuilds missing CLI-bundled workspace dist outputs through the canonical publisher', async () => {

...
import {
  ...
  scenarioCatalog,
  waitForToolCallPayloadSubstring,
} from '../../src/testkit/providers/scenarios/scenarioCatalog';
...
describe('scenarioCatalog: abort continuation contract', () => {
  it('waits for the intended tool call instead of relying on a fixed delivery delay', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'happier-abort-tool-trace-'));
    const traceFile = join(dir, 'tooltrace.jsonl');
    await writeFile(traceFile, '');
    try {
      const waiting = waitForToolCallPayloadSubstring({
        traceFile,
        requiredSubstring: 'ABORT_MEMORY_target',
        timeoutMs: 1_000,
        pollMs: 10,
      });
      await appendFile(traceFile, `${JSON.stringify({
        kind: 'tool-result',
        payload: { output: 'ABORT_MEMORY_target' },
      })}\n`);
      await appendFile(traceFile, `${JSON.stringify({
        kind: 'tool-call',
        payload: { name: 'Bash', input: { command: 'echo ABORT_MEMORY_target && sleep 20' } },
      })}\n`);

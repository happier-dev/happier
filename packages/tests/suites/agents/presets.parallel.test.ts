...
describe('providers: parallel preset helpers', () => {
  ...
  it('resolves a single provider for specific preset', () => {
    ...
    expect(resolveProviderPresetIds('grok')).toEqual(['grok']);
    expect(resolveProviderPresetIds('cursor_acp_stub')).toEqual(['cursor_acp_stub']);
  });

  it('selects only Grok in the serial runner environment', () => {
    const preset = resolveProviderRunPreset('grok', 'smoke');

    expect(preset).not.toBeNull();
    expect(preset?.env.HAPPIER_E2E_PROVIDER_GROK).toBe('1');
    expect(preset?.env.HAPPIER_E2E_PROVIDER_PI).toBe('0');
  });

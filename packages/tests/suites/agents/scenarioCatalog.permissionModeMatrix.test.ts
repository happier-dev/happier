...
describe('scenarioCatalog: ACP permission mode matrix scenarios', () => {
  it('builds mode-scoped outside-workspace scenarios with permission expectations', () => {
    ...
    const modeIds = [
      ...
      'permission_mode_plan_outside_workspace',
    ...
    const planScenario = scenarioCatalog.permission_mode_plan_outside_workspace(provider);
    ...
    expect(resolveMeta(planScenario).permissionMode).toBe('plan');
    ...
    const planKeys = (planScenario.requiredAnyFixtureKeys ?? []).flat();
    ...
    expect(planKeys.some((key) => key.includes('/permission-request/'))).toBe(false);
    ...
    expect(planScenario.permissionAutoDecision).toBe('denied');
    ...
    expect(planScenario.yolo).toBe(false);

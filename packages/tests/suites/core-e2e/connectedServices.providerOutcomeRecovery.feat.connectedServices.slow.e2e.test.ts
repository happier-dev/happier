...
describe('core e2e: connected-service provider-outcome recovery', () => {
  ...
  it('creates durable Claude group 401 recovery before refresh, retries the original prompt once, and clears on provider activity', async () => {
    ...
    await waitFor(async () => {
      const latestAttempts = await readSessionContinuationRecoveryAttempts({
        fixture: claudeFixture!,
        sessionId,
      });
      return latestAttempts.some((attempt) => attempt.status === 'provider_activity_observed');
    }, {
      timeoutMs: 90_000,
      intervalMs: 250,
      context: 'Claude recovery replay observes provider activity after refresh',
    });
    const observedAttempts = await readSessionContinuationRecoveryAttempts({ fixture: claudeFixture, sessionId });
    expect(observedAttempts).toHaveLength(1);
    expect(observedAttempts[0]).toMatchObject({
    ...
    expect(typeof observedAttempts[0]?.sentAtMs).toBe('number');

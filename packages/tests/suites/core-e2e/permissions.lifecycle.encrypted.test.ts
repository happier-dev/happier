...
describe('core e2e: permission lifecycle (encrypted rpc + agentState) + reconnect', () => {
  ...
  it('preserves structured AskUserQuestion answers through the encrypted session RPC round trip', async () => {
    const testDir = run.testDir('user-action-lifecycle-encrypted');
    const saveArtifactsOnSuccess = envFlag(['HAPPIER_E2E_SAVE_ARTIFACTS', 'HAPPY_E2E_SAVE_ARTIFACTS'], false);
    const startedAt = new Date().toISOString();
    ...
    writeTestManifestForServer({
      ...
      startedAt,
      runId: run.runId,
      testName: 'user-action-lifecycle-encrypted',
      sessionIds: [sessionId],
      env: {
        CI: process.env.CI,
        HAPPIER_E2E_SAVE_ARTIFACTS: process.env.HAPPIER_E2E_SAVE_ARTIFACTS ?? process.env.HAPPY_E2E_SAVE_ARTIFACTS,
    ...
    try {
      deviceA.connect();
      await agent.start();
      await waitFor(() => deviceA.isConnected(), { timeoutMs: 20_000 });
      ...
      const uiRpc = createDataKeyRpcClient(deviceA, dataKey);
      const rpcRes = await uiRpc.call(`${sessionId}:permission`, {
        id: requestId,
        approved: true,
      ...
      await agent.waitForCompletedPermission(requestId, { timeoutMs: 15_000 });
      ...
      const requests = (agentState as any).requests ?? {};
      const completed = (agentState as any).completedRequests ?? {};
      expect(requests[requestId]).toBeUndefined();
      expect(completed[requestId]?.status).toBe('approved');
      expect(completed[requestId]?.answers).toEqual({ [question]: answer });

...
const ACP_TEST_RUNTIME = resolveAcpSdkTestRuntime(repoRootDir());
...
const adapterEntry = process.env.HAPPIER_E2E_ACP_AGENT_APP_ADAPTER_ENTRY;
...
connectAcpTestAgentApp({
  acp,
  stream,
  name: "happier-permission-metadata-e2e-agent",
  createAgent: (connection) => new FakeAgent(connection),
});
`,
      'utf8',
    );
    ...
    const cliEnv: NodeJS.ProcessEnv = {
      ...
      HAPPIER_E2E_ACP_SDK_ENTRY: ACP_TEST_RUNTIME.sdkEntry,
      HAPPIER_E2E_ACP_AGENT_APP_ADAPTER_ENTRY: ACP_TEST_RUNTIME.agentAppAdapterEntry,

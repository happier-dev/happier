...
describe('core e2e: channel bridges foundation', () => {
  ...
  it('advertises telegram gate as disabled when the server env gate is disabled', async () => {
    const testDir = run.testDir('feature-negotiation-channel-bridges-telegram-disabled');
    server = await startServerLight({
      testDir,
      dbProvider: 'sqlite',
      extraEnv: {

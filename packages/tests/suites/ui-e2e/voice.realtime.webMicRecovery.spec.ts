...
import {
    VOICE_SURFACE_E2E_FIXTURE_REALTIME_RECOVERABLE_DISCONNECT,
    reachSidebarVoiceSurfaceFixtureHome,
} from '../../src/testkit/uiE2e/voiceSurfaceFixture';

const run = createRunDirs({ runLabel: 'ui-e2e' });

test.describe('ui e2e: voice realtime web mic recovery', () => {
    test.describe.configure({ mode: 'serial' });

    const suiteDir = run.testDir('voice-realtime-web-mic-recovery-suite');
    ...
    test.beforeAll(async () => {
        storageScope = `e2e-voice-web-mic-recovery-${run.runId}`;
        const uiWebEnv = {
            ...process.env,
            EXPO_PUBLIC_DEBUG: '1',
    ...
    test('keeps the sidebar voice surface startable after a recoverable realtime disconnect', async ({ page }) => {
        ...
        await reachSidebarVoiceSurfaceFixtureHome({
            ...
            storageScope,
            fixtureId: VOICE_SURFACE_E2E_FIXTURE_REALTIME_RECOVERABLE_DISCONNECT,
        });

        await expect(page.getByTestId('voice-surface:sidebar')).toHaveCount(1, { timeout: 120_000 });
        await expect(page.getByTestId('voice-surface-status:sidebar:disconnected')).toHaveCount(1, {
            timeout: 120_000,
        });

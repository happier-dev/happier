...
import {
    ...
    VOICE_SURFACE_E2E_FIXTURE_DAEMON_INFERENCE_WEB_FALLBACK_PARITY,
    reachSidebarVoiceSurfaceFixtureHome,
} from '../../src/testkit/uiE2e/voiceSurfaceFixture';

const run = createRunDirs({ runLabel: 'ui-e2e' });

test.describe('ui e2e: voice daemon inference web fallback parity', () => {
    test.describe.configure({ mode: 'serial' });

    const suiteDir = run.testDir('voice-daemon-inference-web-fallback-parity-suite');
    ...
    test.beforeAll(async () => {
        storageScope = `e2e-voice-daemon-web-fallback-${run.runId}`;
        const uiWebEnv = {
            ...process.env,
            EXPO_PUBLIC_DEBUG: '1',
        ...
        server = await startServerLight({
            ...
            extraEnv: {
                ...
                HAPPIER_FEATURE_VOICE__DAEMON_INFERENCE__ENABLED: '1',
            },
        });
    ...
    test('keeps the sidebar hidden-conversation parity intact for web-clamped daemon local-neural fallback settings', async ({ page }) => {
        ...
        await reachSidebarVoiceSurfaceFixtureHome({
            ...
            storageScope,
            fixtureId: VOICE_SURFACE_E2E_FIXTURE_DAEMON_INFERENCE_WEB_FALLBACK_PARITY,
        });
        ...
        await expect(page.getByTestId('voice-surface-open:sidebar')).toHaveCount(1, { timeout: 120_000 });

        await page.getByTestId('voice-surface-open:sidebar').click();

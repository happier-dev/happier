...
import {
    VOICE_SURFACE_E2E_FIXTURE_WELCOME_RESUME_PERSISTENCE,
...
test.describe('ui e2e: voice welcome resume persistence', () => {
    test.describe.configure({ mode: 'serial' });

    const suiteDir = run.testDir('voice-welcome-resume-persistence-suite');
    ...
    test.beforeAll(async () => {
        storageScope = `e2e-voice-welcome-resume-${run.runId}`;
        const uiWebEnv = {
            ...process.env,
            EXPO_PUBLIC_DEBUG: '1',
    ...
    test('keeps the persisted welcome transcript stable across browser reload and hidden-conversation reopen', async ({ page }) => {
        ...
        await reachSidebarVoiceSurfaceFixtureHome({
            ...
            storageScope,
            fixtureId: VOICE_SURFACE_E2E_FIXTURE_WELCOME_RESUME_PERSISTENCE,
        });

        await expect(page.getByTestId('voice-surface:sidebar')).toHaveCount(1, { timeout: 120_000 });
        await expect(page.getByTestId('voice-surface-activity-count:sidebar')).toHaveText('1', { timeout: 120_000 });
        await page.getByTestId('voice-surface-activity-toggle:sidebar').click();
        await expect(page.getByTestId('voice-surface-activity-entry:sidebar:voice-e2e-welcome-1')).toHaveCount(1, { timeout: 120_000 });
        ...
        await expect(transcriptChatList.getByText('Welcome back')).toHaveCount(1, { timeout: 120_000 });
    });
});

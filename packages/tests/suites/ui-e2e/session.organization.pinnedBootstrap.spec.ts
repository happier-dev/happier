...
async function expectNoSessionListRequestUsesPinnedIds(captures: readonly SessionListCapture[]): Promise<void> {
  await expect.poll(() => captures.some((capture) => capture.hasPinnedSessionIdsParam), {
    timeout: 5_000,
  }).toBe(false);
}
...
test.describe('ui e2e: session organization pinned bootstrap', () => {
  ...
  test('loads more than 100 server-backed pins in the pinned section from the initial session list response', async ({ page }) => {
    ...
    await expectNoSessionListRequestUsesPinnedIds(initialListCaptures);

...
test.describe('ui e2e: unauth split shell', () => {
  test.describe.configure({ mode: 'serial' });

  const suiteDir = run.testDir('unauth-split-shell-suite');
  ...
  const uiWebEnv = {
    ...
    EXPO_PUBLIC_HAPPY_STORAGE_SCOPE: `e2e-${run.runId}-unauth-split-shell`,
  ...
  test('mobile first launch shows brand hero, then welcome, then persists across reload', async ({ page }) => {
    test.setTimeout(240_000);
  ...
  test('desktop login and self-hosting entries stay inside the split shell', async ({ page }) => {
    test.setTimeout(240_000);
    await page.setViewportSize({ width: 1100, height: 720 });
    ...
    await expect(page.getByTestId('unauth-shell-workflow-pane')).toBeVisible();
    await expect(page.getByTestId('restore-open-manual')).toBeVisible();
    ...
    await expect(page.getByTestId('unauth-shell-brand-pane')).toBeVisible({ timeout: 120_000 });
    await expect(page.getByTestId('unauth-shell-workflow-pane')).toBeVisible();
    await expect(page.getByTestId('onboarding-wizard-relay-diagram')).toBeVisible();
  });

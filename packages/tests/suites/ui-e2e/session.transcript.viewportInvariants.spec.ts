...
/**
 * Transcript viewport single-owner invariants (port plan W4.1 / G2; invariants A/B/D/E/H)
 * asserted against the live web telemetry buffer `__HAPPIER_TRANSCRIPT_VIEWPORT_EVENTS__()`.
 *
 * Assertion semantics mirror `apps/ui/sources/dev/testkit/transcript/viewportTelemetryAssertions.ts`.
 * They are replicated minimally here because `@happier-dev/tests` cannot import `apps/ui` modules
 * (different package; tsconfig `@/*` maps to `apps/cli/src`).
 *
 * Web-specific deltas vs the UI harness:
 * - `scroll-observed` telemetry is native-only in ChatList, so "final write confirmed" is
...
// Mirrors WRITE_REASON_OWNERS in the UI viewport-telemetry assertion harness.
...
// Mirrors TRANSACTION_OUTCOME_REASONS in the UI harness (no-TTL prepend design: there is no
// `abandoned-ttl` anywhere in the vocabulary).
...
// Mirrors TERMINAL_DECISION_REASONS in the UI harness, EXTENDED for web-trace semantics:
...
// Mirrors DEFAULT_PIN_THRESHOLD_PX in the UI harness.
...
// guarantees a real older page (~30 messages) remains after the cold open. The prepend scenario
// premise guard below fails LOUDLY if the entry phase drains that backlog (G-RV rework: seeding
// inside the cap makes prepend scenarios structurally impossible).
...
async function installViewportTelemetryOverride(page: Page): Promise<void> {
  ...
  // post-load evaluate override is wiped by the next `page.goto`.
...
test.describe('ui e2e: transcript viewport invariants', () => {
  ...
  test('runtime guard: requestAnimationFrame ticks within 400ms', async ({ page }) => {
    ...
    if (!rafTicked) {
      throw new Error([
        'RENDERING-PAUSED TAB TRAP: requestAnimationFrame did not tick within 400ms.',
  ...
  test('seeds a session past the initial-fetch cap and viewport telemetry is readable', async ({ page }) => {
    ...
    // the 150-message initial fetch so older-page prepends stay reachable (see the
    // SEED_TURN_COUNT comment).
  ...
  function assertColdOpenWriteBudget(snapshot: ViewportTelemetrySnapshot): void {
    // The quiescence-settled trace inevitably extends past the entry phase into steady-state
    // follow. Invariant A governs the ENTRY window: entry-owned writes stay within
    // {initial-open, mount-settle} and the ≤2 budget; zero prepend/explicit writes; any follow
    // writes after entry must keep the viewport pinned at the bottom (invariant F shape — late
    // DOM growth repins are follow-owned and bottom-bound, never an entry re-issue storm).
  ...
  test('warm reopen restores the anchor with one bounded entry transaction (invariant B)', async ({ page }) => {
    ...
    // legitimately shifts absolute offsets while the user-visible anchor stays put.
    ...
    // the whole seeded backlog and destroy the prepend scenario's premise.
  ...
  test('older-page prepend: loading indicator, ≤1 page in flight, one transaction outcome (invariants D/H)', async ({ page }) => {
    ...
    // Invariant D on WEB (Lane E2 contract): the native prepend transaction does not run on web —
    // the KEEP web prepend system emits restore-decision events ('pending' on capture, closed by
    // a terminal reason). Assert never-silent via no-silent-bails instead of counting native
    // transaction outcomes (scenario-D outcome counting is native-only).

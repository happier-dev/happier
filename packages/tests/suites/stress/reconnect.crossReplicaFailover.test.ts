...
import { runCrossReplicaReconnectScenario } from '../../src/testkit/stress/scenarios/runCrossReplicaReconnectScenario';
...
describe('stress: cross-replica reconnect failover', () => {
  ...
  it('kills the connected api replica, reconnects onto another replica, and restores session-scoped rpc behavior', async () => {
    if (!target) return;
    await runCrossReplicaReconnectScenario({
      run,
      target,
      config,
      token,

...
import { runRedisInterruptionScenario } from '../../src/testkit/stress/scenarios/runRedisInterruptionScenario';
...
describe('stress: redis interruption', () => {
  ...
  it('temporarily interrupts redis and validates rpc, reconnect, and transcript recovery afterward', async () => {
    if (!target) return;
    await runRedisInterruptionScenario({
      run,
      target,
      config,
      token,

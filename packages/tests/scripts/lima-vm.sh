...
# Create/configure a Lima VM for running Happier stack flows in a clean Linux guest,
# while keeping Expo web URLs openable on the host via localhost port forwarding
# (required for WebCrypto/secure-context APIs).
#
# Usage (Darwin/Linux host):
#   ./packages/tests/scripts/lima-vm.sh [vm-name]
#
# Env overrides:
...
Examples:
  ./packages/tests/scripts/lima-vm.sh
  ./packages/tests/scripts/lima-vm.sh happy-e2e

Notes:
- Run on macOS (Darwin) or Linux host.
- Configures localhost port forwarding so you can open http://localhost / http://*.localhost
  in your host browser (required for WebCrypto APIs used by Expo web).
EOF
}
...
if [[ "${HOST_OS}" != "Darwin" && "${HOST_OS}" != "Linux" ]]; then
  echo "[lima] expected macOS (Darwin) or Linux host; got: ${HOST_OS}" >&2
  exit 1
fi

if ! command -v limactl >/dev/null 2>&1; then
  echo "[lima] limactl not found. Install Lima first." >&2
  exit 1
fi
...
Tips:
  - Open printed URLs on your host via http://localhost:<port> or http://*.localhost:<port>.

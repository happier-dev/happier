...
export async function activateLinkedDirectSession(params: Readonly<{
  ...
  unwrapDataKeyRpcResult(
    await params.machineRpc.call(`${params.machineId}:${RPC_METHODS.DAEMON_DIRECT_SESSION_TAKEOVER}`, {
  ...
  await waitFor(async () => {
    const status = unwrapDataKeyRpcResult(
      await params.machineRpc.call(`${params.machineId}:${RPC_METHODS.DAEMON_DIRECT_SESSION_STATUS_GET}`, {
        machineId: params.machineId,
        sessionId: params.sessionId,

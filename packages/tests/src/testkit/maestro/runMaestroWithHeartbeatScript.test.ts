...
describe('scripts/run-maestro-with-heartbeat.mjs', () => {
    ...
    it('uses the tsx import hook instead of the tsx CLI wrapper', async () => {
        const repoRoot = resolve(__dirname, '../../../../..');
        ...
        await writeFile(
            ...
            [
                ...
                "writeFileSync(markerPath, JSON.stringify({ argv: process.argv.slice(2) }), 'utf8');",
                '',
            ].join('\n'),
            'utf8',
        );
        ...
        const marker = JSON.parse(await readFile(cliMarkerPath, 'utf8')) as { argv?: string[] };

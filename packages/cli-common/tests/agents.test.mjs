...
import { resolvePnpmReleaseAsset } from '../dist/agents/pnpmRelease.js';

function currentAgentInstallPlatform() {
...
async function createWindowsCommandShimFixture({ commandName, commandBody }) {
  const dir = await mkdtemp(join(tmpdir(), 'happier-cli-common-agent-winshim-'));
...
test('planAgentCliInstall returns source-aware plans for claude/codex/gemini', () => {
  const claude = planAgentCliInstall({ agentId: 'claude', platform: 'darwin' });
  ...
  const codex = planAgentCliInstall({ agentId: 'codex', platform: 'linux' });
  assert.equal(codex.ok, true);
  assert.equal(codex.plan.installMode, 'github_release_binary');
  assert.equal(codex.plan.commands.length, 0);

  const gemini = planAgentCliInstall({ agentId: 'gemini', platform: 'win32' });
...
test('planAgentCliInstall uses managed-package installs for qwen', () => {
  const qwen = planAgentCliInstall({ agentId: 'qwen', platform: 'win32' });
  assert.equal(qwen.ok, true);
  assert.equal(qwen.plan.installMode, 'managed_package');
  assert.equal(qwen.plan.commands.length, 0);
});

test('planAgentCliInstall uses a managed package for opencode', () => {
  const opencode = planAgentCliInstall({ agentId: 'opencode', platform: 'win32' });
...
test('installAgentCli runs vendor recipe commands on Windows', async () => {
...
test('installAgentCli gives vendor recipes a managed scratch TMPDIR on Unix', async () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('installAgentCli prepends common and agent-specific user bin dirs for vendor recipes on Unix', async () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('installAgentCli treats vendor recipe exits as success when the CLI becomes resolvable anyway', async () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('installAgentCli times out vendor recipe execution instead of hanging indefinitely', async () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('installAgentCli writes default install logs under HAPPIER_HOME instead of tmpdir', async () => {
  if (process.platform === 'win32') {
    return;
  }
  ...
  try {
    const homeDir = join(dir, 'home');
    const binDir = join(dir, 'bin');
    const expectedLogRoot = join(homeDir, 'logs', 'agent-installs');
    await mkdir(homeDir, { recursive: true });
    await mkdir(binDir, { recursive: true });
...
test('installAgentCli installs managed package-backed CLIs into the managed agent path', async () => {
...
`,
    ...
    const managedPath = resolveAgentCliManagedCommandPath('gemini', { happyHomeDir: homeDir });
...
test('installAgentCli bootstraps a managed JavaScript runtime for managed package-backed CLIs when no explicit runtime override exists', async () => {
...
test('installAgentCli reports managed-runtime-unavailable when no JavaScript runtime can be resolved for a managed package-backed CLI', async () => {
...
test('installAgentCli names HAPPIER_PNPM_BIN when an explicit pnpm override is invalid', async () => {
...
test('installAgentCli names HAPPIER_JS_RUNTIME_PATH when an explicit JS runtime override is invalid', async () => {
...
test('installAgentCli does not treat a system CLI as already-installed when explicitly installing a managed package-backed backend', async () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('installAgentCli still treats an existing managed package-backed backend as already-installed', async () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('installAgentCli installs managed github-release CLIs into the managed agent path', async () => {
...
test('installAgentCli fails closed for codex releases without a digest', async () => {
...
test('installAgentCli does not treat a system CLI as already-installed when explicitly installing a managed binary-backed backend', async () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('installAgentCli still treats an existing managed binary-backed backend as already-installed', async () => {
  if (process.platform === 'win32') {
    return;
  }
...
test('resolveAgentCliCommand does not treat non-executable PATH files as system commands on Unix', async () => {
  if (process.platform === 'win32') {
  ...
  try {
    ...
    const resolution = resolveAgentCliCommand('codex', { processEnv: env });
...
test('resolveAgentCliCommand falls back to ~/.local/bin for vendor-installed CLIs when PATH is missing the binary', async () => {
  if (process.platform === 'win32') {
    return;
  }
  ...
  try {
    ...
    const resolution = resolveAgentCliCommand('claude', {
      processEnv: {
        ...process.env,
        HOME: homeDir,
        PATH: '/usr/bin:/bin',
    ...
    const kimiResolution = resolveAgentCliCommand('kimi', {
      processEnv: {
        ...process.env,
        HOME: homeDir,
        PATH: '/usr/bin:/bin',
...
test('resolveAgentCliCommand falls back to agent-specific user bin directories when PATH is missing the binary', async () => {
  if (process.platform === 'win32') {
    return;
  }
  ...
  try {
    ...
    const resolution = resolveAgentCliCommand('opencode', {
      processEnv: {
        ...process.env,
        HOME: homeDir,
        PATH: '',

...
const archivePlanMode = vi.hoisted(() => ({ kind: 'command' as 'command' | 'library' }));
const runCommandStreamingMock = vi.hoisted(() => vi.fn(async (params: Readonly<{ args: ReadonlyArray<string> }>) => {
...
const extractArchivePayloadToDirectoryMock = vi.hoisted(() =>
  vi.fn(async (params: Readonly<{ extractDir: string }>) => {
    const { mkdir, writeFile } = await import('node:fs/promises');
    const { join } = await import('node:path');
    await mkdir(params.extractDir, { recursive: true });
    await writeFile(join(params.extractDir, process.platform === 'win32' ? 'pnpm.exe' : 'pnpm'), 'managed-pnpm-extracted', 'utf8');
    await mkdir(join(params.extractDir, 'dist'), { recursive: true });
    await writeFile(join(params.extractDir, 'dist', 'index.js'), 'managed-pnpm-support', 'utf8');
  }),
);
...
describe('managedPnpm bootstrap race protection', () => {
  ...
  it('extracts Windows zip assets through the library fallback without requiring tar', async () => {
    if (!originalPlatformDescriptor) {
    ...
    expect(command).toBe(managedPnpmBinPath(testEnv));
    expect(runCommandStreamingMock).not.toHaveBeenCalled();
    expect(extractArchivePayloadToDirectoryMock).toHaveBeenCalledTimes(1);
    await expect(readFile(managedPnpmBinPath(testEnv), 'utf8')).resolves.toBe('managed-pnpm-extracted');
    await expect(readFile(join(dirname(managedPnpmBinPath(testEnv)), 'dist', 'index.js'), 'utf8')).resolves.toBe('managed-pnpm-support');
  });

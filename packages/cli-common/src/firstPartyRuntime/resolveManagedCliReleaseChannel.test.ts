...
function withEmbeddedReleaseRingMarker<T>(releaseRing: string, run: (entrypointPath: string) => T): T {
  const rootDir = mkdtempSync(join(tmpdir(), 'happier-managed-cli-embedded-ring-'));
  const payloadRoot = join(rootDir, 'managed', 'current');
  const packageDistDir = join(payloadRoot, 'package-dist');
  const entrypointPath = join(packageDistDir, 'index.mjs');
  try {
    mkdirSync(packageDistDir, { recursive: true });
    writeFileSync(join(payloadRoot, 'public-release-ring.id'), `${releaseRing}\n`, 'utf8');
    writeFileSync(entrypointPath, 'export default null;\n', 'utf8');
    return run(entrypointPath);
  } finally {
...
describe('resolveManagedCliReleaseChannelSync', () => {
  ...
  it('uses explicit channel flags from argv when args are omitted', () => {
    const resolved = resolveManagedCliReleaseChannelSync({
      argv: ['happier', 'self', 'update', '--preview'],
  ...
  it('uses daemon service channel env before standard release channel env when requested', () => {
    const resolved = resolveManagedCliReleaseChannelSync({
  ...
  it('prefers an embedded payload release ring marker when launcher paths are generic', () => {
    withEmbeddedReleaseRingMarker('preview', (entrypointPath) => {
      const resolved = resolveManagedCliReleaseChannelSync({
        argv: [

...
import { readEmbeddedPublicReleaseRingFromPath } from './embeddedPublicReleaseRingMarker.js';
...
export function resolveManagedCliToolNameForRing(ring: PublicReleaseRingId): ManagedCliToolName {
  return resolveCliInvokerNameForPublicRing(ring);
}
...
function resolveInvokedToolName(candidates: readonly string[]): string | null {
  for (const candidate of candidates) {
    ...
    if (resolvePublicReleaseRingIdForCliInvokerName(name)) {
      return name;
    }
  }
  return null;
...
function resolveCandidateChannel(candidates: readonly string[]): ReleaseChannelCandidateResolution | null {
  ...
  for (const candidate of candidates) {
    ...
    if (ringFromEmbeddedMarker) {
      return { ringId: ringFromEmbeddedMarker, source: 'embedded-marker', invokedToolName };
    }
  }
  ...
  for (const candidate of candidates) {
    ...
    const ringFromInvoker = resolvePublicReleaseRingIdForCliInvokerName(name);
    if (ringFromInvoker) {
...
function resolveManagedCliReleaseChannelWithoutMarker(
  ...
  for (const candidates of [params.args ?? [], params.argv ?? []] as const) {
    const explicit = resolveExplicitArgChannel(candidates);
    if (explicit) {

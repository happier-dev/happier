...
/**
 * Release payloads are flat, while managed installs keep the executable and
 * everything it resolves relative to itself under installRoot/bin. Move that
 * complete closure as one unit so no installer can invent a partial layout.
 */
export async function relocateServerRuntimeArtifactClosure(params: Readonly<{
    ...
    for (const entryName of [names.server, names.migrate, ...SERVER_RUNTIME_DIRECTORY_ENTRY_NAMES]) {

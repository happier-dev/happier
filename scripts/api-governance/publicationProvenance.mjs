...
/** Reapplies prior publication facts without assigning a version to new source symbols. */
export function projectRetainedInventoryProvenance({
  inventory,
  ...
  const retainedInventory = validateInventory(retainedPublishedInventory);
  const retainedSinceBySymbol = new Map();
  for (const symbol of symbols(retainedInventory)) {
    // A generated working inventory can contain both previously published
    // symbols and newly added source symbols. Only the former have a `since`
    // fact to retain; absence means this symbol remains unpublished.
    if (symbol.since === undefined) continue;
    if (!isExactCanonicalPublishedVersion(symbol.since)) {
      fail(createError, `retained published inventory symbol ${symbolKey(symbol)} has invalid @since ${symbol.since}`);
    }

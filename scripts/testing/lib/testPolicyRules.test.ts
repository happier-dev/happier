...
// rg/grep skip it in silence -- returning exit 1 rather than any warning. Three files in
// the mention corridor became invisible to negative-requirement audits exactly this way.
...
test('collectPolicyFindings accepts the escaped NUL spelling', () => {
  const report = collectPolicyFindings([
    {
      filePath: 'apps/ui/sources/sync/domains/input/compositeKey.ts',
      content: 'export const key = (a: string, b: string): string => `${a}\\u0000${b}`;',
    },
  ]);

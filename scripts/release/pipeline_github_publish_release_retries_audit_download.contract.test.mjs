...
test('immutable release audit retries a transient download reset without re-uploading', () => {
  ...
  fs.writeFileSync(resolve(binDir, 'gh'), `#!/usr/bin/env node
const fs = require('node:fs');
const path = require('node:path');
const args = process.argv.slice(2);
fs.appendFileSync(${JSON.stringify(ghLog)}, \`gh \${args.join(' ')}\\n\`);
if (args[0] === 'release' && args[1] === 'view' && args.includes('--json')) { process.stdout.write('asset.txt\\n'); process.exit(0); }
...
if (args[0] === 'release' && args[1] === 'download') {
  ...
  if (count === 1) { process.stderr.write('read: connection reset by peer\\n'); process.exit(1); }
...
`, { encoding: 'utf8', mode: 0o755 });
  ...
  execFileSync(process.execPath, [
    ...
    '--tag', 'server-v0.3.0-dev.test',
    '--title', 'Happier Server v0.3.0-dev.test',
  ...
  assert.equal(log.match(/gh release download server-v0\.3\.0-dev\.test /g)?.length ?? 0, 3);
  assert.equal(log.match(/gh release upload server-v0\.3\.0-dev\.test /g)?.length ?? 0, 0);
});

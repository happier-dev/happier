...
test('self-host.sh suppresses non-actionable tar warnings and prints log guidance', async () => {
  const root = await mkdtemp(join(tmpdir(), 'happier-self-host-installer-'));
...
`,
  ...
  // Build a minimal hstack tarball.
  const version = '1.2.3';
  const artifactStem = `hstack-v${version}-linux-x64`;
  const artifactName = `${artifactStem}.tar.gz`;
  const artifactDir = join(fixtureDir, artifactStem);
  await mkdir(artifactDir, { recursive: true });
  const hstackBin = join(artifactDir, 'hstack');
  await writeFile(
    hstackBin,
    `#!/usr/bin/env bash
set -euo pipefail
...
`,
  ...
  const checksumsName = `checksums-hstack-v${version}.txt`;
  ...
  // Build a fake minisign archive; ensure_minisign should extract it and then run it.
  ...
  await writeFile(minisignX64, '#!/usr/bin/env bash\nexit 0\n', 'utf8');
  ...
  // sha256sum stub: return the pinned minisign archive SHA, and compute real SHA256 otherwise.
...
`,
  ...
  // Stub tar: emit noisy LIBARCHIVE warnings on extract so the installer must suppress them.
...
`,
  ...
  // curl stub: return release JSON (no -o), or copy fixture files to -o destinations.
...
`,
  ...
  const env = {
    ...
    HAPPIER_NONINTERACTIVE: '0',
    HAPPIER_WITH_CLI: '0',
    HAPPIER_GITHUB_TOKEN: '',
    GITHUB_TOKEN: '',
  };

  const res = spawnSync('bash', [installerPath, '--mode', 'user', '--channel', 'preview'], { env, encoding: 'utf8' });
  ...
  const expectedLogDir = join(env.HAPPIER_HOME, 'self-host', 'logs');
  assert.match(stdout, new RegExp(expectedLogDir.replaceAll('\\', '\\\\')));
  assert.match(stdout, /journalctl\s+--user\s+-u\s+happier-server/i);

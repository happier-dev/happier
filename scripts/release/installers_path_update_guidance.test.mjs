...
`,
  ...
  const homeExportLine = `export HAPPIER_HOME_DIR='${installDir.replaceAll("'", `'\"'\"'`)}'`;
...
test('install.sh shell-escapes custom HAPPIER_HOME_DIR exports written to rc files', async () => {
  const root = await mkdtemp(join(tmpdir(), 'happier-installer-home-escape-'));
  const homeDir = join(root, 'home');
  const binDir = join(root, 'bin');
  const outBinDir = join(root, 'out-bin');
  const fixtureDir = join(root, 'fixture');
  const installDir = join(root, `install"$(touch ${join(root, 'rc-pwned')})"space`);
...
`,
  ...
  const bashrc = await readFile(join(homeDir, '.bashrc'), 'utf8');
  const profile = await readFile(join(homeDir, '.profile'), 'utf8');
  const expectedHomeExportLine = `export HAPPIER_HOME_DIR='${installDir.replaceAll("'", `'\"'\"'`)}'`;

  assert.ok(bashrc.includes(expectedHomeExportLine), 'expected installer to single-quote and escape ~/.bashrc HAPPIER_HOME_DIR exports');
  assert.ok(profile.includes(expectedHomeExportLine), 'expected installer to single-quote and escape ~/.profile HAPPIER_HOME_DIR exports');
  assert.doesNotMatch(bashrc, /export HAPPIER_HOME_DIR=".*\$\(|.*`/, 'expected ~/.bashrc export to avoid command substitution syntax');
  assert.doesNotMatch(profile, /export HAPPIER_HOME_DIR=".*\$\(|.*`/, 'expected ~/.profile export to avoid command substitution syntax');

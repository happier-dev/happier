...
test('promote-ui always verifies exact Mermaid generated bytes before candidate promotion', async () => {
  ...
  const bumpIndex = steps.findIndex((step) => step?.id === 'bump_app');
  ...
  assert.ok(verifyIndex < bumpIndex, 'generated bytes must be verified before candidate mutation/promotion');
});

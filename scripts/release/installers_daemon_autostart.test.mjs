...
test('install.sh --yes runs non-interactively without HAPPIER_NONINTERACTIVE and creates no unrequested service state', async () => {
  ...
  try {
    // With the same environment but no flag, the installer renders the
    // post-install summary (`doctor repair --report-only`). `--yes` must take
    // the non-interactive path instead, and must not install or repair a
    // background service nobody asked for.
    assert.equal(
      scenario.log.trim(),
      '',

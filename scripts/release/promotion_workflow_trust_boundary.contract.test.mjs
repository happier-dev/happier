...
test('promotion release actor guards load only immutable workflow control before App secrets', async () => {
  for (const file of promotionWorkflows) {
    ...
    assert.ok(guard, `${file} release_actor_guard job`);
...
test('promotion candidates execute only in secret-free validation jobs and privileged jobs keep trusted control at root', async () => {
  for (const file of promotionWorkflows) {
    const { parsed } = await loadWorkflow(file);
    const resolveSource = parsed?.jobs?.resolve_source;
    const validateCandidate = parsed?.jobs?.validate_candidate;
    const promote = parsed?.jobs?.promote;

    assert.ok(resolveSource, `${file} resolve_source job`);
    ...
    assert.ok(promote, `${file} promote job`);
    ...
    for (const step of promote.steps) {
      if (step?.uses === 'actions/checkout@11d5960a326750d5838078e36cf38b85af677262' && step?.with?.path) {
        assert.equal(step.with['persist-credentials'], false, `${file} nested data checkout must not persist credentials`);
      }
...
test('promotion source refs and OTA messages cannot become shell syntax', async () => {
  const maliciousSource = 'main"; touch /tmp/happier-source-injection; #';
  const maliciousMessage = 'release $(touch /tmp/happier-message-injection) `id`';
  assert.equal(process.env.PROMOTION_TEST_SOURCE = maliciousSource, maliciousSource);
  assert.equal(process.env.PROMOTION_TEST_MESSAGE = maliciousMessage, maliciousMessage);
...
test('promote-ui prepares OTA bytes without secrets and publishes only the bound prepared artifact', async () => {
  ...
  const validate = parsed?.jobs?.validate_candidate;
  const promote = parsed?.jobs?.promote;

...
test('install.sh installs preview CLI assets from a mixed local asset directory without drifting to stable', async () => {
  const root = await mkdtemp(join(tmpdir(), 'happier-installer-cli-mixed-local-assets-'));
  const binDir = join(root, 'bin');
  const installDir = join(root, 'install');
  const outBinDir = join(root, 'out-bin');
  const fixtureDir = join(root, 'fixture');
  ...
  const preview = await writeLegacyCliArtifact(fixtureDir, previewVersion, 'preview');
  const stable = await writeLegacyCliArtifact(fixtureDir, stableVersion, 'stable');
...
printf '%s\\0' \
  '${preview.tarPath}' \
  '${stable.tarPath}' \
  '${preview.checksumsPath}' \
  '${stable.checksumsPath}' \
  '${preview.sigPath}' \
  '${stable.sigPath}'
`,
    'utf8',
  );
  await chmod(join(binDir, 'find'), 0o755);

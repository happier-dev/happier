...
test('install.sh hands a fresh interactive install off to `happier setup`', async () => {
  ...
  const stderr = String(res.stderr ?? '').replaceAll('\r', '');
...
test('install.ps1 requires both authentication and machine registration before skipping setup', async () => {
  const source = await readInstallPs1();
  const decision = extractPowerShellFunction(source, 'Test-InstalledCliReportsConfiguredMachine');
  assert.ok(decision, 'expected the installed CLI readiness probe');
  assert.match(decision, /"authenticated"\s*\\s\*:\\s\*true/);
  assert.match(decision, /"machineRegistered"\s*\\s\*:\\s\*true/);
  assert.match(decision, /-and/, 'both readiness facts must be required');
});

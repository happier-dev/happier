...
/**
 * npm's `files` field accepts broad parent and glob patterns, so source-tree
 * inspection cannot prove what reaches the registry. The publication boundary
 * reads the generated tarball directly and keeps the CLI's managed wrapper
 * archives under the same canonical manifest/checksum owner as postinstall.
 *
 * @param {string} tarballPath
 */
function assertCliManagedRuntimeTarballPublication(tarballPath) {
  let listedEntries;
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    fail(`[npm-publish] Unable to inspect CLI tarball contents: ${message}`);
  }
  ...
  if (entries.some((entry) => entry === unpackedPrefix || entry.startsWith(`${unpackedPrefix}/`))) {
    fail('[npm-publish] CLI tarball must not contain tools/unpacked runtime content; publish checksummed tools/archives only');
  }
  ...
  if (entryCount(checksumPath) !== 1) {
    fail(`[npm-publish] CLI tarball must contain exactly one ${checksumPath}`);
  }
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    fail(`[npm-publish] Unable to read CLI runtime archive checksum inventory: ${message}`);
  }
  ...
  for (const runtimeAsset of getCliRuntimeAssetArchiveManifest()) {
    ...
    if (!expectedChecksum) {
      fail(`[npm-publish] CLI tarball is missing a checksum inventory entry for ${runtimeAsset.archiveName}`);
    }
    let archiveBytes;
    try {
      ...
      const message = err instanceof Error ? err.message : String(err);
      fail(`[npm-publish] Unable to read CLI managed runtime archive ${runtimeAsset.archiveName}: ${message}`);
    }
...
function main() {
  ...
  if (metadata) {
    if (metadata.name === CLI_PACKAGE_NAME) {

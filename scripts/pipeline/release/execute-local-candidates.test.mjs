...
test('local candidate execution accepts the public dev channel without leaking its internal ring name', () => {
  const commands = buildLocalCandidateCommands({
    channel: 'dev',
    sourceSha: SHA,
    repository: 'happier-dev/happier',
    candidates: { cli: '1.2.3-dev.4' },
    phase: 'verify',
    releaseMessage: '',
  });
  assert.deepEqual(commands.map((command) => command.stage), ['verify:cli']);
  assert.deepEqual(
    commands[0].args.slice(commands[0].args.indexOf('--channel'), commands[0].args.indexOf('--channel') + 2),
    ['--channel', 'dev'],
  );
});

 ...
 * @param {string} repoRoot
 */
function readHstackVersion(repoRoot) {
  const pkgJsonPath = withinRepo(repoRoot, path.join('apps', 'stack', 'package.json'));
...
async function preflightMinisignKey({ dryRun }) {
  ...
  if (!keyRaw) {
    fail('[pipeline] MINISIGN_SECRET_KEY is required to publish signed hstack release artifacts.');
  }
...
async function ensureCleanArtifactsDir(repoRoot, opts) {
  const rel = 'dist/release-assets/stack';
...
async function main() {
  ...
  if (channel === 'stable' && !allowStable) {
    fail('Stable hstack binary publishing is disabled. Re-run with --allow-stable true if intentional.');
  }
  ...
  try {
    if (channel === 'preview') {
      if (dryRun) {
        ...
        restoreVersion = patchPackageVersion(stackPkgJson, version);
      }
    }
    ...
    run(
      opts,
      process.execPath,
      ['scripts/pipeline/release/build-hstack-binaries.mjs', '--channel', channel, '--version', version],
      {
        cwd: repoRoot,
        env: {
          ...process.env,
    ...
    const artifactsDir = withinRepo(repoRoot, 'dist/release-assets/stack');
    const checksums = withinRepo(repoRoot, `dist/release-assets/stack/checksums-hstack-v${version}.txt`);
    const signature = withinRepo(repoRoot, `dist/release-assets/stack/checksums-hstack-v${version}.txt.minisig`);
    const manifestPath = withinRepo(repoRoot, `dist/release-assets/stack/manifests/v1/hstack/${channel}/latest.json`);
    ...
    run(
      ...
      [
        ...
        version,
        '--artifacts-dir',
        'dist/release-assets/stack',
        '--out-dir',
        'dist/release-assets/stack/manifests',
        '--assets-base-url',
        assetsBaseUrl,
        '--commit-sha',
        targetSha,

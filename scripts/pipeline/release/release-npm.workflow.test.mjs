...
test('npm publication workflow is reusable-only and receives its exact source identity from release admission', async () => {
  ...
  assert.match(releaseWorkflow, /PLUGIN_SDK_API_CLASSIFICATION: \$\{\{ needs\.release_preflight\.outputs\.plugin_sdk_api_classification \}\}/mu);
  assert.match(releaseWorkflow, /SDK_API_CLASSIFICATION: \$\{\{ needs\.release_preflight\.outputs\.sdk_api_classification \}\}/mu);
  assert.match(
    releaseWorkflow,

 ...
 *   originRun: unknown;
 *   artifacts: unknown;
 *   expected: { repository: string; workflowPath: string; channel: string; sourceSha?: string };
 * }} input
 */
...
export function resolveReleaseResume(input) {
  ...
  const statusRunId = typeof status.run === 'number'
    ? status.run
    : Number(requiredString(status.run, 'resume status run'));

...
> **Local host policy:** Argent is CLI-only on this machine. Invoke
> `gather-workspace-data` as `argent run gather-workspace-data ...`; inspect its
> exact arguments first with `argent run gather-workspace-data --help`. Do not
> start `argent mcp`, and stop the shared server when finished.
...
1. **Run `argent run gather-workspace-data`** with the project's workspace path.

...
Re-read this when a task is hard, ambiguous, or high-stakes, and after context compaction on long-running work. The procedures matter most exactly when you feel sure — confidence and correctness come apart precisely where you cannot feel the difference from the inside.
...
4. Numbers reported by anyone — line counts, test results, coverage, benchmarks, including past-you — get **re-measured**, not trusted. This repo's history includes delegate lanes reporting "green" suites that were red at their own commit, and inflated LOC deltas. Not malice; drift. Re-measurement is cheap; propagated error is not.
...
2. Run the standard attacks, in order of cheapness:
   - **Alternative cause:** what else explains all the same evidence? If you can't name a second candidate, you haven't looked — real evidence almost always underdetermines the cause.
...
4. **Big diffs as progress.** Rewriting what you don't understand instead of understanding it. Motion mistaken for work. Its mirror twin is equally fake: **minimal-diff piety** — leaving the real fix undone to keep the patch small. The right size is the size of the coherent fix, no more, no less.

...
- target, intent basis, observed moving-work basis (current HEAD, dirty-state acknowledgement, and relevant current paths), and runtime/artifact identity only where the reviewed claim consumes one;
...
- Record a concise observed basis: plan revision when applicable, current HEAD and dirty-state acknowledgement, relevant paths/symbols/flows, checks run, and runtime artifact where relevant. Do not freeze or hash worktrees, create source-candidate manifests, leases, receipts, custody records, or global invalidation machinery.

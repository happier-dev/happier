...
- Source development may start from a valid last-green output while changed source outputs refresh in the background. For a checkout-derived repository producer, `dev.mjs` schedules successful server/daemon refreshes through the canonical publisher: one publication may run at a time and later refreshes coalesce into one trailing identity recomputation. The repository source owner is the only place allowed to advance a producer snapshot; a controlled consumer never starts a competing publisher.

...
Repair the originating layer. A plan-level correction that changes approved outcomes remains `AMENDMENT_REQUIRED` under `skills/happier-implement-plan`; a harness defect is not evidence that production is wrong; an environment failure is not fixed by weakening product behavior.

...
Keep the public comment and exact GitHub disposition separate. A blocking evidence request normally proposes `needs:reporter` plus removal of `needs:maintainer`; a correction awaiting project-side merge, release, or follow-up remains `needs:maintainer`. The workflow's hidden saved-reply directives are a manual maintainer convenience, not an agent mutation channel.
